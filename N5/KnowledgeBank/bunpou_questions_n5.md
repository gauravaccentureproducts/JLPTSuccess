# JLPT N5 Bunpou (文法) Practice Questions

100 questions covering the Bunpou (grammar) section of the JLPT N5. The Bunpou section tests three skills: choosing the correct grammar form, arranging words into a sentence, and using grammar across a connected passage.

## Subtypes covered

| Mondai | Subtype | Count |
|---|---|---|
| Mondai 1 | 文の文法1 (sentence grammar 1) - choose the correct grammar form | 60 |
| Mondai 2 | 文の文法2 (sentence composition / 並べ替え) - arrange 4 elements, identify what goes in the starred position | 30 |
| Mondai 3 | 文章の文法 (text grammar) - short passage with multiple grammar blanks | 10 |

## Notation rules

- The blank in Mondai 1 is shown as `（  　）`.
- In Mondai 2, the four shuffled elements are listed as 1-4 and the original sentence shows positions A B C D where one is marked `★`. The answer identifies which option-number goes in the `★` slot.
- Mondai 3 passages have multiple inline blanks numbered `[ 1 ]` `[ 2 ]` etc., each with its own 4-option set.
- All stems and correct answers use only N5-syllabus kanji. Distractors may include non-N5 kanji where authentic JLPT format requires it.
- No em dashes (U+2014) appear in this file.

## Engine display note

For mock-test mode, the app's test engine MUST hide the `**Answer:**` line and rationale until the student commits an answer. The visible-by-default format here is for self-study reference; runtime test rendering is the engine's responsibility.

## Numeral convention

Numbers are written using both kanji forms (一, 二, 三, 五, 十, 百, 千) and arabic numerals (1, 2, 100, 1000) - mirroring authentic JLPT papers, which use kanji numerals in narrative text and arabic numerals in prices, addresses, schedules, and time tables. This is intentional, not inconsistency.

## Paper segmentation policy (formalized 2026-05-04, ISSUE-007)

`data/papers/bunpou/paper-N.json` files segment this 100-question corpus into 7 papers using a **Q-order slice rule**: paper-1 covers Q1-Q15, paper-2 covers Q16-Q30, ..., paper-7 covers Q91-Q100. The mapping is recorded in each paper-JSON's `source_question_range` field and in `data/papers/manifest.json`.

**Why Q-order slices, not JLPT-format mixed-Mondai:**
- The Q-numbered MD source is the canonical reference. A learner reading the MD or comparing notes against the deployed papers can predict exactly which paper a Q is in.
- Mondai 2 (Q61-90, sentence rearrangement) has a fixed choice-encoding: each numbered fragment is part of the test data. Choice order cannot be permuted without breaking the question. This conflicts with the per-paper position-balance goal that mixed-Mondai segmentation is supposed to deliver.
- The autonomous-improvement Round 4 rebalance (v1.12.27) achieved 25/25/25/25 corpus-wide for bunpou by re-permuting only the unconstrained Mondai 1 + Mondai 3 items. Two papers (paper-5 [4,5,1,5] and paper-6 [7,1,4,3]) are still per-paper-skewed because every item in those papers is constrained Mondai 2.

**Accepted-with-rationale cost:** a learner practicing paper-5 in isolation will see C-position 1 time out of 15. A test-wise student exploiting that pattern would gain at most ~5 percentage points on a single paper. Across the corpus the distribution is exactly uniform, so multi-paper practice is unbiased.

**Future enhancement (out of current scope):** an "exam-realism" test mode that virtually constructs a 15- or 17-item mixed-Mondai paper at runtime by sampling 9 from Mondai 1 + 4 from Mondai 2 + 4 from Mondai 3. This delivers the JLPT paper format without touching the source-of-truth Q-order mapping. Tracked as ISSUE-006 in the 2026-05-04 audit.

---

## Mondai 1 - 文の文法1 (Sentence Grammar 1)

60 questions. Choose the form that fits the blank.

### Q1

わたし（　　）たなかです。

1. が
2. は
3. を
4. に

**Answer: 2** - topic marker は.

### Q2

きのう ともだち（　　）えいがを 見ました。

1. を
2. と
3. が
4. に

**Answer: 2** - 友だちと (with).

### Q3

あさ、コーヒー（　　）のみました。

1. が
2. に
3. を
4. と

**Answer: 3** - direct object marker.

### Q4

学校（　　）バスで いきます。

1. を
2. は
3. へ
4. が

**Answer: 3** - direction.

### Q5

きょう（　　）何曜日 ですか。

1. が
2. を
3. は
4. に

**Answer: 3** - topic marker.

### Q6

ぎんこうは 駅（　　）まえに あります。

1. を
2. の
3. が
4. は

**Answer: 2** - possessive / location-of.

### Q7

七時（　　）おきます。

1. と
2. に
3. を
4. で

**Answer: 2** - time of action.

### Q8

きょうしつ（　　）日本語を べんきょうします。

1. に
2. へ
3. を
4. で

**Answer: 4** - location of action (where you do it).

### Q9

としょかん（　　）本を よみます。

1. に
2. へ
3. を
4. で

**Answer: 4** - location of action.

### Q10

たなかさん（　　）アメリカへ いきました。

1. を
2. に
3. が
4. は

**Answer: 4** - new topic (or focus).

### Q11

つくえの 上に 本（　　）あります。

1. に
2. を
3. が
4. は

**Answer: 3** - existence subject.

### Q12

きょうしつに 学生（　　）三人 います。

1. を
2. は
3. に
4. が

**Answer: 4** - existence subject.

### Q13

おかね（　　）ありませんから、何も 買いません。

1. を
2. に
3. は
4. が

**Answer: 4** - subject of ある (negative).

### Q14

わたしは ねこ（　　）すきです。

1. を
2. が
3. に
4. の

**Answer: 2** - sub-が-suki: subject of suki takes が. Stem now anchored with わたしは to disambiguate from contrastive-topic は reading.

### Q15

日本語（　　）すこし わかります。

1. を
2. へ
3. に
4. が

**Answer: 4** - わかる takes が.

### Q16

ピアノ（　　）できます。

1. を
2. が
3. に
4. へ

**Answer: 2** - できる takes が.

### Q17

新しい かばん（　　）ほしいです。

1. を
2. と
3. に
4. が

**Answer: 4** - ほしい takes が.

### Q18

毎日 一時間 ピアノ（　　）れんしゅうします。

1. に
2. が
3. を
4. へ

**Answer: 3** - を marks the direct object of れんしゅうする (practice the piano).

### Q19

ねつ（　　）あります。

1. を
2. と
3. に
4. が

**Answer: 4** - ねつが ある (have a fever).

### Q20

きょう（　　）あした、しけんが あります。

1. か
2. と
3. に
4. を

**Answer: 1** - か (or).

### Q21

その 本は たなかさん（　　）もらいました。

1. に
2. が
3. を
4. と

**Answer: 1** - source for もらう.

### Q22

子どもに 本（　　）あげます。

1. が
2. に
3. を
4. と

**Answer: 3** - direct object.

### Q23

学校は 八時（　　）三時までです。

1. と
2. へ
3. に
4. から

**Answer: 4** - from-to.

### Q24

とうきょう（　　）おおさかまで しんかんせんで いきます。

1. に
2. を
3. から
4. と

**Answer: 3** - from.

### Q25

ペン（　　）三本 あります。

1. を
2. が
3. に
4. へ

**Answer: 2** - 「ペンが 三本 あります」: が marks the existing subject.

### Q26

毎日 日本語（　　）べんきょうします。

1. が
2. に
3. を
4. と

**Answer: 3** - を marks the direct object of べんきょうする.

### Q27

毎日 学校で ともだちに あい（　　）。

1. ます
2. ません
3. ました
4. ませんでした

**Answer: 1** - 毎日 (every day) + present-affirmative ます for habitual action.

### Q28

きのう、おさけを のみ（　　）。

1. ます
2. ません
3. ました
4. ませんでした

**Answer: 3** - past affirmative (kinou + drank).

### Q29

きのうは あめ（　　）。

1. です
2. でした
3. じゃありません
4. ありません

**Answer: 2** - past copula.

### Q30

私は 学生 では （　　）。

1. あります
2. ありません
3. ありました
4. ない

**Answer: 2** - polite negative.

### Q31

その カメラは とても （　　）です。

1. たかい
2. たかいの
3. たかく
4. たかくて

**Answer: 1** - i-adj predicate.

### Q32

この りんごは あまり （　　）。

1. おいしいです
2. おいしくなかったです
3. おいしかったです
4. おいしくないです

**Answer: 4** - あまり + negative.

### Q33

きのう 食べた りょうりは とても （　　）です。

1. おいしかった
2. おいしく
3. おいしい
4. おいしくない

**Answer: 1** - past i-adj.

### Q34

きょうしつは あまり （　　）。

1. しずかじゃ ありません
2. しずかで
3. しずかな
4. しずかでした

**Answer: 1** - あまり + neg construction. しずかじゃ ありません is the clean N5 polite-negative i-na-adj form (replaces colloquial 「しずかじゃない」+です from a prior version).

### Q35

たなかさんは とても しんせつ （　　） 人です。

1. の
2. に
3. で
4. な

**Answer: 4** - na-adj + な + Noun.

### Q36

きょうは 雨ですから、外へ いき （　　）。

1. ます
2. ましょう
3. たいです
4. ません

**Answer: 4** - rain → don't go.

### Q37

おなかが すきましたから、何か 食べ （　　）。

1. たいです
2. ます
3. ません
4. ましょうか

**Answer: 1** - hungry + want to eat.

### Q38

しごとが ありますから、はやく いえへ かえり （　　）。

1. ません
2. ました
3. ます
4. たいでした

**Answer: 3** - present time, going home.

### Q39

つかれましたから、すこし やすみ （　　）。

1. ます
2. たい
3. ました
4. ません

**Answer: 2** - want to rest.

### Q40

しごとが おわってから 何（　　）食べに 行きませんか。

1. に
2. を
3. か
4. が

**Answer: 3** - 何か.

### Q41

つくえの 上に 本が 三（　　）あります。

1. ぼん
2. ぽん
3. さつ
4. ほん

**Answer: 3** - 三 + さつ (counter for books). Stem now includes the numeral 三 so the counter has something to attach to (prior version was structurally broken).

### Q42

その こうえんで 子どもが あそんで （　　）。

1. ます
2. います
3. あります
4. なります

**Answer: 2** - ている for ongoing action.

### Q43

あさごはんを （　　） から、学校へ いきます。

1. たべる
2. たべて
3. たべた
4. たべます

**Answer: 2** - て-form + から (after doing).

### Q44

その 本を かして （　　）。

1. ます
2. ください
3. たいです
4. なります

**Answer: 2** - polite request.

### Q45

ここで しゃしんを とって （　　）か。

1. ます
2. たいです
3. もいいです
4. ください

**Answer: 3** - permission + question.

### Q46

きょうしつでは たばこを すっては （　　）。

1. いいです
2. ください
3. いけません
4. たいです

**Answer: 3** - 「~ては いけません」 expresses prohibition.

### Q47

あさ おきて、コーヒーを （　　）、ニュースを 見ます。

1. のんだ
2. のみます
3. のんで
4. のむ

**Answer: 3** - て-form for sequential.

### Q48

きょうは あめが ふって、かぜも （　　）。

1. つよい
2. つよいです
3. つよくて
4. つよかった

**Answer: 2** - parallel adjective predicate.

### Q49

その みせの ケーキは おいしくて、（　　）です。

1. やすかった
2. やすく
3. やすい
4. やすくない

**Answer: 3** - chained i-adj predicates.

### Q50

きょうは あつい（　　）、まどを あけました。

1. と
2. から
3. けど
4. が

**Answer: 2** - reason connector. けど (option 3) and が (option 4) are concessive ("although"), not causal; they would invert the logic.

### Q51

きのうは いそがしかった（　　）、しゅくだいを しませんでした。

1. と
2. けど
3. が
4. から

**Answer: 4** - reason connector with past adj. けど (option 2) and が (option 3) are concessive; they would invert the logic.

### Q52

うちに 来る （　　）に 電話して ください。

1. まえ
2. あと
3. うしろ
4. うえ

**Answer: 1** - before doing X.

### Q53

しゅくだいを した （　　）で、テレビを 見ます。

1. あと
2. まえ
3. なか
4. うえ

**Answer: 1** - after doing X.

### Q54

ねる まえに、はを みがき（　　）。

1. ます
2. ました
3. たいです
4. ません

**Answer: 1** - habitual present.

### Q55

私は 中国へ 行った こと（　　）あります。

1. を
2. が
3. に
4. へ

**Answer: 2** - こと takes が with ある.

### Q56

スーパーで パンや ぎゅうにゅう（　　）を 買いました。

1. を
2. と
3. に
4. など

**Answer: 4** - listing with や...など.

### Q57

きょうは てんき（　　）いいですから、こうえんへ 行きましょう。

1. が
2. を
3. の
4. に

**Answer: 1** - subject of i-adj predicate.

### Q58

きのう、ともだち（　　）うちへ 来ました。

1. を
2. に
3. が
4. と

**Answer: 3** - subject of intransitive 来る.

### Q59

中国語 と えいご（　　）、どちらが むずかしいですか。

1. に
2. と
3. の
4. が

**Answer: 2** - listing for どちら comparison.

### Q60

とうきょうは おおさか（　　）大きいです。

1. の
2. が
3. と
4. より

**Answer: 4** - comparison より.

---

## Mondai 2 - 文の文法2 (Sentence Composition)

30 questions. Arrange the four numbered options into the sentence so it reads correctly. The position marked `★` is the one whose option-number is the answer.

### Q61

きのう ___ ___ ★ ___ 見ました。

1. を
2. ともだちと
3. えいが
4. いっしょに

**Answer: 3** (えいが goes in ★).

### Q62

わたしは ___ ___ ★ ___ あります。

1. ことが
2. 行った
3. に
4. 中国

**Answer: 2** (行った goes in ★).

### Q63

あした ___ ___ ★ ___ 行きます。

1. に
2. の
3. ともだち
4. うち

**Answer: 4** (うち goes in ★).

### Q64

___ ___ ★ ___ ですか。

1. 何
2. あの 店
3. の
4. 名前は

**Answer: 4** (名前は goes in ★). Order: あの店(2) の(3) 名前は(4=★) 何(1) ですか = "What is the name of that shop?"

### Q65

わたしの ___ ___ ★ ___ あります。

1. に
2. うえ
3. 本が
4. つくえの

**Answer: 1** (に goes in ★).

### Q66

きょうは ___ ___ ★ ___ 食べました。

1. を
2. パン
3. ホテルで
4. あさごはんに

**Answer: 2** (パン goes in ★).

### Q67

___ ___ ★ ___ ありませんか。

1. に
2. しゅくだいは
3. あした
4. まで

**Answer: 4** (まで goes in ★).

### Q68

___ ___ ★ ___ かりました。

1. を
2. 友だちに
3. 三さつ
4. 本

**Answer: 1** (を goes in ★).

### Q69

___ ___ ★ ___ あいました。

1. の
2. 学校
3. 友だちに
4. まえで

**Answer: 4** (まえで goes in ★).

### Q70

___ ___ ★ ___ よみたいです。

1. ある
2. 本を
3. としょかんに
4. 三さつ

**Answer: 2** (本を goes in ★).

### Q71

___ ___ ★ ___ きました。

1. ふって
2. が
3. 雨
4. たくさん

**Answer: 4** (たくさん goes in ★).

### Q72

れいぞうこの ___ ___ ★ ___ ありますか。

1. に
2. のみもの
3. 中
4. が

**Answer: 2** (のみもの goes in ★).

### Q73

わたしは ___ ___ ★ ___ ません。

1. に
2. あまり
3. ぎんこう
4. 行き

**Answer: 1** (に goes in ★).

### Q74

___ ___ ★ ___ ですか。

1. の
2. かばん
3. だれ
4. これは

**Answer: 1** (の goes in ★).

### Q75

あした ___ ___ ★ ___ 来ません。

1. は
2. パーティーに
3. から
4. しごとが あります

**Answer: 2** - Order: しごとが あります(4) から(3) パーティーに(2=★) は(1) 来ません = "Because I have work, I won't come to the party." (から replaces ので per N5-canonical reason-conjunction policy.)

### Q76

___ ___ ★ ___ じょうずです。

1. が
2. 日本語
3. たなかさんは
4. とても

**Answer: 1** (が goes in ★).

### Q77

___ ___ ★ ___ ましょう。

1. へ
2. いっしょに
3. 行き
4. こうえん

**Answer: 1** (へ goes in ★).

### Q78

___ ___ ★ ___ きてください。

1. もって
2. を
3. その本
4. ここに

**Answer: 1** (もって goes in ★).

### Q79

___ ___ ★ ___ 食べました。

1. ぜんぶ
2. ケーキ
3. を
4. その

**Answer: 3** (を goes in ★).

### Q80

___ ___ ★ ___ ありますか。

1. が
2. しゅくだい
3. あした
4. は

**Answer: 2** (しゅくだい goes in ★).

### Q81

___ ___ ★ ___ 行きました。

1. と
2. に
3. えいがかん
4. 友だち

**Answer: 3** (えいがかん goes in ★).

### Q82

___ ___ ★ ___ ですか。

1. の
2. これ
3. 本
4. は だれ

**Answer: 1** (の goes in ★).

### Q83

しゅくだいを ___ ___ ★ ___ 食べました。

1. してから
2. ばんごはんを
3. いえで
4. みんなで

**Answer: 4** (みんなで goes in ★). Order: してから(1) いえで(3) みんなで(4=★) ばんごはんを(2) 食べました = "After homework, everyone ate dinner together at home."

### Q84

___ ___ ★ ___ あります。

1. の うえに
2. やま
3. ゆきが
4. たくさん

**Answer: 4** (たくさん goes in ★).

### Q85

私は ___ ___ ★ ___ おもいます。

1. が
2. 新しい
3. ほしいと
4. かばん

**Answer: 1** (が goes in ★). Order: 新しい(2) かばん(4) が(1=★) ほしいと(3) おもいます.

### Q86

___ ___ ★ ___ できます。

1. すこし
2. ピアノを
3. ひくことが
4. たなかさんは

**Answer: 1** (すこし goes in ★).

### Q87

___ ___ ★ ___ おきます。

1. に
2. あさ
3. 七時
4. まいにち

**Answer: 3** (七時 goes in ★).

### Q88

___ ___ ★ ___ いませんから、たべません。

1. が
2. おなか
3. すいて
4. まだ

**Answer: 4** (まだ goes in ★).

### Q89

___ ___ ★ ___ いません。

1. しゅくだい
2. が
3. まだ
4. おわって

**Answer: 3** (まだ goes in ★).

### Q90

わたしは ___ ___ ★ ___ つもりです。

1. に
2. らいねん
3. 大学
4. 行く

**Answer: 1** (に goes in ★).

---

## Mondai 3 - 文章の文法 (Text Grammar)

10 questions across 2 short passages. Each passage has 5 numbered blanks; pick the best fit for each.

### Passage A (Q91-Q95)

> たなかさんの 一日

> わたしは まいにち あさ 六時 [ 1 ] おきます。 七時ごろ [ 2 ] あさごはんを 食べます。 あさごはんは いつも パン [ 3 ] ぎゅうにゅうです。 ８時に いえを 出て、電車で 学校 [ 4 ] 行きます。 学校は 九時 [ 5 ] 三時までです。

#### Q91 (blank 1)

1. で
2. を
3. が
4. に

**Answer: 4** - time + に.

#### Q92 (blank 2)

1. に
2. が
3. を
4. へ

**Answer: 1** - approximate time (七時ごろ) + に for time of action.

#### Q93 (blank 3)

1. に
2. と
3. を
4. の

**Answer: 2** - listing と.

#### Q94 (blank 4)

1. で
2. を
3. へ
4. が

**Answer: 3** - direction. (Note: に would also be grammatically valid for a motion-verb destination at N5, so it has been replaced with で as a clearly-wrong distractor: 学校で 行きます would mark 学校 as the location of the going-action, which is ungrammatical.)

#### Q95 (blank 5)

1. へ
2. を
3. から
4. に

**Answer: 3** - 9 o'clock from / starting at.

### Passage B (Q96-Q100)

> わたしの しゅみ

> わたしは おんがく [ 1 ] すきです。 まいにち、 学校から かえってから、いえで ピアノを 一時間 [ 2 ]。 にちようびは 友だちと いっしょに [ 3 ] に 行きます。 友だち も ピアノが じょうずですから、 とても たのしいです。 こんしゅうは いそがしくて、ピアノを ひく 時間 [ 4 ] ありませんでした。 来週は ぜったいに 一日 [ 5 ] ひきたいと おもいます。

#### Q96 (blank 1)

1. に
2. が
3. を
4. と

**Answer: 2** - すき takes が.

#### Q97 (blank 2)

1. ひきます
2. ひいて います
3. ひきました
4. ひきたいです

**Answer: 1** - habitual present.

#### Q98 (blank 3)

1. ピアノを ひいて
2. ピアノきょうしつ
3. ピアノで うた
4. ピアノは すき

**Answer: 2** - 「ピアノきょうしつ」 (compound noun, no の) - に行きます target. (The original option 4 「ピアノを 買い」 was dropped because Verb-stem + に+行く is grammatically valid, making it a structurally-correct second answer; replaced with 「ピアノは すき」 which is broken before に行きます.)

#### Q99 (blank 4)

1. が
2. を
3. に
4. の

**Answer: 1** - time + が + ありません (no time).

#### Q100 (blank 5)

1. ぐらい
2. しか
3. だけ
4. でも

**Answer: 4** - 「でも」 attached to a quantity means "even (just)" - 一日でも = "even (just) one day". Combined with 「ぜったいに」, this expresses "I definitely want to play, even just for one day next week." 「ぐらい」 (approximate) would clash with 「ぜったいに」 (firm resolve).

---

## End of file

100 questions complete: 60 文の文法1 + 30 文の文法2 + 10 文章の文法.

Continues in `dokkai_questions_n5.md`.
