# JLPT N5 Tutor - Design System: Zen Modern

**Status:** LIVE. Canonical source of truth for the N5 app visual and interaction design.
**Checked by:** `tools/check_design_system.py` (8 D-rules: D-1..D-8), wired into `.github/workflows/content-integrity.yml` as a **non-blocking visibility check** (`continue-on-error: true`) - it logs a known backlog of pre-existing CSS/JS violations rather than failing the build, pending a design-cleanup close-out. `css/main.css`, the tool's docstring, and the workflow's `# Source:` comment all name this file as their source of truth.
**Companion:** functional, data, and governance contracts live in `JLPT-N5-Current-Implementation-Spec.md` (its section 36 points here for design tokens).
**Provenance:** this design system was briefly merged into the v3.1 functional-spec supplement (2026-05-04), then restored to its own file on 2026-05-29 when the supplement functional content (sections A-F) was folded into the canonical implementation spec (sections 26-38) and the supplement retired. The full subsection-level redirect table is preserved in git history; see spec section 38 for the supersession record.
