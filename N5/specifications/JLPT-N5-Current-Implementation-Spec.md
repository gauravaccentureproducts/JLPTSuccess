...
**Version:** v1.12.51 (post-round-9 close-out + listening multi-voice render + Hindi audit cycle 1+2)
**Built at:** see `data/version.json` (regenerated by `tools/build_version_json.py`)
**Last updated:** 2026-05-07 (Hindi quality+coverage audit cycle 1+2 closed; HI-01..HI-19 + cycle-2 mechanical pass; JA-41 invariant added)
...
Final code-mix count: ~213 single-English-word residuals in
`llm_curated` content — long-tail polish targets for a future
human-review cycle.

**Provenance honesty principle (R-0):** the `*_provenance` flag
on every Hindi field MUST honestly reflect whether the rubric
(R-1..R-7 in the audit prompt) was actually applied. Mechanical
glossary translation = `llm_curated`. Native-speaker rubric pass
= `native_reviewed`. Cycle 1 enforced this by downgrading 433
auto_generated → llm_curated on cycle-1 rewrites. Cycle 2 phase 3
(deferred) flips llm_curated → native_reviewed entry-by-entry
after rubric is applied per-entry.
