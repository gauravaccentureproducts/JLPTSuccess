...
**`JLPT-N5-Current-Implementation-Spec.md`** - the **authoritative living spec**. Describes what the app actually is today (see the spec's "Version" + "Last updated" header for the current checkpoint; its §25 CI-invariants reference indexes all wired JA-NN rules - 171 at the 2026-05-29 checkpoint, per `tools/check_content_integrity.py`). If you need ground truth for any decision or hand-off, start here.
...
**`jlpt-n5-design-system-zen-modern.md`** - the **live design-system companion** (Zen Modern / Muji). Canonical source of truth for the app's visual + interaction design; checked by `tools/check_design_system.py` (8 D-rules; currently a non-blocking CI visibility check with a known violation backlog) and named as the source of truth by `css/main.css`. The implementation spec's §36 points here for design tokens.

## Historical record

The table below maps every file in this folder to its status. The two **CURRENT** docs (above) are living and may be edited per the rules below; the archived `.docx` snapshots are historical - **do not edit those**.

| File | Status | Date | Notes |
|---|---|---|---|
| `JLPT-N5-Current-Implementation-Spec.md` | **CURRENT** | 2026-05-29 | Source of truth (functional + implementation) |
| `jlpt-n5-design-system-zen-modern.md` | **CURRENT (companion)** | 2026-05-29 | Live design-system spec (Zen Modern); was the v3.1 supplement, renamed 2026-05-29 |
| `JLPT-N5-Functional-Spec-v3.1-supplement.md` | Superseded (renamed) | 2026-04-30 | Renamed to `jlpt-n5-design-system-zen-modern.md` on 2026-05-29; functional content merged into the current spec §§26-38 |
| `JLPT N5 - Consolidated Spec.docx` | Historical (archived) | 2026-05-04 | v3-consolidated form, pre-migration; moved to `../not-required/specs-archive/` on 2026-05-29 |
| `JLPT N5 Grammar Tutor – Functional Spec.docx` | Historical (archived) | 2026-04-30 | v3 (informally amended), the original .docx spec; moved to `../not-required/specs-archive/` on 2026-05-29 |
