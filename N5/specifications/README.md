...
**`JLPT-N5-Current-Implementation-Spec.md`** — the **authoritative living spec**. Describes what the app actually is today (v1.12.30, 2026-05-04). If you need ground truth for any decision or hand-off, start here.
...
3. **Service worker cache version reset.** Was `jlpt-n5-tutor-v138`; now `jlptsuccess-n5-v1.12.30` (mirrors `version.json:version`).
4. **Logo deployed.** The five-bar ladder mark from `assets/logo/` (top-level shared) is now wired into the favicon + PWA icons.
5. **Content scale grew.** Audio MP3 count now 711, locale files at 5, integrity invariants at 47.
