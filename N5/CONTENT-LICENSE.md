...
| `data/vocab.json` (998 entries) | 998 form/reading/gloss/section/pos rows, all with `gloss_hi`; 132 verbs with `transitivity` field (BUG-016 fix, 2026-05-16); 311 nouns with `counter` dict {kanji,reading} (BUG-015 schema normalization); pitch-accent + collocations on high-frequency entries (count: 1041 → 1009 → 999 → 998 — the 2026-05-16 dedup pass for BUG-018 dropped 10 cross-section duplicates with subset glosses; the BUG-017 form-rename for OOS kanji collided with one pre-existing kana entry and merged) | Compiled by project authors from public N5-syllabus references (see §4) |
...
| **JLPT N5 official syllabus** (published by JEES) | The kanji list (106 chars), the vocabulary scope (~800 core entries; we expanded to 998 with related N5 vocabulary (was 1009 before BUG-018 dedup + BUG-017 collision merge on 2026-05-16)), the grammar inventory (~150 patterns; we cover 178 with related forms) | Anything that would constitute the "exam itself" |
