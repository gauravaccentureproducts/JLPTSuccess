...
A_NN_CATEGORIES = [
    ...
    ("A82", "NATIVE-TEACHER REVIEW OF A SHIPPABLE CORPUS — 11 durable defect classes (NTR-001..013)", "Added 2026-05-22 from 13-bug NTR-* close-out. Native-teacher / JLPT-expert review of v1.15.8 (BUG-161..173) opening with verdict 'strong N5 corpus — competitive with Try!/So-matome on substance.' Eleven durable defect classes that surface only after structural CI gates pass: F.44.1 multi-artifact discipline gap (NTR-001 — 99/3036 vocab examples leaked non-whitelist kanji; LLM-curation ran 3x human-baseline rate; closed via 99 kana-substitution rewrites + JA-150). F.44.2 half-finished duplicate cleanup (NTR-002 — n5-045 self-identified duplicate via contrasts block; marked deprecated). F.44.3 gloss-primary inversion vs modern usage (NTR-003 — かれ/かのじょ glossed 'he/she (boyfriend - more advanced)' but boyfriend is modern primary). F.44.4 pronoun missing register caveat (NTR-004 — あなた glossed bare 'you' without restriction note). F.44.5 section/category mislabel (NTR-005 おはし sec 20 vs 19; NTR-006 えいが sec 26 vs 37). F.44.6 etymology-as-mnemonic conflation (NTR-007 — 三 mnemonic claimed -さん shares etymology). F.44.7 defensible-but-deviant pitch-accent (NTR-008 — 3 entries annotated native_review_pending). F.44.8 particle-nuance not flagged in Q&A (NTR-010 — q-0226 は-contrast). F.44.9 field-name overclaim (NTR-011 — `collocations` was particle templates; renamed). F.44.10 prescriptive-vs-colloquial reading split (NTR-012 — 七 しち prescriptive but なな colloquial; reading_rule note). F.44.11 counter applied indiscriminately + sub-typo (NTR-013). New CI invariant JA-150 (vocab-example kanji whitelist). 6 of 13 findings added a CI invariant or annotation; 7 are content edits / terminology. Bug-spec verification: 0 of 13 rejected as stale-snapshot (human-curated review, not autonomous-audit). Procedure-manual F.44. Source: AUDIT-COVERAGE Part 40."),
    ("A83", "DEFERRED-ITEM BATCH CLOSE-OUT — cohort sweep + annotation-only + sample audit (3 of 3 deferred NTR follow-ups closed)", "Added 2026-05-23 from NTR-* follow-up batch. Three close-out shapes generalized for Nx-builders when a native-teacher batch leaves items deferred at first-pass close. Shape 1 (cohort sweep): when one finding implies a class, mechanical regex-pattern sweep over the cohort; N5 instance audit_kanji_mnemonic_etymology_2026_05_22.py scanned all 106 mnemonics for r\"(borrowed everywhere|borrowed from|comes from|same root|honorific|particle|-さん|-さま)\"; 1 hit (八 conflating 蜂 'bee'); softened; 104/106 clean against the N patterns scanned. Shape 2 (annotation-only when verifier == author): when the only verifier of a canonical reference is the same author who produced the audit, annotate to preserve the gap; don't auto-correct (circular authority); N5 instance fix_pitch_accent_nhk_refinement_2026_05_22.py added per-sense NHK-claim metadata + audio_uses_drop + nhk_2016_claim_provenance to 3 of 4 entries; primary drop values UNCHANGED. Shape 3 (deterministic-stratified sample with named dimensions): SHA256-seeded sampling + explicit regex/heuristic patterns + bounded-honesty result; N5 instance audit_llm_curated_vocab_sample_2026_05_22.py sampled 99/914 llm_curated examples (10.8%) across D1 over-formal / D2 unidiomatic / D3 headword-absent / D5 template-cluster; 0 real findings (3 D3 false positives from rendaku/する-verb). Operational rule: batch the three close-outs as a single audit/fix/fix triple + ONE Rule 4/5 propagation commit. No new CI invariant; methodology / annotation propagation. Procedure-manual F.44.15 + F.44.16. Source: AUDIT-COVERAGE Part 41."),
    ("A84", "STALE-SNAPSHOT RE-PASTE TRIAGE — verify-before-file + STALE/REAL/PARTIAL/REJECT buckets + three durable defect classes (M/N/O)", "Added 2026-05-23 from NTR-FU-001/002/003 close-out. The 2026-05-22 NTR review document was re-pasted as 13 'Pending bugs' on 2026-05-23. Verify-before-file pass (F.41.4) found 9 of 13 stale-snapshot (already closed in NTR-001..013 batch within session), 3 broader-scope follow-ups (NTR-FU-001/002/003 → BUG-174/175/176), 1 rejected with rationale (singular pronouns counting people with 人 IS semantically valid). Three durable defect classes: Class M reverse-direction CI gate asymmetry (NTR-FU-001 / JA-151: vocab→whitelist ungated while whitelist→vocab gated by JA-147; 11 entries had drifted). Class N multi-band metric flattened to single-tier (NTR-FU-002: listening pacing_status='in_range' against learner [180,240] hid JEES-strict [220,240] and ideal [200,220] bands; 38/50 below strict, 3/50 below ideal; added per-item pacing_band_strict + pacing_band_ideal; kept pacing_status backward-compat). Class O field-name overclaim broadened beyond bounded close (NTR-FU-003 / JA-152: NTR-011 renamed 12 pronouns collocations→particle_examples but UI reader wasn't updated, silent regression; 983 other entries kept legacy name; corpus-wide rename + UI + locales + min.js cascade fixes both). Operational rule: STALE/REAL/PARTIAL/REJECT triage; file only REAL+PARTIAL; document STALE+REJECT in commit + audit-coverage. Procedure-manual F.44.17 + F.44.18. Source: AUDIT-COVERAGE Part 42."),
    ("A85", "RE-PASTE TRIAGE DISCIPLINE TIGHTENING — verification-script correctness + 4 follow-up sub-classes (B-sub / D-sub / K-sub / new Class P)", "Added 2026-05-23 from NTR-FU-004/005/006/007 close-out. Reviewer's 2026-05-23 re-re-pass against the regenerated review packet (v1.16.2) caught a false-positive STALE classification from Part 42: my verification script used `g.get('grammar')` instead of `g.get('patterns')` for grammar.json, silently iterated empty, and the empty output was misread as STALE-confirmed. Re-verification with correct lookup showed eda9441 DID land deprecation flags BUT cleanup discipline didn't propagate (n5-045 still in core_n5 list + contrasts.note still self-identifying). F.44.19 amendment: STALE classification MUST run actual data-inspection that PRINTS non-empty per-claim output; silent iterations are failed verification. F.44.20 four follow-up sub-classes: Class B sub-class deprecation-lattice-complete-on-entry-but-incomplete-on-catalog (NTR-FU-004 + JA-153 + JA-148/JA-34 extensions). Class D sub-class pronoun-usage-note-added-but-example-cohort-sweep-incomplete (NTR-FU-005). Class K sub-class reflexive-vs-singular-pronoun-counter-distinction (NTR-FU-006). Class P new ID-immutability-vs-section-retag-divergence (NTR-FU-007 + JA-154; horizontal sweep found 1 additional entry beyond reviewer's 2). Procedure-manual F.44.19 + F.44.20. Source: AUDIT-COVERAGE Part 43."),
    ("A86", "NATIVE-SPEAKER-VERIFICATION AUDIT-BLOCK SCHEMA — scaffold-don't-fake; CI-lock; protocol-doc the human work", "Added 2026-05-23. Reviewer asked Claude to verify 3 pitch-accent entries against NHK 2016 + audio recordings. Per F.44.7 + F.44.15 Shape 2 + NATIVE-SPEAKER-RE-VERIFICATION.md, this is human-only work (Claude as same author = circular authority). Declined-with-alternative shipped: (1) audit-block schema on 3 canary entries (みなさん, あなた, きのう) with verifier_pending=true + blank result_schema + per-entry review questions verbatim from task. (2) Queue file PITCH-ACCENT-VERIFICATION-QUEUE-2026-05-23.md/.json — 587 remaining by-reading entries sorted by N5 vocab frequency proxy (section number). (3) Pitch-accent verification protocol section in NATIVE-SPEAKER-RE-VERIFICATION.md — 5 steps (NHK lookup with page citation, audio location in listening passages, audio-vs-dictionary reconciliation defaulting to audio, match_kind promotion, vocab.json cross-update). (4) Grandfather rule documented in n5_pitch_accent_reference.json _meta: 354 pre-existing match_kind=exact entries from kanjium-exact-form lookup accepted as legacy provenance. (5) JA-155 CI invariant: match_kind=exact + audit block must have verifier_pending=false + result_schema populated. Four discipline anchors: no LLM-authored verification of native-intuition claims; scaffold-don't-fake; CI invariant locks the discipline; document the grandfather. Procedure-manual F.44.21 + F.44.22. Source: AUDIT-COVERAGE Part 44."),
    ("A87", "CI-INVARIANT RE-PASTE DISCIPLINE — verify-before-add; resist symbolic-consolidation-invariant temptation", "Added 2026-05-23. Reviewer asked to 'add a CI invariant that prevents future vocab examples from re-introducing the kanji-whitelist regression (BUG-1 / NTR-001 / v1.16.2 close)' plus extend to grammar/reading/listening. Verification per F.44.17 + F.44.19 (verify-before-classify with PRINT non-empty per-claim output) showed the predicate is ALREADY load-bearing: JA-150 (added 2026-05-22 NTR-001 close commit 578b56f) enforces the EXACT task predicate (whitelist ∪ exception) on vocab.json examples. JA-13 enforces a STRICTER predicate (whitelist only) across grammar/questions/reading/listening. JA-28 enforces the union for dokkai-paper context. Live state ran with the union predicate: 0 violations across all 4 corpora. Triaged STALE per F.44.17; no new invariant added. The 'symbolic consolidation invariant' option was considered and rejected per F.44.23: redundant with JA-150 + adds CI surface without detection value + documentation belongs in audit log not CI. Methodology lesson generalizes F.44.17 to CI-invariant re-paste tasks: same verify-before-fix discipline as bug-fix re-paste. Procedure-manual F.44.23 + F.44.24. Source: AUDIT-COVERAGE Part 45."),
    ("A88", "REVIEW-PACKET STALENESS DISCIPLINE — regen-after-data-change locked by JA-156", "Added 2026-05-23. User-caught discipline gap: session shipped v1.16.0 → v1.16.4 across 4 minor version bumps + 18+ commits; author explicitly regenerated the gitignored data/_review_packet/ exactly once (at v1.16.2). User asked 'have you updated review packet? because the review is based on it' — verification showed the packet had happened-to-be-current via a side-effect, but the author had NOT explicitly regenerated it. Failure mode: when a stale packet is shared with an external reviewer, the reviewer surfaces 'bugs' already fixed in the working tree, forcing a STALE/REAL/PARTIAL/REJECT triage cycle in the next session (F.44.17). Codification: JA-156 CI invariant (when data/_review_packet/ exists locally, its version.json.version must equal data/version.json.version; skip-on-absent for remote CI) + F.44.25 procedural discipline (regen-after-data-change rule + pre-share verification). Generalizable pattern (F.44.26): any gitignored generated artifact shared externally needs a freshness CI invariant of this shape. Per F.44.23 verify-before-add: confirmed no existing invariant; 0 grep matches in check_content_integrity.py. Procedure-manual F.44.25 + F.44.26. Source: AUDIT-COVERAGE Part 46."),
    ("A89", "REVIEWER-PROMPT PREFLIGHT STRENGTHENING — 3-block BINDING preflight + JA-161 lock", "Added 2026-05-24. Reviewer v4 pass against v1.16.9 packet cited correct version.json.version (v1.16.9) but quoted 4 Hindi rationale strings from v1.16.8 broken state (the strings just fixed in v1.16.9). Per F.44.19 verify-before-fix against the actual packet on disk: all 4 strings matched the FIXED state. Reviewer was reading recalled / cached content, not the uploaded packet — new false-positive class FP-RECALL-NOT-READ. Three plausible mechanisms (all addressed by the new defense): Project-Knowledge cache effect (fresh version.json on top of stale paper files), conversation-history recall (reviewer participated in prior session against prior version), training-data leakage. Codification: docs/REVIEW-PACKET-PROMPT.md gains 3-block BINDING preflight (version.json echo + content-fingerprint echo with explicit STALE-MARKER strings for the 4 v1.16.8 broken Hindi rationales + read-not-recalled attestation paste). Per-finding amplification: Observed MUST be verbatim copy-paste (no paraphrase / translation-from-memory / recall); explicit Read-not-recalled checkbox per finding; verification-discipline gains 3 new BINDING bullets. JA-161 substring-locks the 12 required defense markers in the canonical prompt. Procedure-manual F.44.27 + F.44.28. Source: AUDIT-COVERAGE Part 50."),
]
...
PHASE0_BLOCKS = [
    ...
    ("P0-native-teacher-review", "Phase-0 native-teacher-review regression block (added 2026-05-22)"),
    ("P0-fix-commit-shape", "Phase-0 Fix Commit shape + TASKS-vs-code coverage-gap block (added 2026-05-22)"),
    ("P0-deferred-NTR-item-closure", "Phase-0 deferred-NTR-item closure regression block (added 2026-05-23)"),
    ("P0-stale-snapshot-repaste-triage", "Phase-0 stale-snapshot re-paste triage regression block (added 2026-05-23)"),
    ("P0-repaste-triage-discipline", "Phase-0 re-paste triage discipline tightening + 4 follow-up sub-classes block (added 2026-05-23)"),
