import*as $ from"./storage.js";import{currentLocale as b}from"./i18n.js";function y(n){const a=b();if(a&&a!=="en"){const l=n[`meanings_${a}`];if(Array.isArray(l)&&l.length)return l}return n.meanings||[]}let f=null;async function _(){return f||(f=await(await fetch("data/kanji.json")).json(),f)}async function C(n,a){await _();const l=f.entries||[],i=a?decodeURIComponent(a):"";if(!i)return h(n,l);const r=l.find(t=>t.glyph===i);if(!r){n.innerHTML=`
    ...
    `;return}return L(n,r,l)}let m="",d="all",g="all",u="lesson";function w(n){return n==null?"":n<=5?"1-5":n<=10?"6-10":n<=15?"11-15":"16+"}function S(n){return n==null?"":n<=30?"1-30":n<=60?"31-60":n<=90?"61-90":"91-106"}function x(n,a,l,i){if(a){const r=n.additional_readings||{};if(![n.glyph||"",...n.on||[],...n.kun||[],...r.on||[],...r.kun||[],...n.meanings||[]].join(" ").toLowerCase().includes(a))return!1}return!(l!=="all"&&w(n.stroke_count)!==l||i!=="all"&&S(n.lesson_order)!==i)}function v(n){switch(u){case"frequency":return n.frequency_rank??999;case"strokes":return n.stroke_count??999;case"glyph":return n.glyph||"";default:return n.lesson_order??999}}function h(n,a){const l=m.trim().toLowerCase(),i=a.filter(o=>x(o,l,d,g)).slice().sort((o,c)=>{const p=v(o),j=v(c);return typeof p=="string"?p.localeCompare(j):p-j}),r=i.map(o=>`
  ...
  `;const k=document.getElementById("kanji-filter-q");k&&k.addEventListener("input",()=>{m=k.value,h(n,a);const o=document.getElementById("kanji-filter-q");if(o){o.focus();const c=o.value;o.setSelectionRange(c.length,c.length)}}),n.querySelectorAll("[data-filter-group]").forEach(o=>{o.addEventListener("click",()=>{const c=o.dataset.filterGroup,p=o.dataset.filterValue;c==="stroke"?d=p:c==="lesson"&&(g=p),h(n,a)})});const s=document.getElementById("kanji-sort");s&&s.addEventListener("change",()=>{u=s.value,h(n,a)})}function L(n,a,l){const i=l.findIndex(s=>s.glyph===a.glyph),r=i>0?l[i-1]:null,t=i<l.length-1?l[i+1]:null,k=$.isKanjiKnown(a.glyph);n.innerHTML=`
      ...
      <div class="kanji-glyph-row pattern-header">
        <div class="kanji-glyph-cluster">
          ...
          <div class="kanji-readings">
            ...
            ${(()=>{const s=y(a);return s.length?`<p><strong>Meaning:</strong> ${s.map(e).join(", ")}</p>`:""})()}
          </div>
        </div>
  ...
  `,document.getElementById("mark-known-kanji")?.addEventListener("change",s=>{$.setKanjiKnown(a.glyph,s.target.checked)})}function e(n){return String(n??"").replace(/[&<>"']/g,a=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[a])}export{C as renderKanji};
