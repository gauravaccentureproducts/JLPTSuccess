import*as a from"./storage.js";import{setLocale as v,currentLocale as h,enabledLocales as b,t}from"./i18n.js";import"./furigana.js";import{exportVocabTSV as f,exportGrammarTSV as $,exportKanjiTSV as E}from"./corpus-export.js";import{navigateTo as k}from"./router.js";const w={en:"English",hi:"\u0939\u093F\u0928\u094D\u0926\u0940"};async function j(r){const s=a.getSettings();r.innerHTML=`
    ...
    <section class="settings-section">
      ...
      <label class="settings-row">
        ...
        <select id="set-locale">
          ${b.map(e=>`<option value="${e}" ${h()===e?"selected":""}>${w[e]||e}</option>`).join("")}
        </select>
      </label>
      <label class="settings-row">
  ...
  `;let m=null;const c=e=>{let n=document.getElementById("settings-saved-toast");n||(n=document.createElement("div"),n.id="settings-saved-toast",n.className="settings-saved-toast",n.setAttribute("role","status"),n.setAttribute("aria-live","polite"),document.body.appendChild(n)),n.textContent=e?`Saved: ${e}`:"Saved",n.classList.add("is-visible"),m&&clearTimeout(m),m=setTimeout(()=>{n.classList.remove("is-visible")},1800)};document.getElementById("set-locale").addEventListener("change",async e=>{await v(e.target.value),document.dispatchEvent(new CustomEvent("locale-changed")),location.reload()}),document.getElementById("set-theme").addEventListener("change",e=>{a.setSettings({theme:e.target.value}),x()}),document.getElementById("set-font").addEventListener("change",e=>{a.setSettings({fontSize:e.target.value}),S()}),document.getElementById("set-test-length").addEventListener("change",e=>{a.setSettings({lastTestLength:parseInt(e.target.value,10)}),c(`Default test length = ${e.target.value} questions`)}),document.getElementById("set-daily-new").addEventListener("change",e=>{a.setSettings({dailyNewLimit:parseInt(e.target.value,10)}),c(`Daily new-card limit = ${e.target.value}`)}),document.getElementById("set-daily-review").addEventListener("change",e=>{a.setSettings({dailyReviewCap:parseInt(e.target.value,10)}),c(`Daily review cap = ${e.target.value}`)}),document.getElementById("set-daily-goal").addEventListener("change",e=>{a.setSettings({dailyGoalReviews:parseInt(e.target.value,10)}),c(`Daily review goal = ${e.target.value}`)}),document.getElementById("set-auto-furigana").addEventListener("change",e=>{a.setSettings({autoFurigana:!!e.target.checked}),c(`Auto-furigana = ${e.target.checked?"on":"off"}`),document.dispatchEvent(new CustomEvent("furigana-rerender"))}),document.getElementById("set-show-recommender").addEventListener("change",e=>{a.setSettings({showRecommender:!!e.target.checked}),c(`Recommended-next = ${e.target.checked?"on":"off"}`)}),document.getElementById("set-srs-gating")?.addEventListener("change",e=>{a.setSettings({srsGatingEnabled:!!e.target.checked}),c(`SRS gating = ${e.target.checked?"on":"off"}`)}),document.getElementById("set-audio-rate").addEventListener("change",e=>{a.setSettings({audioPlaybackRate:parseFloat(e.target.value)}),_(),c(`Audio playback speed = ${e.target.value}\xD7`)}),document.getElementById("set-reduce-motion").addEventListener("change",e=>{const n=e.target.value,i=n==="auto"?null:n==="on";a.setSettings({reduceMotion:i}),L(),c(`Reduce motion = ${n==="auto"?"Follow system":n==="on"?"Always reduce":"Never reduce"}`)});const g=document.getElementById("set-corpus-export-msg");function u(e){g&&(g.textContent=e,g.style.color="var(--c-success)",setTimeout(()=>{g.textContent===e&&(g.textContent="")},4e3))}document.getElementById("set-export-vocab").addEventListener("click",async()=>{const e=await f();u(`Exported ${e} vocab entries to TSV. Check downloads.`)}),document.getElementById("set-export-grammar").addEventListener("click",async()=>{const e=await $();u(`Exported ${e} grammar patterns to TSV. Check downloads.`)}),document.getElementById("set-export-kanji").addEventListener("click",async()=>{const e=await E();u(`Exported ${e} kanji entries to TSV. Check downloads.`)}),document.getElementById("set-privacy-check").addEventListener("click",()=>{const e=document.getElementById("set-privacy-result");try{const n=location.origin,i=performance.getEntriesByType("resource")||[],d=i.filter(o=>{try{return new URL(o.name).origin===n}catch{return!1}}),l=i.filter(o=>{try{return new URL(o.name).origin!==n}catch{return!1}});if(l.length===0)e.textContent=t("settings.privacy_check_ok",{same:d.length}),e.style.color="var(--c-success)";else{const o=Array.from(new Set(l.map(y=>{try{return new URL(y.name).origin}catch{return y.name}}))).slice(0,3).join(", ");e.textContent=t("settings.privacy_check_fail",{cross:l.length,offending:o}),e.style.color="var(--c-error)"}}catch(n){e.textContent=`Verification error: ${n.message}`,e.style.color="var(--c-error)"}}),document.getElementById("set-export").addEventListener("click",()=>{const e=a.exportProgress(),n=new Blob([JSON.stringify(e,null,2)],{type:"application/json"}),i=URL.createObjectURL(n),d=`jlpt-n5-progress-${new Date().toISOString().slice(0,10)}.json`,l=document.createElement("a");l.href=i,l.download=d,document.body.appendChild(l),l.click(),document.body.removeChild(l),URL.revokeObjectURL(i);const o=document.getElementById("set-import-msg");o&&(o.textContent=`Exported to ${d} (check your downloads folder).`,o.style.color="var(--c-success)",setTimeout(()=>{o.textContent.startsWith("Exported")&&(o.textContent="")},4e3))});const p=document.getElementById("set-import-file");document.getElementById("set-import-trigger").addEventListener("click",()=>p.click()),p.addEventListener("change",async e=>{const n=e.target.files?.[0],i=document.getElementById("set-import-msg");if(n)try{const d=await n.text(),l=JSON.parse(d),o=a.importProgress(l);i.textContent=o.message,i.style.color=o.ok?"var(--c-success)":"var(--c-error)",o.ok&&setTimeout(()=>location.reload(),800)}catch(d){i.textContent=`Import failed: ${d.message}`,i.style.color="var(--c-error)"}}),document.getElementById("set-reset").addEventListener("click",()=>{document.getElementById("reset-confirm").hidden=!1,document.getElementById("reset-phrase").focus()}),document.getElementById("reset-cancel-btn").addEventListener("click",()=>{document.getElementById("reset-confirm").hidden=!0,document.getElementById("reset-phrase").value="",document.getElementById("reset-confirm-btn").disabled=!0}),document.getElementById("reset-phrase").addEventListener("input",e=>{document.getElementById("reset-confirm-btn").disabled=e.target.value.trim()!=="RESET"}),document.getElementById("reset-confirm-btn").addEventListener("click",()=>{a.reset(),k("learn"),location.reload()})}function x(){const s=a.getSettings().theme||"system";document.documentElement.setAttribute("data-theme",s)}function S(){const r=a.getSettings();document.documentElement.setAttribute("data-font",r.fontSize||"m")}function _(){const r=a.getSettings().audioPlaybackRate||1;document.querySelectorAll("audio").forEach(s=>{try{s.playbackRate=r}catch{}})}function L(){const r=a.getSettings().reduceMotion;r===!0?document.documentElement.setAttribute("data-reduce-motion","on"):r===!1?document.documentElement.setAttribute("data-reduce-motion","off"):document.documentElement.removeAttribute("data-reduce-motion")}export{_ as applyAudioRate,S as applyFontSize,L as applyReduceMotion,x as applyTheme,j as renderSettings};
