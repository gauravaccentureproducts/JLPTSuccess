import*as o from"./storage.js";import{setLocale as u,currentLocale as p,supportedLocales as h,t}from"./i18n.js";import"./furigana.js";const y={en:"English",hi:"\u0939\u093F\u0928\u094D\u0926\u0940"};async function _(a){const s=o.getSettings();a.innerHTML=`
    ...
    <section class="settings-section">
      ...
      <label class="settings-row">
        ...
        <select id="set-locale">
          ${h.map(e=>`<option value="${e}" ${p()===e?"selected":""}>${y[e]||e}</option>`).join("")}
        </select>
      </label>
      <label class="settings-row">
  ...
  `;let g=null;const d=e=>{let n=document.getElementById("settings-saved-toast");n||(n=document.createElement("div"),n.id="settings-saved-toast",n.className="settings-saved-toast",n.setAttribute("role","status"),n.setAttribute("aria-live","polite"),document.body.appendChild(n)),n.textContent=e?`Saved: ${e}`:"Saved",n.classList.add("is-visible"),g&&clearTimeout(g),g=setTimeout(()=>{n.classList.remove("is-visible")},1800)};document.getElementById("set-locale").addEventListener("change",async e=>{await u(e.target.value),document.dispatchEvent(new CustomEvent("locale-changed")),location.reload()}),document.getElementById("set-theme").addEventListener("change",e=>{o.setSettings({theme:e.target.value}),v()}),document.getElementById("set-font").addEventListener("change",e=>{o.setSettings({fontSize:e.target.value}),b()}),document.getElementById("set-test-length").addEventListener("change",e=>{o.setSettings({lastTestLength:parseInt(e.target.value,10)}),d(`Default test length = ${e.target.value} questions`)}),document.getElementById("set-daily-new").addEventListener("change",e=>{o.setSettings({dailyNewLimit:parseInt(e.target.value,10)}),d(`Daily new-card limit = ${e.target.value}`)}),document.getElementById("set-daily-review").addEventListener("change",e=>{o.setSettings({dailyReviewCap:parseInt(e.target.value,10)}),d(`Daily review cap = ${e.target.value}`)}),document.getElementById("set-daily-goal").addEventListener("change",e=>{o.setSettings({dailyGoalReviews:parseInt(e.target.value,10)}),d(`Daily review goal = ${e.target.value}`)}),document.getElementById("set-auto-furigana").addEventListener("change",e=>{o.setSettings({autoFurigana:!!e.target.checked}),d(`Auto-furigana = ${e.target.checked?"on":"off"}`),document.dispatchEvent(new CustomEvent("furigana-rerender"))}),document.getElementById("set-show-recommender").addEventListener("change",e=>{o.setSettings({showRecommender:!!e.target.checked}),d(`Recommended-next = ${e.target.checked?"on":"off"}`)}),document.getElementById("set-audio-rate").addEventListener("change",e=>{o.setSettings({audioPlaybackRate:parseFloat(e.target.value)}),f(),d(`Audio playback speed = ${e.target.value}\xD7`)}),document.getElementById("set-reduce-motion").addEventListener("change",e=>{const n=e.target.value,i=n==="auto"?null:n==="on";o.setSettings({reduceMotion:i}),$(),d(`Reduce motion = ${n==="auto"?"Follow system":n==="on"?"Always reduce":"Never reduce"}`)}),document.getElementById("set-export").addEventListener("click",()=>{const e=o.exportProgress(),n=new Blob([JSON.stringify(e,null,2)],{type:"application/json"}),i=URL.createObjectURL(n),c=`jlpt-n5-progress-${new Date().toISOString().slice(0,10)}.json`,r=document.createElement("a");r.href=i,r.download=c,document.body.appendChild(r),r.click(),document.body.removeChild(r),URL.revokeObjectURL(i);const l=document.getElementById("set-import-msg");l&&(l.textContent=`Exported to ${c} (check your downloads folder).`,l.style.color="var(--c-success)",setTimeout(()=>{l.textContent.startsWith("Exported")&&(l.textContent="")},4e3))});const m=document.getElementById("set-import-file");document.getElementById("set-import-trigger").addEventListener("click",()=>m.click()),m.addEventListener("change",async e=>{const n=e.target.files?.[0],i=document.getElementById("set-import-msg");if(n)try{const c=await n.text(),r=JSON.parse(c),l=o.importProgress(r);i.textContent=l.message,i.style.color=l.ok?"var(--c-success)":"var(--c-error)",l.ok&&setTimeout(()=>location.reload(),800)}catch(c){i.textContent=`Import failed: ${c.message}`,i.style.color="var(--c-error)"}}),document.getElementById("set-reset").addEventListener("click",()=>{document.getElementById("reset-confirm").hidden=!1,document.getElementById("reset-phrase").focus()}),document.getElementById("reset-cancel-btn").addEventListener("click",()=>{document.getElementById("reset-confirm").hidden=!0,document.getElementById("reset-phrase").value="",document.getElementById("reset-confirm-btn").disabled=!0}),document.getElementById("reset-phrase").addEventListener("input",e=>{document.getElementById("reset-confirm-btn").disabled=e.target.value.trim()!=="RESET"}),document.getElementById("reset-confirm-btn").addEventListener("click",()=>{o.reset(),location.hash="#/learn",location.reload()})}function v(){const s=o.getSettings().theme||"system";document.documentElement.setAttribute("data-theme",s)}function b(){const a=o.getSettings();document.documentElement.setAttribute("data-font",a.fontSize||"m")}function f(){const a=o.getSettings().audioPlaybackRate||1;document.querySelectorAll("audio").forEach(s=>{try{s.playbackRate=a}catch{}})}function $(){const a=o.getSettings().reduceMotion;a===!0?document.documentElement.setAttribute("data-reduce-motion","on"):a===!1?document.documentElement.setAttribute("data-reduce-motion","off"):document.documentElement.removeAttribute("data-reduce-motion")}export{f as applyAudioRate,b as applyFontSize,$ as applyReduceMotion,v as applyTheme,_ as renderSettings};
