import*as a from"./storage.js";import{setLocale as u,currentLocale as p,supportedLocales as h,t}from"./i18n.js";import"./furigana.js";const y={en:"English",hi:"\u0939\u093F\u0928\u094D\u0926\u0940"};async function S(o){const s=a.getSettings();o.innerHTML=`
  ...
  `;let g=null;const i=e=>{let n=document.getElementById("settings-saved-toast");n||(n=document.createElement("div"),n.id="settings-saved-toast",n.className="settings-saved-toast",n.setAttribute("role","status"),n.setAttribute("aria-live","polite"),document.body.appendChild(n)),n.textContent=e?`Saved: ${e}`:"Saved",n.classList.add("is-visible"),g&&clearTimeout(g),g=setTimeout(()=>{n.classList.remove("is-visible")},1800)};document.getElementById("set-locale").addEventListener("change",async e=>{await u(e.target.value),document.dispatchEvent(new CustomEvent("locale-changed")),location.reload()}),document.getElementById("set-theme").addEventListener("change",e=>{a.setSettings({theme:e.target.value}),v()}),document.getElementById("set-font").addEventListener("change",e=>{a.setSettings({fontSize:e.target.value}),b()}),document.getElementById("set-test-length").addEventListener("change",e=>{a.setSettings({lastTestLength:parseInt(e.target.value,10)}),i(`Default test length = ${e.target.value} questions`)}),document.getElementById("set-daily-new").addEventListener("change",e=>{a.setSettings({dailyNewLimit:parseInt(e.target.value,10)}),i(`Daily new-card limit = ${e.target.value}`)}),document.getElementById("set-daily-review").addEventListener("change",e=>{a.setSettings({dailyReviewCap:parseInt(e.target.value,10)}),i(`Daily review cap = ${e.target.value}`)}),document.getElementById("set-daily-goal").addEventListener("change",e=>{a.setSettings({dailyGoalReviews:parseInt(e.target.value,10)}),i(`Daily review goal = ${e.target.value}`)}),document.getElementById("set-auto-furigana").addEventListener("change",e=>{a.setSettings({autoFurigana:!!e.target.checked}),i(`Auto-furigana = ${e.target.checked?"on":"off"}`),document.dispatchEvent(new CustomEvent("furigana-rerender"))}),document.getElementById("set-show-recommender").addEventListener("change",e=>{a.setSettings({showRecommender:!!e.target.checked}),i(`Recommended-next = ${e.target.checked?"on":"off"}`)}),document.getElementById("set-srs-gating")?.addEventListener("change",e=>{a.setSettings({srsGatingEnabled:!!e.target.checked}),i(`SRS gating = ${e.target.checked?"on":"off"}`)}),document.getElementById("set-audio-rate").addEventListener("change",e=>{a.setSettings({audioPlaybackRate:parseFloat(e.target.value)}),f(),i(`Audio playback speed = ${e.target.value}\xD7`)}),document.getElementById("set-reduce-motion").addEventListener("change",e=>{const n=e.target.value,l=n==="auto"?null:n==="on";a.setSettings({reduceMotion:l}),$(),i(`Reduce motion = ${n==="auto"?"Follow system":n==="on"?"Always reduce":"Never reduce"}`)}),document.getElementById("set-export").addEventListener("click",()=>{const e=a.exportProgress(),n=new Blob([JSON.stringify(e,null,2)],{type:"application/json"}),l=URL.createObjectURL(n),c=`jlpt-n5-progress-${new Date().toISOString().slice(0,10)}.json`,r=document.createElement("a");r.href=l,r.download=c,document.body.appendChild(r),r.click(),document.body.removeChild(r),URL.revokeObjectURL(l);const d=document.getElementById("set-import-msg");d&&(d.textContent=`Exported to ${c} (check your downloads folder).`,d.style.color="var(--c-success)",setTimeout(()=>{d.textContent.startsWith("Exported")&&(d.textContent="")},4e3))});const m=document.getElementById("set-import-file");document.getElementById("set-import-trigger").addEventListener("click",()=>m.click()),m.addEventListener("change",async e=>{const n=e.target.files?.[0],l=document.getElementById("set-import-msg");if(n)try{const c=await n.text(),r=JSON.parse(c),d=a.importProgress(r);l.textContent=d.message,l.style.color=d.ok?"var(--c-success)":"var(--c-error)",d.ok&&setTimeout(()=>location.reload(),800)}catch(c){l.textContent=`Import failed: ${c.message}`,l.style.color="var(--c-error)"}}),document.getElementById("set-reset").addEventListener("click",()=>{document.getElementById("reset-confirm").hidden=!1,document.getElementById("reset-phrase").focus()}),document.getElementById("reset-cancel-btn").addEventListener("click",()=>{document.getElementById("reset-confirm").hidden=!0,document.getElementById("reset-phrase").value="",document.getElementById("reset-confirm-btn").disabled=!0}),document.getElementById("reset-phrase").addEventListener("input",e=>{document.getElementById("reset-confirm-btn").disabled=e.target.value.trim()!=="RESET"}),document.getElementById("reset-confirm-btn").addEventListener("click",()=>{a.reset(),location.hash="#/learn",location.reload()})}function v(){const s=a.getSettings().theme||"system";document.documentElement.setAttribute("data-theme",s)}function b(){const o=a.getSettings();document.documentElement.setAttribute("data-font",o.fontSize||"m")}function f(){const o=a.getSettings().audioPlaybackRate||1;document.querySelectorAll("audio").forEach(s=>{try{s.playbackRate=o}catch{}})}function $(){const o=a.getSettings().reduceMotion;o===!0?document.documentElement.setAttribute("data-reduce-motion","on"):o===!1?document.documentElement.setAttribute("data-reduce-motion","off"):document.documentElement.removeAttribute("data-reduce-motion")}export{f as applyAudioRate,b as applyFontSize,$ as applyReduceMotion,v as applyTheme,S as renderSettings};
