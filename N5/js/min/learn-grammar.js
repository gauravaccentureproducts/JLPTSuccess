import{renderJa as b}from"./furigana.js";import*as B from"./storage.js";import{esc as a,wireExpandCollapseControls as ee}from"./learn.js";import{currentLocale as A,t as i}from"./i18n.js";import{assetUrl as te}from"./router.js";let v=null,C=null;async function ae(){return v||C||(C=fetch("data/audio_manifest.json").then(n=>n.ok?n.json():null).then(n=>(n&&Array.isArray(n.items)?v=new Set(n.items.map(t=>t.path)):v=new Set,v)).catch(()=>(v=new Set,v)),C)}function ne(n){return v?v.has(n):!1}function se(n){const t=A();if(t&&t!=="en"){const s=n[`explanation_${t}`];if(typeof s=="string"&&s.trim())return s}return n.explanation_en||""}function re(n){const t=A();if(t&&t!=="en"){const s=n[`meaning_${t}`];if(typeof s=="string"&&s.trim())return s}return n.meaning_en||""}function oe(n){const t=A();if(t&&t!=="en"&&n.l1_notes&&typeof n.l1_notes=="object"){const s=n.l1_notes[t];if(typeof s=="string"&&s.trim())return s}return null}const S=[["Sentence Basics",["Copula and Basic Sentence Structure","Particles","Demonstratives","Question Words"]],["Verbs",["Verbs - Tense and Politeness (\u307E\u3059-form)","Verbs - Plain (Dictionary) Form and Negation","Te-form and Related Patterns","Existence and Possession","Desiderative and Volitional","Giving and Receiving (basic)","Additional Upper N5 / Borderline Patterns - Permission and Obligation","Additional Upper N5 / Borderline Patterns - Experience and Advice","Additional Upper N5 / Borderline Patterns - Compound and Listed Actions","Additional Upper N5 / Borderline Patterns - Excess","Additional Upper N5 / Borderline Patterns - Intention","Additional Upper N5 / Borderline Patterns - Way of Doing","Additional Upper N5 / Borderline Patterns - Prohibitive (Casual)"]],["Adjectives and Comparison",["Adjectives","Comparison and Preference"]],["Time, Counters, Connectives",["Counters and Quantity","Time Expressions","Conjunctions and Connectives","Asking and Stating with \u304B\u3089 / \u306E\u3067 (basic causation)","Existence-of-Plans and Frequency"]],["Set Phrases and Discourse",["Nominalization and Modification","Common Set Patterns","Functional Expressions (Non-Grammar, Common Usage)","Other Core Patterns","Honorific / Polite Vocabulary at N5 (functional)","Additional Upper N5 / Borderline Patterns - Explanation and Emphasis","Additional Upper N5 / Borderline Patterns - Quotation (Casual)","Additional Upper N5 / Borderline Patterns - Sentence-Final Exclamation"]]],T={"n5-135":"Verbs","n5-144":"Verbs","n5-153":"Verbs","n5-154":"Verbs","n5-162":"Verbs","n5-163":"Verbs"};function U(n){if(typeof n=="object"&&n&&n.id in T)return T[n.id];const t=typeof n=="string"?n:n?.category||"";for(const[s,u]of S)if(u.includes(t))return s;return"Set Phrases and Discourse"}function ie(n){const t=new Map;for(const[u]of S)t.set(u,[]);for(const u of n){const g=U(u);t.has(g)&&t.get(g).push(u)}const s=[];for(const[,u]of t)u.sort((g,y)=>(g.patternOrder??0)-(y.patternOrder??0)),s.push(...u);return s}let P="";function le(n,t){return t?[n.pattern,n.meaning_en,n.meaning_ja||"",n.notes||"",(n.examples||[]).map(u=>u.ja).join(" ")].join(" ").toLowerCase().includes(t):!0}function q(n,t){const s=new Map;for(const[r]of S)s.set(r,[]);const u=P.trim().toLowerCase(),g=t.patterns.filter(r=>le(r,u));for(const r of g){const l=U(r);s.get(l).push(r)}const y=r=>r.toLowerCase().replace(/[^a-z0-9]+/g,"-").replace(/^-|-$/g,"");let c=`
  ...
  `:"",j=g.map((e,o)=>{const f=!e.ja||e.ja.includes("(see ")?null:`audio/grammar/${t.id}.${o}.mp3`,$=f&&ne(f)?f:null;return`
    <li>
  ...
  `}).join(""),M=y.map(e=>{if(e.kind==="register_variant"){const o=e.label_a?`<span class="variant-label">${a(e.label_a)}</span>`:"",p=e.label_b?`<span class="variant-label">${a(e.label_b)}</span>`:"",f=e.form_a??e.wrong??"",$=e.form_b??e.right??"";return`
        <li class="variant-pair">
          <div class="variant-row">${o}<span class="variant-form">${b(f)}</span></div>
    ...
    `}).join(""),E=Array.isArray(t.wrong_corrected_pair)?t.wrong_corrected_pair:[],R=e=>{if(!e)return"";const o=`grammar_detail.cat_${e}`,p=i(o)!==o?i(o):e;return`<span class="error-category-badge cat-${a(e)}">${a(p)}</span>`},D=i("grammar_detail.wcp_wrong"),F=i("grammar_detail.wcp_correct"),H=E.map(e=>`
    <li>
  ...
  `).join(""),L=t.politeness_ladder&&typeof t.politeness_ladder=="object"?t.politeness_ladder:null,O=L?`
    <section class="politeness-ladder">
      <h3 class="section-title">${a(i("grammar_detail.ladder_section"))}</h3>
      <table class="ladder-table">
        <tbody>
          ${["casual","polite","humble","respectful"].map(e=>{const o=L[e];if(!o)return"";const p=i(`grammar_detail.ladder_${e}`);return`
  ...
  `:"",z="",N=Array.isArray(t.public_domain_refs)?t.public_domain_refs:[],G=N.length?`
    <section class="pd-refs">
      ...
      <ul class="pd-refs-list">
        ${N.map(e=>{const o=a(e.source_type||"?"),p=a(e.work_title||""),f=a(e.author||""),$=e.author_death_year?` (died ${e.author_death_year})`:"",k=e.pd_status?`<span class="pd-status muted small">${a(e.pd_status)}</span>`:"",J=e.context?`<p class="pd-context muted small">${a(e.context)}</p>`:"",K=e.pattern_role?`<p class="pd-role muted small"><em>${a(e.pattern_role)}</em></p>`:"",X=e.url?` <a href="${a(e.url)}" target="_blank" rel="noopener" class="pd-link">\u2197 source</a>`:"",Y=e.quote_ja?`<blockquote class="pd-quote-ja" lang="ja">${a(e.quote_ja)}</blockquote>`:"",Z=e.quote_translation_en?`<p class="pd-quote-en muted small">${a(e.quote_translation_en)}</p>`:"";return`
  ...
  `:"",W=r?'<span class="status-badge mastered">\u2605 Mastered</span>':l?'<span class="status-badge weak">Needs practice</span>':"",V=e=>{const o=Array.isArray(s)?s.find(p=>p&&p.id===e):null;return a(o&&o.pattern?o.pattern:e)},Q=`
    <article class="pattern-detail">
      ${w}
      <a class="back-link no-print" href="#/learn/grammar">\u2190 ${a(i("grammar_detail.back_to_list"))}</a>
      ...
      <div class="pattern-header">
        ...
        <label class="known-toggle no-print" title="Manually mark as known. Cleared on the next miss in Test or Drill.">
          ...
          <span>${a(i("grammar_detail.mark_as_known"))}</span>
          ${W}
        </label>
        <button type="button" id="pattern-print-btn" class="btn-secondary no-print pattern-print-btn"
          ...
          \u{1F5A8} ${a(i("grammar_detail.print_pdf"))}
        </button>
      </div>
      ...
      <section>
        <h3 class="section-title">${a(i("grammar_detail.explanation"))}</h3>
        <p>${a(se(t))}</p>
      </section>

      ${(()=>{const e=t.essay;if(!e||typeof e!="object")return"";const o=e.provenance==="needs_native_review",p=(f,$,k)=>!$&&!k?"":$?`<p><strong>${a(f)}:</strong> ${a($)}</p>`:`<p><strong>${a(f)}:</strong> <span class="muted small">${a(k)}</span></p>`;return`
          <section class="pattern-essay">
            <h3 class="section-title">${a(i("grammar_detail.deep_dive"))} ${o?'<span class="essay-stub-badge muted small">stub</span>':""}</h3>
            ${p(i("grammar_detail.deep_dive_at_a_glance"),e.intro)}
            ${p(i("grammar_detail.deep_dive_why"),e.why_it_matters,o?"Pending native author.":"")}
            ${p(i("grammar_detail.deep_dive_pitfalls"),e.common_pitfalls)}
            ${p(i("grammar_detail.deep_dive_contrasts"),e.contrasts)}
            ${p(i("grammar_detail.deep_dive_practice"),e.closing_practice_tip,o?"Pending native author.":"")}
      ...
      ${(()=>{const e=oe(t);return e?`
          <section class="l1-note">
            <h3 class="section-title">${a(i("grammar_detail.l1_note"))}</h3>
            <p>${a(e)}</p>
          </section>
        `:""})()}

      <section>
        <h3 class="section-title">${a(i("grammar_detail.examples"))} (${g.length})</h3>
        <ul class="example-list">${j}</ul>
      </section>

      ${y.length?`
        <section>
          <h3 class="section-title">${a(i("grammar_detail.common_mistakes"))}</h3>
      ...
      ${E.length?`
        <section class="wrong-corrected-pair">
          <h3 class="section-title">${a(i("grammar_detail.wcp_section"))} (${E.length})</h3>
      ...
      ${t.notes?`<section><h3 class="section-title">${a(i("grammar_detail.notes"))}</h3><p>${a(t.notes)}</p></section>`:""}
  ...
  `;n.innerHTML=Q,document.getElementById("mark-known")?.addEventListener("change",e=>{B.setManuallyKnown(t.id,e.target.checked),pe(n,t,s)}),document.getElementById("pattern-print-btn")?.addEventListener("click",()=>{window.print()})}export{ie as buildOrderedPatternList,pe as renderGrammarPatternDetail,q as renderGrammarTOC};
