import{renderJa as $}from"./furigana.js";import*as V from"./storage.js";import{esc as a,wireExpandCollapseControls as ee}from"./learn.js";import{currentLocale as A,t as l}from"./i18n.js";import{assetUrl as te}from"./router.js";let v=null,E=null;async function ae(){return v||E||(E=fetch("data/audio_manifest.json").then(n=>n.ok?n.json():null).then(n=>(n&&Array.isArray(n.items)?v=new Set(n.items.map(t=>t.path)):v=new Set,v)).catch(()=>(v=new Set,v)),E)}function ne(n){return v?v.has(n):!1}function se(n){const t=A();if(t&&t!=="en"){const s=n[`explanation_${t}`];if(typeof s=="string"&&s.trim())return s}return n.explanation_en||""}function re(n){const t=A();if(t&&t!=="en"){const s=n[`meaning_${t}`];if(typeof s=="string"&&s.trim())return s}return n.meaning_en||""}function oe(n){const t=A();if(t&&t!=="en"&&n.l1_notes&&typeof n.l1_notes=="object"){const s=n.l1_notes[t];if(typeof s=="string"&&s.trim())return s}return null}const S=[["Sentence Basics",["Copula and Basic Sentence Structure","Particles","Demonstratives","Question Words"]],["Verbs",["Verbs - Tense and Politeness (\u307E\u3059-form)","Verbs - Plain (Dictionary) Form and Negation","Te-form and Related Patterns","Existence and Possession","Desiderative and Volitional","Giving and Receiving (basic)","Additional Upper N5 / Borderline Patterns - Permission and Obligation","Additional Upper N5 / Borderline Patterns - Experience and Advice","Additional Upper N5 / Borderline Patterns - Compound and Listed Actions","Additional Upper N5 / Borderline Patterns - Excess","Additional Upper N5 / Borderline Patterns - Intention","Additional Upper N5 / Borderline Patterns - Way of Doing","Additional Upper N5 / Borderline Patterns - Prohibitive (Casual)"]],["Adjectives and Comparison",["Adjectives","Comparison and Preference"]],["Time, Counters, Connectives",["Counters and Quantity","Time Expressions","Conjunctions and Connectives","Asking and Stating with \u304B\u3089 / \u306E\u3067 (basic causation)","Existence-of-Plans and Frequency"]],["Set Phrases and Discourse",["Nominalization and Modification","Common Set Patterns","Functional Expressions (Non-Grammar, Common Usage)","Other Core Patterns","Honorific / Polite Vocabulary at N5 (functional)","Additional Upper N5 / Borderline Patterns - Explanation and Emphasis","Additional Upper N5 / Borderline Patterns - Quotation (Casual)","Additional Upper N5 / Borderline Patterns - Sentence-Final Exclamation"]]],B={"n5-135":"Verbs","n5-144":"Verbs","n5-153":"Verbs","n5-154":"Verbs","n5-162":"Verbs","n5-163":"Verbs"};function U(n){if(typeof n=="object"&&n&&n.id in B)return B[n.id];const t=typeof n=="string"?n:n?.category||"";for(const[s,d]of S)if(d.includes(t))return s;return"Set Phrases and Discourse"}function ie(n){const t=new Map;for(const[d]of S)t.set(d,[]);for(const d of n){const h=U(d);t.has(h)&&t.get(h).push(d)}const s=[];for(const[,d]of t)d.sort((h,y)=>(h.patternOrder??0)-(y.patternOrder??0)),s.push(...d);return s}let C="";function le(n,t){return t?[n.pattern,n.meaning_en,n.meaning_ja||"",n.notes||"",(n.examples||[]).map(d=>d.ja).join(" ")].join(" ").toLowerCase().includes(t):!0}function q(n,t){const s=new Map;for(const[r]of S)s.set(r,[]);const d=C.trim().toLowerCase(),h=t.patterns.filter(r=>le(r,d));for(const r of h){const o=U(r);s.get(o).push(r)}const y=r=>r.toLowerCase().replace(/[^a-z0-9]+/g,"-").replace(/^-|-$/g,"");let u=`
  ...
  `;for(const[r,o]of s){if(o.length===0)continue;o.sort((c,g)=>(c.patternOrder??0)-(g.patternOrder??0));const m=!!d;u+=`<details class="toc-category" id="cat-${y(r)}"${m?" open":""}>`,u+=`<summary><h3>${a(r)} <span class="cat-count muted small">(${o.length})</span></h3></summary>`,u+='<div class="grammar-grid">';for(const c of o){const g=(()=>{const w=(c.examples||[]).filter(j=>j&&j.ja);return w[0]?w[0].ja:""})(),x=(()=>{const w=A&&A();if(w&&w!=="en"){const j=c[`meaning_${w}`];if(typeof j=="string"&&j.trim())return j}return c.meaning_en||""})();u+=`
      ...
      `}u+="</div></details>"}h.length===0?u+='<div class="placeholder"><p>No patterns match the current filter.</p></div>':t.patterns.length===1&&(u+='<div class="placeholder" style="margin-top:24px"><p>Scaffold currently has 1 example pattern. Add more to <code>data/grammar.json</code> as you author content.</p></div>'),u+=`
      ...
      <tbody>
        ${t.map((r,o)=>`
          <tr>
            ...
            ${o===0?`<td class="pattern-usage-form" rowspan="${t.length}" lang="ja">${$(n.pattern)}</td>`:""}
          </tr>
        `).join("")}
      </tbody>
    </table>
  ...
  `:"",j=h.map((e,i)=>{const f=!e.ja||e.ja.includes("(see ")?null:`audio/grammar/${t.id}.${i}.mp3`,b=f&&ne(f)?f:null;return`
    <li>
  ...
  `}).join(""),M=y.map(e=>{if(e.kind==="register_variant"){const i=e.label_a?`<span class="variant-label">${a(e.label_a)}</span>`:"",p=e.label_b?`<span class="variant-label">${a(e.label_b)}</span>`:"",f=e.form_a??e.wrong??"",b=e.form_b??e.right??"";return`
        <li class="variant-pair">
          <div class="variant-row">${i}<span class="variant-form">${$(f)}</span></div>
    ...
    `}).join(""),P=Array.isArray(t.wrong_corrected_pair)?t.wrong_corrected_pair:[],R=e=>{if(!e)return"";const i=`grammar_detail.cat_${e}`,p=l(i)!==i?l(i):e;return`<span class="error-category-badge cat-${a(e)}">${a(p)}</span>`},D=l("grammar_detail.wcp_wrong"),O=l("grammar_detail.wcp_correct"),F=P.map(e=>`
    <li>
  ...
  `).join(""),N=t.politeness_ladder&&typeof t.politeness_ladder=="object"?t.politeness_ladder:null,H=N?`
    <section class="politeness-ladder">
      ...
      <table class="ladder-table">
        <tbody>
          ${["casual","polite","humble","respectful"].map(e=>{const i=N[e];if(!i)return"";const p=l(`grammar_detail.ladder_${e}`);return`
  ...
  `:"",G=r?'<span class="status-badge mastered">\u2605 Mastered</span>':o?'<span class="status-badge weak">Needs practice</span>':"",T=e=>{const i=Array.isArray(s)?s.find(p=>p&&p.id===e):null;return a(i&&i.pattern?i.pattern:e)},Q=`
    <article class="pattern-detail">
      ...
      ${(()=>{const e=t.essay;if(!e||typeof e!="object")return"";const i=e.provenance==="needs_native_review",p=(f,b,k)=>!b&&!k?"":b?`<p><strong>${a(f)}:</strong> ${a(b)}</p>`:`<p><strong>${a(f)}:</strong> <span class="muted small">${a(k)}</span></p>`;return`
      ...
      ${P.length?`
        <section class="wrong-corrected-pair">
          <h3 class="section-title">${a(l("grammar_detail.wcp_section"))} (${P.length})</h3>
