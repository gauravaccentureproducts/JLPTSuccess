import{renderJa as _}from"./furigana.js";import*as V from"./storage.js";import{esc as a,wireExpandCollapseControls as X}from"./learn.js";import{currentLocale as A,t as o}from"./i18n.js";import{assetUrl as Y}from"./router.js";let v=null,P=null;async function Z(){return v||P||(P=fetch("data/audio_manifest.json").then(n=>n.ok?n.json():null).then(n=>(n&&Array.isArray(n.items)?v=new Set(n.items.map(e=>e.path)):v=new Set,v)).catch(()=>(v=new Set,v)),P)}function ee(n){return v?v.has(n):!1}function te(n){const e=A();if(e&&e!=="en"){const s=n[`explanation_${e}`];if(typeof s=="string"&&s.trim())return s}return n.explanation_en||""}function ae(n){const e=A();if(e&&e!=="en"){const s=n[`meaning_${e}`];if(typeof s=="string"&&s.trim())return s}return n.meaning_en||""}function ne(n){const e=A();if(e&&e!=="en"&&n.l1_notes&&typeof n.l1_notes=="object"){const s=n.l1_notes[e];if(typeof s=="string"&&s.trim())return s}return null}const S=[["Sentence Basics",["Copula and Basic Sentence Structure","Particles","Demonstratives","Question Words"]],["Verbs",["Verbs - Tense and Politeness (\u307E\u3059-form)","Verbs - Plain (Dictionary) Form and Negation","Te-form and Related Patterns","Existence and Possession","Desiderative and Volitional","Giving and Receiving (basic)","Additional Upper N5 / Borderline Patterns - Permission and Obligation","Additional Upper N5 / Borderline Patterns - Experience and Advice","Additional Upper N5 / Borderline Patterns - Compound and Listed Actions","Additional Upper N5 / Borderline Patterns - Excess","Additional Upper N5 / Borderline Patterns - Intention","Additional Upper N5 / Borderline Patterns - Way of Doing","Additional Upper N5 / Borderline Patterns - Prohibitive (Casual)"]],["Adjectives and Comparison",["Adjectives","Comparison and Preference"]],["Time, Counters, Connectives",["Counters and Quantity","Time Expressions","Conjunctions and Connectives","Asking and Stating with \u304B\u3089 / \u306E\u3067 (basic causation)","Existence-of-Plans and Frequency"]],["Set Phrases and Discourse",["Nominalization and Modification","Common Set Patterns","Functional Expressions (Non-Grammar, Common Usage)","Other Core Patterns","Honorific / Polite Vocabulary at N5 (functional)","Additional Upper N5 / Borderline Patterns - Explanation and Emphasis","Additional Upper N5 / Borderline Patterns - Quotation (Casual)","Additional Upper N5 / Borderline Patterns - Sentence-Final Exclamation"]]],B={"n5-135":"Verbs","n5-144":"Verbs","n5-153":"Verbs","n5-154":"Verbs","n5-162":"Verbs","n5-163":"Verbs"};function T(n){if(typeof n=="object"&&n&&n.id in B)return B[n.id];const e=typeof n=="string"?n:n?.category||"";for(const[s,p]of S)if(p.includes(e))return s;return"Set Phrases and Discourse"}function se(n){const e=new Map;for(const[p]of S)e.set(p,[]);for(const p of n){const g=T(p);e.has(g)&&e.get(g).push(p)}const s=[];for(const[,p]of e)p.sort((g,y)=>(g.patternOrder??0)-(y.patternOrder??0)),s.push(...p);return s}let k="";function re(n,e){return e?[n.pattern,n.meaning_en,n.meaning_ja||"",n.notes||"",(n.examples||[]).map(p=>p.ja).join(" ")].join(" ").toLowerCase().includes(e):!0}function U(n,e){const s=new Map;for(const[r]of S)s.set(r,[]);const p=k.trim().toLowerCase(),g=e.patterns.filter(r=>re(r,p));for(const r of g){const i=T(r);s.get(i).push(r)}const y=r=>r.toLowerCase().replace(/[^a-z0-9]+/g,"-").replace(/^-|-$/g,"");let l=`
  ...
  `,g=e.length?`
    <table class="pattern-usage-table" aria-label="Attach points for ${a(n.pattern)}">
      <tbody>
        ${e.map((l,$)=>`
          <tr>
            <td class="pattern-usage-pos">${a(oe(l))}</td>
            ${$===0?`<td class="pattern-usage-form" rowspan="${e.length}" lang="ja">${_(n.pattern)}</td>`:""}
          </tr>
        `).join("")}
      </tbody>
    </table>
  ...
  `:"";return`<section class="pattern-usage">${p}${g}${y}</section>`}async function le(n,e,s){await Z();const p=e.form_rules?.conjugations??[],g=e.examples??[],y=e.common_mistakes??[],l=V.getPatternEntry(e.id),$=!!l?.isManuallyKnown,r=!!l?.isMastered,i=!!l?.isWeak&&!r,u=Array.isArray(s)?se(s):[],c=u.findIndex(t=>t.id===e.id),h=c>0?u[c-1]:null,x=c>=0&&c<u.length-1?u[c+1]:null,w=h||x?`
  ...
  `:"",j=g.map((t,d)=>{const f=!t.ja||t.ja.includes("(see ")?null:`audio/grammar/${e.id}.${d}.mp3`,b=f&&ee(f)?f:null;return`
    <li>
      <span class="form-tag">${a(t.form||"")}</span>
      ${_(t.ja,t.furigana)}
      ${t.translation_en?`<span class="translation">${a(t.translation_en)}</span>`:""}
      ${b?`<audio class="example-audio" controls preload="none" src="${a(Y(b))}"></audio>`:""}
    </li>
  `}).join(""),M=y.map(t=>{if(t.kind==="register_variant"){const d=t.label_a?`<span class="variant-label">${a(t.label_a)}</span>`:"",m=t.label_b?`<span class="variant-label">${a(t.label_b)}</span>`:"",f=t.form_a??t.wrong??"",b=t.form_b??t.right??"";return`
  ...
  `:"",G=r?'<span class="status-badge mastered">\u2605 Mastered</span>':i?'<span class="status-badge weak">Needs practice</span>':"",W=`
    <article class="pattern-detail">
      ...
      ${(()=>{const t=e.essay;if(!t||typeof t!="object")return"";const d=t.provenance==="needs_native_review",m=(f,b,C)=>!b&&!C?"":b?`<p><strong>${a(f)}:</strong> ${a(b)}</p>`:`<p><strong>${a(f)}:</strong> <span class="muted small">${a(C)}</span></p>`;return`
  ...
  `;n.innerHTML=W,document.getElementById("mark-known")?.addEventListener("change",t=>{V.setManuallyKnown(e.id,t.target.checked),le(n,e,s)}),document.getElementById("pattern-print-btn")?.addEventListener("click",()=>{window.print()})}export{se as buildOrderedPatternList,le as renderGrammarPatternDetail,U as renderGrammarTOC};
