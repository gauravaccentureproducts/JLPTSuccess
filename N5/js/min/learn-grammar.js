import{renderJa as h}from"./furigana.js";import*as V from"./storage.js";import{esc as a,wireExpandCollapseControls as Z}from"./learn.js";import{currentLocale as A,t as o}from"./i18n.js";import{assetUrl as ee}from"./router.js";let v=null,C=null;async function te(){return v||C||(C=fetch("data/audio_manifest.json").then(n=>n.ok?n.json():null).then(n=>(n&&Array.isArray(n.items)?v=new Set(n.items.map(e=>e.path)):v=new Set,v)).catch(()=>(v=new Set,v)),C)}function ae(n){return v?v.has(n):!1}function ne(n){const e=A();if(e&&e!=="en"){const s=n[`explanation_${e}`];if(typeof s=="string"&&s.trim())return s}return n.explanation_en||""}function se(n){const e=A();if(e&&e!=="en"){const s=n[`meaning_${e}`];if(typeof s=="string"&&s.trim())return s}return n.meaning_en||""}function re(n){const e=A();if(e&&e!=="en"&&n.l1_notes&&typeof n.l1_notes=="object"){const s=n.l1_notes[e];if(typeof s=="string"&&s.trim())return s}return null}const S=[["Sentence Basics",["Copula and Basic Sentence Structure","Particles","Demonstratives","Question Words"]],["Verbs",["Verbs - Tense and Politeness (\u307E\u3059-form)","Verbs - Plain (Dictionary) Form and Negation","Te-form and Related Patterns","Existence and Possession","Desiderative and Volitional","Giving and Receiving (basic)","Additional Upper N5 / Borderline Patterns - Permission and Obligation","Additional Upper N5 / Borderline Patterns - Experience and Advice","Additional Upper N5 / Borderline Patterns - Compound and Listed Actions","Additional Upper N5 / Borderline Patterns - Excess","Additional Upper N5 / Borderline Patterns - Intention","Additional Upper N5 / Borderline Patterns - Way of Doing","Additional Upper N5 / Borderline Patterns - Prohibitive (Casual)"]],["Adjectives and Comparison",["Adjectives","Comparison and Preference"]],["Time, Counters, Connectives",["Counters and Quantity","Time Expressions","Conjunctions and Connectives","Asking and Stating with \u304B\u3089 / \u306E\u3067 (basic causation)","Existence-of-Plans and Frequency"]],["Set Phrases and Discourse",["Nominalization and Modification","Common Set Patterns","Functional Expressions (Non-Grammar, Common Usage)","Other Core Patterns","Honorific / Polite Vocabulary at N5 (functional)","Additional Upper N5 / Borderline Patterns - Explanation and Emphasis","Additional Upper N5 / Borderline Patterns - Quotation (Casual)","Additional Upper N5 / Borderline Patterns - Sentence-Final Exclamation"]]],B={"n5-135":"Verbs","n5-144":"Verbs","n5-153":"Verbs","n5-154":"Verbs","n5-162":"Verbs","n5-163":"Verbs"};function T(n){if(typeof n=="object"&&n&&n.id in B)return B[n.id];const e=typeof n=="string"?n:n?.category||"";for(const[s,p]of S)if(p.includes(e))return s;return"Set Phrases and Discourse"}function oe(n){const e=new Map;for(const[p]of S)e.set(p,[]);for(const p of n){const g=T(p);e.has(g)&&e.get(g).push(p)}const s=[];for(const[,p]of e)p.sort((g,y)=>(g.patternOrder??0)-(y.patternOrder??0)),s.push(...p);return s}let P="";function ie(n,e){return e?[n.pattern,n.meaning_en,n.meaning_ja||"",n.notes||"",(n.examples||[]).map(p=>p.ja).join(" ")].join(" ").toLowerCase().includes(e):!0}function U(n,e){const s=new Map;for(const[r]of S)s.set(r,[]);const p=P.trim().toLowerCase(),g=e.patterns.filter(r=>ie(r,p));for(const r of g){const i=T(r);s.get(i).push(r)}const y=r=>r.toLowerCase().replace(/[^a-z0-9]+/g,"-").replace(/^-|-$/g,"");let l=`
      ...
      <tbody>
        ${e.map((l,$)=>`
          <tr>
            ...
            ${$===0?`<td class="pattern-usage-form" rowspan="${e.length}" lang="ja">${h(n.pattern)}</td>`:""}
          </tr>
        `).join("")}
      </tbody>
    </table>
  ...
  `:"",j=g.map((t,d)=>{const _=!t.ja||t.ja.includes("(see ")?null:`audio/grammar/${e.id}.${d}.mp3`,b=_&&ae(_)?_:null;return`
    <li>
  ...
  `}).join(""),I=y.map(t=>{if(t.kind==="register_variant"){const d=t.label_a?`<span class="variant-label">${a(t.label_a)}</span>`:"",m=t.label_b?`<span class="variant-label">${a(t.label_b)}</span>`:"",_=t.form_a??t.wrong??"",b=t.form_b??t.right??"";return`
        <li class="variant-pair">
          <div class="variant-row">${d}<span class="variant-form">${h(_)}</span></div>
          <div class="variant-row">${m}<span class="variant-form">${h(b)}</span></div>
          <span class="why">${a(t.why)}</span>
        </li>
      `}return`
      <li>
        <div><span class="wrong">${h(t.wrong)}</span></div>
        <div><span class="right">${h(t.right)}</span></div>
        <span class="why">${a(t.why)}</span>
      </li>
    `}).join(""),E=Array.isArray(e.wrong_corrected_pair)?e.wrong_corrected_pair:[],M=t=>{if(!t)return"";const d=`grammar_detail.cat_${t}`,m=o(d)!==d?o(d):t;return`<span class="error-category-badge cat-${a(t)}">${a(m)}</span>`},R=o("grammar_detail.wcp_wrong"),D=o("grammar_detail.wcp_correct"),F=E.map(t=>`
    <li>
      <div class="wcp-header">${M(t.error_category)}</div>
      <div class="wcp-row wcp-row-wrong">
        <span class="wcp-mark" aria-hidden="true">\u2717</span>
        <span class="wcp-label">${a(R)}</span>
        <span class="wrong">${h(t.wrong)}</span>
      </div>
      <div class="wcp-row wcp-row-correct">
        <span class="wcp-mark" aria-hidden="true">\u2713</span>
        <span class="wcp-label">${a(D)}</span>
        <span class="right">${h(t.correct)}</span>
      </div>
      <span class="why">${a(t.why)}</span>
    </li>
  ...
  `:"",O="",L=Array.isArray(e.public_domain_refs)?e.public_domain_refs:[],z=L.length?`
    <section class="pd-refs">
      ...
      <ul class="pd-refs-list">
        ${L.map(t=>{const d=a(t.source_type||"?"),m=a(t.work_title||""),_=a(t.author||""),b=t.author_death_year?` (died ${t.author_death_year})`:"",k=t.pd_status?`<span class="pd-status muted small">${a(t.pd_status)}</span>`:"",Q=t.context?`<p class="pd-context muted small">${a(t.context)}</p>`:"",J=t.pattern_role?`<p class="pd-role muted small"><em>${a(t.pattern_role)}</em></p>`:"",K=t.url?` <a href="${a(t.url)}" target="_blank" rel="noopener" class="pd-link">\u2197 source</a>`:"",X=t.quote_ja?`<blockquote class="pd-quote-ja" lang="ja">${a(t.quote_ja)}</blockquote>`:"",Y=t.quote_translation_en?`<p class="pd-quote-en muted small">${a(t.quote_translation_en)}</p>`:"";return`
            <li class="pd-ref pd-ref-${d}">
              <div class="pd-ref-header">
                <strong class="pd-work-title" lang="ja">${m}</strong>${K}
                ${_?`<span class="pd-author muted small">\u2014 ${_}${b}</span>`:""}
              </div>
              ${k}
              ${X}
              ${Y}
  ...
  `;n.innerHTML=W,document.getElementById("mark-known")?.addEventListener("change",t=>{V.setManuallyKnown(e.id,t.target.checked),de(n,e,s)}),document.getElementById("pattern-print-btn")?.addEventListener("click",()=>{window.print()})}export{oe as buildOrderedPatternList,de as renderGrammarPatternDetail,U as renderGrammarTOC};
