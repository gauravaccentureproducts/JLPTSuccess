import{renderJa as _}from"./furigana.js";import*as V from"./storage.js";import{esc as a,wireExpandCollapseControls as W}from"./learn.js";import{currentLocale as A,t as r}from"./i18n.js";let $=null,x=null;async function K(){return $||x||(x=fetch("data/audio_manifest.json").then(n=>n.ok?n.json():null).then(n=>(n&&Array.isArray(n.items)?$=new Set(n.items.map(e=>e.path)):$=new Set,$)).catch(()=>($=new Set,$)),x)}function J(n){return $?$.has(n):!1}function X(n){const e=A();if(e&&e!=="en"){const s=n[`explanation_${e}`];if(typeof s=="string"&&s.trim())return s}return n.explanation_en||""}function Y(n){const e=A();if(e&&e!=="en"){const s=n[`meaning_${e}`];if(typeof s=="string"&&s.trim())return s}return n.meaning_en||""}function Z(n){const e=A();if(e&&e!=="en"&&n.l1_notes&&typeof n.l1_notes=="object"){const s=n.l1_notes[e];if(typeof s=="string"&&s.trim())return s}return null}const S=[["Sentence Basics",["Copula and Basic Sentence Structure","Particles","Demonstratives","Question Words"]],["Verbs",["Verbs - Tense and Politeness (\u307E\u3059-form)","Verbs - Plain (Dictionary) Form and Negation","Te-form and Related Patterns","Existence and Possession","Desiderative and Volitional","Giving and Receiving (basic)","Additional Upper N5 / Borderline Patterns - Permission and Obligation","Additional Upper N5 / Borderline Patterns - Experience and Advice","Additional Upper N5 / Borderline Patterns - Compound and Listed Actions","Additional Upper N5 / Borderline Patterns - Excess","Additional Upper N5 / Borderline Patterns - Intention","Additional Upper N5 / Borderline Patterns - Way of Doing","Additional Upper N5 / Borderline Patterns - Prohibitive (Casual)"]],["Adjectives and Comparison",["Adjectives","Comparison and Preference"]],["Time, Counters, Connectives",["Counters and Quantity","Time Expressions","Conjunctions and Connectives","Asking and Stating with \u304B\u3089 / \u306E\u3067 (basic causation)","Existence-of-Plans and Frequency"]],["Set Phrases and Discourse",["Nominalization and Modification","Common Set Patterns","Functional Expressions (Non-Grammar, Common Usage)","Other Core Patterns","Honorific / Polite Vocabulary at N5 (functional)","Additional Upper N5 / Borderline Patterns - Explanation and Emphasis","Additional Upper N5 / Borderline Patterns - Quotation (Casual)","Additional Upper N5 / Borderline Patterns - Sentence-Final Exclamation"]]],B={"n5-135":"Verbs","n5-144":"Verbs","n5-153":"Verbs","n5-154":"Verbs","n5-162":"Verbs","n5-163":"Verbs"};function U(n){if(typeof n=="object"&&n&&n.id in B)return B[n.id];const e=typeof n=="string"?n:n?.category||"";for(const[s,d]of S)if(d.includes(e))return s;return"Set Phrases and Discourse"}function ee(n){const e=new Map;for(const[d]of S)e.set(d,[]);for(const d of n){const u=U(d);e.has(u)&&e.get(u).push(d)}const s=[];for(const[,d]of e)d.sort((u,b)=>(u.patternOrder??0)-(b.patternOrder??0)),s.push(...d);return s}let C="";function te(n,e){return e?[n.pattern,n.meaning_en,n.meaning_ja||"",n.notes||"",(n.examples||[]).map(d=>d.ja).join(" ")].join(" ").toLowerCase().includes(e):!0}function I(n,e){const s=new Map;for(const[o]of S)s.set(o,[]);const d=C.trim().toLowerCase(),u=e.patterns.filter(o=>te(o,d));for(const o of u){const l=U(o);s.get(l).push(o)}const b=o=>o.toLowerCase().replace(/[^a-z0-9]+/g,"-").replace(/^-|-$/g,"");let c=`
  ...
  `;for(const[o,l]of s){if(l.length===0)continue;l.sort((i,g)=>(i.patternOrder??0)-(g.patternOrder??0));const p=!!d;c+=`<details class="toc-category" id="cat-${b(o)}"${p?" open":""}>`,c+=`<summary><h3>${a(o)} <span class="cat-count muted small">(${l.length})</span></h3></summary>`,c+='<div class="grammar-grid">';for(const i of l){const g=(()=>{const y=(i.examples||[]).filter(w=>w&&w.ja);return y[0]?y[0].ja:""})(),v=i.genki_lesson?`G${i.genki_lesson.book}\xB7L${i.genki_lesson.lesson}`:"",P=(()=>{const y=A&&A();if(y&&y!=="en"){const w=i[`meaning_${y}`];if(typeof w=="string"&&w.trim())return w}return i.meaning_en||""})();c+=`
        <a class="grammar-card" href="#/learn/${encodeURIComponent(i.id)}">
          ...
          <span class="grammar-card-print-meaning">${a(P)}</span>
          <span class="grammar-card-print-example" lang="ja">${a(g)}</span>
          ${v?`<span class="grammar-card-print-lesson">${a(v)}</span>`:""}
        </a>
      `}c+="</div></details>"}u.length===0?c+='<div class="placeholder"><p>No patterns match the current filter.</p></div>':e.patterns.length===1&&(c+='<div class="placeholder" style="margin-top:24px"><p>Scaffold currently has 1 example pattern. Add more to <code>data/grammar.json</code> as you author content.</p></div>'),c+=`
      ...
      <tbody>
        ${e.map((c,h)=>`
          <tr>
            ...
            ${h===0?`<td class="pattern-usage-form" rowspan="${e.length}" lang="ja">${_(n.pattern)}</td>`:""}
          </tr>
        `).join("")}
      </tbody>
    </table>
  ...
  `:"";return`<section class="pattern-usage">${d}${u}${b}</section>`}async function se(n,e,s){await K();const d=e.form_rules?.conjugations??[],u=e.examples??[],b=e.common_mistakes??[],c=V.getPatternEntry(e.id),h=!!c?.isManuallyKnown,o=!!c?.isMastered,l=!!c?.isWeak&&!o,p=Array.isArray(s)?ee(s):[],i=p.findIndex(t=>t.id===e.id),g=i>0?p[i-1]:null,v=i>=0&&i<p.length-1?p[i+1]:null,P=g||v?`
  ...
  `:"",y=u.map((t,m)=>{const j=!t.ja||t.ja.includes("(see ")?null:`audio/grammar/${e.id}.${m}.mp3`,k=j&&J(j)?j:null;return`
    <li>
  ...
  `).join(""),E=Array.isArray(e.wrong_corrected_pair)?e.wrong_corrected_pair:[],R=t=>{if(!t)return"";const m=`grammar_detail.cat_${t}`,f=r(m)!==m?r(m):t;return`<span class="error-category-badge cat-${a(t)}">${a(f)}</span>`},q=r("grammar_detail.wcp_wrong"),D=r("grammar_detail.wcp_correct"),F=E.map(t=>`
    <li>
  ...
  `).join(""),L=e.politeness_ladder&&typeof e.politeness_ladder=="object"?e.politeness_ladder:null,O=L?`
    <section class="politeness-ladder">
      ...
      <table class="ladder-table">
        <tbody>
          ${["casual","polite","humble","respectful"].map(t=>{const m=L[t];if(!m)return"";const f=r(`grammar_detail.ladder_${t}`);return`
  ...
  `:"",N=Array.isArray(e.authentic_citations)?e.authentic_citations:[],G=N.length?`
    <section class="authentic-citations">
      ...
      <ul class="citations-list">
        ${N.map(t=>`
          <li>
            <strong class="citation-source">${a(t.source||"")}</strong>
            ${t.context?` \u2014 <span class="citation-context">${_(t.context)}</span>`:""}
          </li>
        `).join("")}
      </ul>
    </section>
  `:"",H=o?'<span class="status-badge mastered">\u2605 Mastered</span>':l?'<span class="status-badge weak">Needs practice</span>':"",z=(()=>{const t=e.genki_lesson;return t?`<span class="pattern-lesson-tag" title="Genki ${t.book} Lesson ${t.lesson}">G${t.book}\xB7L${t.lesson}</span>`:""})(),Q=`
    <article class="pattern-detail">
      ...
      ${(()=>{const t=e.essay;if(!t||typeof t!="object")return"";const m=t.provenance==="needs_native_review",f=(j,k,T)=>!k&&!T?"":k?`<p><strong>${a(j)}:</strong> ${a(k)}</p>`:`<p><strong>${a(j)}:</strong> <span class="muted small">${a(T)}</span></p>`;return`
          <section class="pattern-essay">
            <h3 class="section-title">${a(r("grammar_detail.deep_dive"))} ${m?'<span class="essay-stub-badge muted small">stub</span>':""}</h3>
            ${f(r("grammar_detail.deep_dive_at_a_glance"),t.intro)}
            ${f(r("grammar_detail.deep_dive_why"),t.why_it_matters,m?"Pending native author.":"")}
            ${f(r("grammar_detail.deep_dive_pitfalls"),t.common_pitfalls)}
            ${f(r("grammar_detail.deep_dive_contrasts"),t.contrasts)}
            ${f(r("grammar_detail.deep_dive_practice"),t.closing_practice_tip,m?"Pending native author.":"")}
          </section>
        `})()}
      ...
      <section>
        <h3 class="section-title">${a(r("grammar_detail.examples"))} (${u.length})</h3>
      ...
      ${e.authentic_refs?.length?`
        ...
        <section class="grammar-authentic-refs">
          ...
          <ul class="authentic-ref-list">
            ${e.authentic_refs.map(t=>{const m=t.split(".")[1]||"authentic";return`<li><a href="#/authentic">${a(t)}</a> <span class="muted small">(${a(m)})</span></li>`}).join("")}
          </ul>
        </section>
      `:""}
    </article>
  `;n.innerHTML=Q,document.getElementById("mark-known")?.addEventListener("change",t=>{V.setManuallyKnown(e.id,t.target.checked),se(n,e,s)}),document.getElementById("pattern-print-btn")?.addEventListener("click",()=>{window.print()})}export{ee as buildOrderedPatternList,se as renderGrammarPatternDetail,I as renderGrammarTOC};
