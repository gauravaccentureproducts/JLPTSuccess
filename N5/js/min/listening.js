import{renderJa as s}from"./furigana.js";import*as f from"./storage.js";import{hasAlignedTranscript as $,renderTranscriptHTML as h,wireTranscriptSync as v}from"./listening-transcript.js";let l=null,c=null;const g={task:"\u304B\u3060\u3044\u308A\u304B\u3044 (\u30BF\u30B9\u30AF\u308A\u304B\u3044)",point:"\u30DD\u30A4\u30F3\u30C8\u308A\u304B\u3044",utterance:"\u306F\u3064\u308F\u3072\u3087\u3046\u3052\u3093",response:"\u305D\u304F\u3058\u304A\u3046\u3068\u3046"};async function y(){if(l)return l;try{const i=await fetch("data/listening.json");if(!i.ok)return l={items:[]},l;l=await i.json()}catch{l={items:[]}}return l}async function j(i){return await y(),c?m(i):u(i)}function u(i){const e=l.items||[];if(e.length===0){i.innerHTML=`
      <h2>${s("\u3061\u3087\u3046\u304B\u3044 \u308C\u3093\u3057\u3085\u3046")}</h2>
    ...
    ${Object.entries(o).map(([t,n])=>`
      <details class="listening-section">
        ...
        <ul class="listening-list">
          ${n.map(r=>`<li><button class="listening-pick" data-id="${d(r.id)}">${r.title_ja?s(r.title_ja):d(r.id)}</button></li>`).join("")}
        </ul>
      </details>
    `).join("")}
  `;const a=i.querySelector(".toc-expand-all"),p=i.querySelector(".toc-collapse-all");a&&a.addEventListener("click",()=>{i.querySelectorAll("details.listening-section").forEach(t=>t.open=!0)}),p&&p.addEventListener("click",()=>{i.querySelectorAll("details.listening-section").forEach(t=>t.open=!1)}),i.querySelectorAll("[data-id]").forEach(t=>{t.addEventListener("click",()=>{c={item:e.find(r=>r.id===t.dataset.id),picked:null},m(i)})})}function m(i){const e=c.item,o=c.picked,a=o!=null,p=o===e.correctAnswer;a&&f.setListeningCompleted(e.id),i.innerHTML=`
    <article class="listening-item">
      <div class="srs-progress">
        <span><a id="listening-back" href="#/listening">\u2190 ${s("\u30EA\u30B9\u30C8\u306B \u3082\u3069\u308B")}</a></span>
      </div>
      <h2>${e.title_ja?s(e.title_ja):d(e.id)}</h2>
      <p class="muted small">${s("\u3051\u3044\u3057\u304D")}: ${s(g[e.format]||e.format)}</p>
      <div class="listening-audio">
        ${e.audio?`<audio controls preload="none" src="${d(e.audio)}">Audio</audio>`:`<p class="muted small">${s("\u304A\u3093\u305B\u3044\u30D5\u30A1\u30A4\u30EB\u306F \u307E\u3060 \u3042\u308A\u307E\u305B\u3093\u3002")}</p>`}
      </div>
      ${e.prompt_ja?`<p>${s(e.prompt_ja)}</p>`:""}
      ${e.choices?`
        <div class="choice-grid">
          ${e.choices.map(t=>{let n="choice-button";return a?t===e.correctAnswer?n+=" correct-choice":t===o&&(n+=" wrong-choice"):o===t&&(n+=" selected"),`<button data-pick="${d(t)}" class="${n}" ${a?"disabled":""}>${s(t)}</button>`}).join("")}
        </div>
      `:""}
      ${a?`
        <div class="drill-feedback ${p?"correct":"incorrect"}">
          <div class="feedback-headline">${p?s("\u305B\u3044\u304B\u3044"):s("\u3056\u3093\u306D\u3093")}</div>
          ${$(e)?`<details open><summary>${s("\u30B9\u30AF\u30EA\u30D7\u30C8")}</summary>${h(e)}</details>`:e.script_ja?`<details><summary>${s("\u30B9\u30AF\u30EA\u30D7\u30C8\u3092 \u898B\u308B")}</summary><div>${s(e.script_ja)}</div></details>`:""}
