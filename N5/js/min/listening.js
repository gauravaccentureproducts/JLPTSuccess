import{renderJa as s}from"./furigana.js";import*as A from"./storage.js";import{hasAlignedTranscript as x,renderTranscriptHTML as E,wireTranscriptSync as T}from"./listening-transcript.js";import{t as d}from"./i18n.js";let u=null,$=null;const k={task:"\u304B\u3060\u3044\u308A\u304B\u3044 (\u30BF\u30B9\u30AF\u308A\u304B\u3044)",point:"\u30DD\u30A4\u30F3\u30C8\u308A\u304B\u3044",utterance:"\u306F\u3064\u308F\u3072\u3087\u3046\u3052\u3093",response:"\u305D\u304F\u3058\u304A\u3046\u3068\u3046"};async function L(){if(u)return u;try{const a=await fetch("data/listening.json");if(!a.ok)return u={items:[]},u;u=await a.json()}catch{u={items:[]}}return u}async function P(a,t){await L();const m=(t||"").trim();if(m){const p=(u.items||[]).find(h=>h.id===m);if(!p){$=null,location.hash="#/listening";return}return(!$||$.item?.id!==p.id)&&($={item:p,picked:null}),j(a)}return $=null,S(a)}function S(a){const t=u.items||[];if(t.length===0){a.innerHTML=`
      <h2>${s("\u3061\u3087\u3046\u304B\u3044 \u308C\u3093\u3057\u3085\u3046")}</h2>
    ...
    ${Object.entries(m).map(([i,c])=>`
      <details class="listening-section">
        <summary><h3>${s(k[i]||i)} <span class="muted small">(${c.length})</span></h3></summary>
        <ul class="listening-list">
          ${c.map(o=>`<li><button class="listening-pick" data-id="${n(o.id)}">${o.title_ja?s(o.title_ja):n(o.id)}</button></li>`).join("")}
        </ul>
      </details>
    `).join("")}
  `;const p=a.querySelector(".toc-expand-all"),h=a.querySelector(".toc-collapse-all");p&&p.addEventListener("click",()=>{a.querySelectorAll("details.listening-section").forEach(i=>i.open=!0)}),h&&h.addEventListener("click",()=>{a.querySelectorAll("details.listening-section").forEach(i=>i.open=!1)}),a.querySelectorAll("[data-id]").forEach(i=>{i.addEventListener("click",()=>{location.hash=`#/listening/${encodeURIComponent(i.dataset.id)}`})})}function j(a){const t=$.item,m=$.picked,p=m!=null,h=m===t.correctAnswer;p&&A.setListeningCompleted(t.id);const i=u?.items||[],c=i.findIndex(e=>e.id===t.id),o=c>0?i[c-1]:null,g=c>=0&&c<i.length-1?i[c+1]:null,w=o||g?`
    <nav class="listening-nav" aria-label="Listening item navigation">
      ${o?`<button type="button" class="listening-nav-btn listening-nav-prev" data-nav="prev" title="${n(o.title_ja||o.id)}">
             <span class="listening-nav-arrow" aria-hidden="true">&larr;</span>
             <span class="listening-nav-meta">
               <span class="listening-nav-label muted small">${s("\u307E\u3048")}</span>
               <span class="listening-nav-name" lang="ja">${o.title_ja?s(o.title_ja):n(o.id)}</span>
             </span>
           </button>`:'<span class="listening-nav-btn listening-nav-empty" aria-hidden="true"></span>'}
      ${g?`<button type="button" class="listening-nav-btn listening-nav-next" data-nav="next" title="${n(g.title_ja||g.id)}">
             <span class="listening-nav-meta">
               <span class="listening-nav-label muted small">${s("\u3064\u304E")}</span>
               <span class="listening-nav-name" lang="ja">${g.title_ja?s(g.title_ja):n(g.id)}</span>
             </span>
  ...
  `:"";a.innerHTML=`
    <article class="listening-item">
      ...
      <h2>${t.title_ja?s(t.title_ja):n(t.id)}</h2>
      <p class="muted small">${s("\u3051\u3044\u3057\u304D")}: ${s(k[t.format]||t.format)}</p>
      <div class="listening-audio">
        ${t.audio?`
          <audio id="listening-audio-${n(t.id)}" controls preload="none" src="${n(t.audio)}">Audio</audio>
          ${t.audio_slow?`
            ...
            <div class="listening-speed-toggle" role="group" aria-label="${n(s("\u3055\u3044\u305B\u3044 \u305D\u304F\u3069"))}">
              <button type="button" class="listening-speed-btn is-active" data-listening-speed="normal"
                      data-audio-target="listening-audio-${n(t.id)}"
                      data-audio-normal="${n(t.audio)}"
                      data-audio-slow="${n(t.audio_slow)}">
                ${s("\u3075\u3064\u3046")} (1.0\xD7)
              </button>
          ...
          `:""}
        `:`<p class="muted small">${s("\u304A\u3093\u305B\u3044\u30D5\u30A1\u30A4\u30EB\u306F \u307E\u3060 \u3042\u308A\u307E\u305B\u3093\u3002")}</p>`}
      </div>
      ...
      ${(()=>{const e=Array.isArray(t.listening_strategy_hints)?t.listening_strategy_hints:[];return e.length?`
          <aside class="listening-strategy-hints">
            <details>
              <summary><strong>${n(d("chokai_detail.strategy_hints"))} (${e.length})</strong></summary>
              <ul>
                ${e.map(l=>`<li class="muted small">${n(l)}</li>`).join("")}
              </ul>
            </details>
          </aside>
        `:""})()}
      ${(()=>{const e=t.speech_rate_classification;return!e||typeof e!="object"||!e.category?"":`
          <p class="muted small listening-speech-rate">
            <strong>${n(d("chokai_detail.speech_rate"))}:</strong> ${n(e.category)}
            ${e.morae_per_min?` <span class="muted">(${n(e.morae_per_min)} mora/min)</span>`:""}
            ${e.note?`<br><span class="muted small">${n(e.note)}</span>`:""}
          </p>
        `})()}
      ${(()=>{const e=t.register_signal_l;return!e||typeof e!="object"||!e.register?"":`
          <p class="muted small listening-register-signal">
            <strong>${n(d("chokai_detail.register"))}:</strong> ${n(e.register)}${e.confidence?` <span class="muted">(${n(e.confidence)} ${n(d("chokai_detail.confidence"))})</span>`:""}
            ${Array.isArray(e.signals)&&e.signals.length?`<br><span class="muted small">${n(d("chokai_detail.signals"))}: ${e.signals.map(n).join("; ")}</span>`:""}
          </p>
        `})()}
      ${(()=>{const e=t.speaker_demographics;return!e||typeof e!="object"||!Array.isArray(e.roles_detected)||!e.roles_detected.length?"":`
          <p class="muted small listening-speakers">
            <strong>${n(d("chokai_detail.speakers_detected"))} (${e.n_speakers_inferred||e.roles_detected.length}):</strong>
            ${e.roles_detected.map(l=>`<span class="speaker-role-chip" lang="ja">${n(l.tag||l.role||"?")}</span>`).join(" ")}
          </p>
        `})()}
      ${(()=>{const e=Array.isArray(t.prosody_hints)?t.prosody_hints:[];return e.length?`
          <aside class="listening-prosody">
            <details>
              <summary class="muted small"><strong>${n(d("chokai_detail.prosody_intonation_cues"))}</strong></summary>
              <ul>
                ${e.map(l=>`<li class="muted small">${n(l)}</li>`).join("")}
              </ul>
            </details>
      ...
      ${(()=>{const e=t.time_target_seconds;return!e||typeof e!="object"?"":`
          <p class="muted small listening-time-target">
            <strong>${n(d("chokai_detail.target_time"))}:</strong> ~${n(e.estimated_total_seconds||e.jlpt_target_seconds_per_question||"?")}s
            ${e.audio_seconds_estimated?`<span class="muted"> (${n(d("chokai_detail.audio_approx"))}${n(e.audio_seconds_estimated)}s)</span>`:""}
          </p>
        `})()}
      ${(()=>{const e=t.distractor_pattern_hint;return!e||typeof e!="object"||!e.pattern?"":`
          <aside class="listening-distractor-pattern">
            <details>
              <summary class="muted small"><strong>${n(d("chokai_detail.distractor_pattern"))}:</strong> ${n(e.pattern)}</summary>
              ${e.note?`<p class="muted small">${n(e.note)}</p>`:""}
              ${e.mentioned_count!=null?`<p class="muted small">${n(d("chokai_detail.mentioned_rejected_count"))}: ${n(e.mentioned_count)}</p>`:""}
            </details>
          </aside>
        `})()}
      ${t.authentic_categories?.length?`
        ...
        <aside class="listening-authentic-link muted small">
          ...
          ${t.authentic_categories.map(e=>`<a href="#/authentic" class="authentic-cat-chip">${n(e)}</a>`).join(" ")}
        </aside>
      `:""}
          ...
          <details>
            ...
            <ul>
              ${t.inference_question_expansion.prompts.map(e=>`
                <li>
                  <span class="inference-type-chip muted small">${n(e.type||"inference")}</span>
                  <span lang="ja">${n(e.prompt_ja||"")}</span>
                  ${e.prompt_en?`<br><span class="muted small">${n(e.prompt_en)}</span>`:""}
                  ${e.hint?`<br><em class="muted small">Hint: ${n(e.hint)}</em>`:""}
                </li>
              `).join("")}
            </ul>
          </details>
