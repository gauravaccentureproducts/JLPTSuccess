import*as u from"./storage.js";import{t as w}from"./i18n.js";let f=null;async function k(){return f||(f=await(await fetch("data/grammar.json")).json(),f)}async function N(c){const r=await k(),i=(r.patterns||[]).map(p=>p.id),a=new Map((r.patterns||[]).map(p=>[p.id,p])),o=u.getMasteredPatternIds(),e=u.getWeakPatternIds(),t=u.getSeenPatternIds(),s=i.filter(p=>!t.includes(p)),n=u.getResults(),l=n.length,h=n[n.length-1];if(l===0&&t.length===0){c.innerHTML=`
      <h2>${w("page.summary")}</h2>
    ...
    <section class="summary-stats">
      ...
      <div class="stat-card neutral">
        ...
        ${h?`<div class="stat-hint">Last: ${T(h.timestamp)}</div>`:'<div class="stat-hint">None yet</div>'}
      </div>
    </section>
  ...
  `,document.getElementById("reset-progress")?.addEventListener("click",()=>{confirm("Reset all progress? This clears every test result, the rolling history, and weak-pattern flags.")&&confirm("Are you sure? This cannot be undone.")&&(u.reset(),location.hash="#/learn",location.reload())}),document.getElementById("retake-diagnostic")?.addEventListener("click",()=>{location.hash="#/diagnostic"})}function I(c,r){const i=new Map;for(const e of c)for(const t of e.responses||[]){if(!t.grammarPatternId)continue;const s=i.get(t.grammarPatternId)||{attempts:0,correct:0};s.attempts+=1,t.isCorrect&&(s.correct+=1),i.set(t.grammarPatternId,s)}if(i.size===0)return"";const a=[...i.entries()].map(([e,t])=>({pid:e,attempts:t.attempts,correct:t.correct,accuracy:t.correct/t.attempts,pattern:r.get(e)})).filter(e=>e.attempts>=2).sort((e,t)=>e.accuracy-t.accuracy).slice(0,6);return a.length===0?"":`
  ...
  `}function S(c){if(!c||c.length===0)return"";const r=c.slice(-10).reverse(),i=r.map(a=>{const o=a.total>0?Math.round(a.correct/a.total*100):0,e=new Date(a.timestamp).toLocaleString();return`<tr><td>${d(e)}</td><td>${a.total}</td><td>${a.correct}</td><td>${o}%</td></tr>`}).join("");return`
  ...
  `}function M(c,r,i,a){const o=new Map;for(const s of c){const n=s.category||"Other";o.has(n)||o.set(n,{order:s.categoryOrder??99,patterns:[]}),o.get(n).patterns.push(s)}const e=[...o.entries()].sort((s,n)=>s[1].order-n[1].order),t=e.map(([s,{patterns:n}])=>{const l=n.length,h=n.filter(m=>r.has(m.id)).length,p=n.filter(m=>i.has(m.id)).length,y=n.filter(m=>a.has(m.id)).length;let g="untested",v=`${l} pattern${l===1?"":"s"} - none seen yet`;p>0?(g="weak",v=`${p} weak / ${l} total. Review needed.`):h===l&&l>0?(g="mastered",v=`All ${l} mastered.`):y>0&&(g="in-progress",v=`${h} mastered / ${y} seen / ${l} total.`);const b=l>0?Math.round(h/l*100):0;return`
