import*as g from"./storage.js";import{t as h}from"./i18n.js";import{navigateTo as b}from"./router.js";let $=null;async function I(){return $||($=await(await fetch("data/grammar.json")).json(),$)}async function L(c){const r=await I(),i=(r.patterns||[]).map(p=>p.id),a=new Map((r.patterns||[]).map(p=>[p.id,p])),o=g.getMasteredPatternIds(),e=g.getWeakPatternIds(),t=g.getSeenPatternIds(),s=i.filter(p=>!t.includes(p)),n=g.getResults(),l=n.length,m=n[n.length-1];if(l===0&&t.length===0){c.innerHTML=`
  ...
  `,document.getElementById("reset-progress")?.addEventListener("click",()=>{confirm("Reset all progress? This clears every test result, the rolling history, and weak-pattern flags.")&&confirm("Are you sure? This cannot be undone.")&&(g.reset(),b("learn"),location.reload())}),document.getElementById("retake-diagnostic")?.addEventListener("click",()=>{b("diagnostic")})}function x(c,r){const i=new Map;for(const e of c)for(const t of e.responses||[]){if(!t.grammarPatternId)continue;const s=i.get(t.grammarPatternId)||{attempts:0,correct:0};s.attempts+=1,t.isCorrect&&(s.correct+=1),i.set(t.grammarPatternId,s)}if(i.size===0)return"";const a=[...i.entries()].map(([e,t])=>({pid:e,attempts:t.attempts,correct:t.correct,accuracy:t.correct/t.attempts,pattern:r.get(e)})).filter(e=>e.attempts>=2).sort((e,t)=>e.accuracy-t.accuracy).slice(0,6);return a.length===0?"":`
  ...
  `}function R(c,r,i,a){const o=new Map;for(const s of c){const n=s.category||"Other";o.has(n)||o.set(n,{order:s.categoryOrder??99,patterns:[]}),o.get(n).patterns.push(s)}const e=[...o.entries()].sort((s,n)=>s[1].order-n[1].order),t=e.map(([s,{patterns:n}])=>{const l=n.length,m=n.filter(u=>r.has(u.id)).length,p=n.filter(u=>i.has(u.id)).length,w=n.filter(u=>a.has(u.id)).length;let v="untested",f=`${l} pattern${l===1?"":"s"} - none seen yet`;p>0?(v="weak",f=`${p} weak / ${l} total. Review needed.`):m===l&&l>0?(v="mastered",f=`All ${l} mastered.`):w>0&&(v="in-progress",f=`${m} mastered / ${w} seen / ${l} total.`);const k=l>0?Math.round(m/l*100):0;return`
      <div class="heat-cell heat-${v}" title="${d(f)}" data-cat="${d(s)}">
        ...
        <div class="heat-bar"><div style="width:${k}%"></div></div>
      </div>
    `}).join("");return`
