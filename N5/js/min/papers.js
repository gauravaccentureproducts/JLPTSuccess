import"./furigana.js";import*as S from"./storage.js";let b=null;const y=new Map;let f=null,m="setup",h=null;const E={moji:11,goi:11,bunpou:23,dokkai:23};let d=null,$=null;async function k(){if(b)return b;const t=await fetch("data/papers/manifest.json");if(!t.ok)throw new Error(`Failed to load papers manifest: ${t.status}`);return b=await t.json(),b}async function L(t,e){const a=`${t}-${e}`;if(y.has(a))return y.get(a);const s=await fetch(`data/papers/${t}/paper-${e}.json`);if(!s.ok)throw new Error(`Failed to load paper ${a}: ${s.status}`);const p=await s.json();return y.set(a,p),p}function j(t){return`jlpt-n5-tutor.paper.${t}`}function x(t){try{const e=localStorage.getItem(j(t));return e?JSON.parse(e):{attempts:0,bestScore:null,lastScore:null}}catch{return{attempts:0,bestScore:null,lastScore:null}}}function T(t,e,a){const s=x(t),p={attempts:(s.attempts||0)+1,bestScore:s.bestScore==null?{correct:e,total:a}:e>s.bestScore.correct?{correct:e,total:a}:s.bestScore,lastScore:{correct:e,total:a,dateIso:new Date().toISOString()}};return localStorage.setItem(j(t),JSON.stringify(p)),p}async function D(t,e){const a=(e||"").split("/").filter(Boolean);if(a.length<2&&(m==="attempting"||m==="results")&&(m="setup",f=null,h=null),m==="attempting"&&f)return v(t);if(m==="results"&&h)return P(t);if(a.length===0)return M(t);if(a.length===1)return A(t,a[0]);if(a.length===2){const[s,p]=a;return B(t,s,parseInt(p,10))}return M(t)}async function M(t){const e=await k(),a=e.categories.map(s=>{const p=s.papers.filter(r=>(x(r.id).attempts||0)>0).length;return`
  ...
  `}function _(t){return`<span class="paper-cat-letter" lang="ja">${{moji:"\u5B57",goi:"\u8A9E",bunpou:"\u6CD5",dokkai:"\u8AAD"}[t]||"?"}</span>`}async function A(t,e){const s=(await k()).categories.find(r=>r.id===e);if(!s){t.innerHTML=`
      <article class="papers-index">
    ...
    `;return}const p=s.papers.map(r=>{const l=x(r.id),o=(l.attempts||0)>0,n=l.bestScore,i=o?`<span class="paper-badge paper-badge-done">${n?`${n.correct}/${n.total}`:"Done"}</span>`:'<span class="paper-badge paper-badge-new">New</span>';return`
  ...
  `).join("");t.innerHTML=`
    <article class="paper-attempting">
      ...
      <form class="paper-choices" id="paper-choices-form">${l}</form>
...
`),a=[];let s=!1,p=!1,r=[];function l(){if(r.length===0)return;const n=r[0],i=r.slice(2);let u='<table class="paper-passage-table"><thead><tr>';for(const g of n)u+=`<th lang="ja">${g}</th>`;u+="</tr></thead><tbody>";for(const g of i){u+="<tr>";for(const I of g)u+=`<td lang="ja">${I}</td>`;u+="</tr>"}u+="</tbody></table>",a.push(u),r=[],s=!1}for(let n of e){let i=n;if(i.startsWith("> ")||i===">")p=!0,i=i.replace(/^> ?/,"");else if(i.trim()===""){s&&l(),a.length&&!a[a.length-1].endsWith("<br>")&&a.push("<br>");continue}const u=i.match(/^\|(.*)\|$/);if(u){const g=u[1].split("|").map(I=>I.trim());s=!0,r.push(g);continue}s&&l(),i.trim()&&a.push(`<div class="paper-passage-line" lang="ja">${i}</div>`)}s&&l();const o=a.join(`
`);return p?`<blockquote class="paper-passage-blockquote">${o}</blockquote>`:o}function q(t){const e=f;d&&(clearInterval(d),d=null),$=null;let a=0;const s=e.questions.map((r,l)=>{const o=e.answers[l],n=o===r.correctIndex;return n&&(a+=1),{idx:l,kbSourceId:r.kbSourceId,stem_html:r.stem_html,choices:r.choices,correctIndex:r.correctIndex,chosen:o,isRight:n,rationale:r.rationale||""}}),p=e.questions.length;T(e.paperId,a,p);try{e.questions.forEach((r,l)=>{const o=e.answers[l]===r.correctIndex;S.recordAttempt?.(`paper:${e.categoryId}:${r.kbSourceId}`,o,"paper")})}catch{}return h={paperId:e.paperId,paperName:e.paper.name,categoryId:e.categoryId,correct:a,total:p,detail:s},m="results",f=null,P(t)}function P(t){const e=h;if(!e){t.innerHTML='<p>No results to display. <a href="#/papers">Pick a paper.</a></p>';return}const a=Math.round(e.correct/e.total*100),s=a>=80?"Great work":a>=60?"Solid":a>=40?"Keep going":"Review and retry",p=e.detail.map(r=>`
    <li class="paper-review-item ${r.isRight?"review-right":"review-wrong"}">
      ...
      <ol class="paper-review-choices">
        ${r.choices.map((l,o)=>`<li class="${o===r.correctIndex?"choice-correct":o===r.chosen?"choice-chosen-wrong":""}" lang="ja">${c(l)}</li>`).join("")}
      </ol>
