import"./furigana.js";import*as I from"./storage.js";let $=null;const v=new Map;let m=null,d="setup",g=null;const M={moji:11,goi:11,bunpou:23,dokkai:23};let i=null,f=null;async function y(){if($)return $;const t=await fetch("data/papers/manifest.json");if(!t.ok)throw new Error(`Failed to load papers manifest: ${t.status}`);return $=await t.json(),$}async function q(t,e){const s=`${t}-${e}`;if(v.has(s))return v.get(s);const a=await fetch(`data/papers/${t}/paper-${e}.json`);if(!a.ok)throw new Error(`Failed to load paper ${s}: ${a.status}`);const n=await a.json();return v.set(s,n),n}function S(t){return`jlpt-n5-tutor.paper.${t}`}function w(t){try{const e=localStorage.getItem(S(t));return e?JSON.parse(e):{attempts:0,bestScore:null,lastScore:null}}catch{return{attempts:0,bestScore:null,lastScore:null}}}function P(t,e,s){const a=w(t),n={attempts:(a.attempts||0)+1,bestScore:a.bestScore==null?{correct:e,total:s}:e>a.bestScore.correct?{correct:e,total:s}:a.bestScore,lastScore:{correct:e,total:s,dateIso:new Date().toISOString()}};return localStorage.setItem(S(t),JSON.stringify(n)),n}async function A(t,e){const s=(e||"").split("/").filter(Boolean);if(s.length<2&&(d==="attempting"||d==="results")&&(d="setup",m=null,g=null),d==="attempting"&&m)return h(t);if(d==="results"&&g)return j(t);if(s.length===0)return x(t);if(s.length===1)return L(t,s[0]);if(s.length===2){const[a,n]=s;return _(t,a,parseInt(n,10))}return x(t)}async function x(t){const e=await y(),s=e.categories.map(a=>{const n=a.papers.filter(r=>(w(r.id).attempts||0)>0).length;return`
  ...
  `}function E(t){return`<span class="paper-cat-letter" lang="ja">${{moji:"\u5B57",goi:"\u8A9E",bunpou:"\u6CD5",dokkai:"\u8AAD"}[t]||"?"}</span>`}async function L(t,e){const a=(await y()).categories.find(r=>r.id===e);if(!a){t.innerHTML=`
    ...
    `;return}const n=a.papers.map(r=>{const o=w(r.id),u=(o.attempts||0)>0,p=o.bestScore,l=u?`<span class="paper-badge paper-badge-done">${p?`${p.correct}/${p.total}`:"Done"}</span>`:'<span class="paper-badge paper-badge-new">New</span>';return`
  ...
  `}async function _(t,e,s){if(!Number.isInteger(s)||s<1){t.innerHTML="<p>Invalid paper number.</p>";return}const a=await q(e,s),n=!!I.getSettings().examMode,r=M[e]||15;return m={paper:a,paperId:a.id,categoryId:e,paperNumber:s,questions:a.questions,answers:new Array(a.questions.length).fill(null),currentIdx:0,submitted:!1,examMode:n,durationSec:n?r*60:null},n?(f=Date.now()+r*60*1e3,i&&clearInterval(i),i=setInterval(()=>T(t),1e3)):(f=null,i&&(clearInterval(i),i=null)),d="attempting",h(t)}function T(t){if(!m||!f)return;const e=f-Date.now(),s=document.getElementById("paper-timer");if(!s)return;if(e<=0){i&&clearInterval(i),i=null,s.textContent="00:00",s.classList.add("timer-expired"),k(t);return}const a=Math.floor(e/6e4),n=Math.floor(e%6e4/1e3);s.textContent=`${String(a).padStart(2,"0")}:${String(n).padStart(2,"0")}`,e<6e4&&s.classList.add("timer-low")}function h(t){const e=m;if(!e){t.innerHTML='<p>No active session. <a href="#/papers">Pick a paper.</a></p>';return}const s=e.questions[e.currentIdx],a=e.questions.length,n=e.answers.filter(p=>p!==null).length,r=s.passage_text?`<div class="paper-passage" lang="ja">${b(s.passage_text)}</div>`:"",o=s.choices.map((p,l)=>`
    <label class="paper-choice ${e.answers[e.currentIdx]===l?"selected":""}">
      ...
      <span class="paper-choice-text" lang="ja">${b(p)}</span>
    </label>
  `).join("");t.innerHTML=`
    <article class="paper-attempting">
      ...
      ${r}
      <div class="paper-question-stem" lang="ja">${b(s.stem_html)}</div>
  ...
  `,document.getElementById("paper-choices-form").addEventListener("change",p=>{p.target&&p.target.name==="choice"&&(e.answers[e.currentIdx]=parseInt(p.target.value,10),h(t))}),document.getElementById("paper-prev")?.addEventListener("click",()=>{e.currentIdx>0&&(e.currentIdx-=1,h(t))}),document.getElementById("paper-next")?.addEventListener("click",()=>{e.currentIdx<a-1&&(e.currentIdx+=1,h(t))}),document.getElementById("paper-submit")?.addEventListener("click",()=>{k(t)})}function B(t){t<0&&(t=0);const e=Math.floor(t/6e4),s=Math.floor(t%6e4/1e3);return`${String(e).padStart(2,"0")}:${String(s).padStart(2,"0")}`}function b(t){return t?`<span class="ja-text" lang="ja">${t}</span>`:""}function k(t){const e=m;i&&(clearInterval(i),i=null),f=null;let s=0;const a=e.questions.map((r,o)=>{const u=e.answers[o],p=u===r.correctIndex;return p&&(s+=1),{idx:o,kbSourceId:r.kbSourceId,stem_html:r.stem_html,choices:r.choices,correctIndex:r.correctIndex,chosen:u,isRight:p,rationale:r.rationale||""}}),n=e.questions.length;P(e.paperId,s,n);try{e.questions.forEach((r,o)=>{const u=e.answers[o]===r.correctIndex;I.recordAttempt?.(`paper:${e.categoryId}:${r.kbSourceId}`,u,"paper")})}catch{}return g={paperId:e.paperId,paperName:e.paper.name,categoryId:e.categoryId,correct:s,total:n,detail:a},d="results",m=null,j(t)}function j(t){const e=g;if(!e){t.innerHTML='<p>No results to display. <a href="#/papers">Pick a paper.</a></p>';return}const s=Math.round(e.correct/e.total*100),a=s>=80?"Great work":s>=60?"Solid":s>=40?"Keep going":"Review and retry",n=e.detail.map(r=>`
    <li class="paper-review-item ${r.isRight?"review-right":"review-wrong"}">
      ...
      <ol class="paper-review-choices">
        ${r.choices.map((o,u)=>`<li class="${u===r.correctIndex?"choice-correct":u===r.chosen?"choice-chosen-wrong":""}" lang="ja">${c(o)}</li>`).join("")}
      </ol>
  ...
  `,document.getElementById("paper-retry")?.addEventListener("click",()=>{d="setup",g=null})}function c(t){return String(t??"").replace(/[&<>"']/g,e=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[e])}export{A as renderPapers};
