import*as e from"./storage.js";import{t as r,currentLocale as _}from"./i18n.js";import{gatherSignal as K,recommend as O}from"./pedagogy-recommender.js";let v=null,j=null;async function G(){if(v)return v;const t=["grammar","vocab","kanji","reading","listening"].map(n=>fetch(`data/${n}.json`).then(o=>o.ok?o.json():null).catch(()=>null)),[u,c,y,m,d]=await Promise.all(t),i=(n,...o)=>{if(!n)return 0;for(const f of o)if(Array.isArray(n[f]))return n[f].length;return 0};if(v={grammar:i(u,"patterns"),vocab:i(c,"entries"),kanji:i(y,"entries"),reading:i(m,"passages"),listening:i(d,"items")},u&&Array.isArray(u.patterns)){j={};for(const n of u.patterns){if(!n?.id)continue;const o=n.pattern||n.name||n.meaning_en||"";j[n.id]=o?`${n.id} - ${o}`:n.id}}return v}const p=s=>{const t=_()==="hi"?"hi-IN":"en-US";return Intl.NumberFormat(t).format(s||0)};function V(s){const t=e.getHistory(),u=e.getKnownKanji?e.getKnownKanji():{},c=e.getKnownVocab?e.getKnownVocab():{},y=e.getResults(),m=Object.values(t).filter($=>$&&($.isMastered||$.isManuallyKnown)).length,d=Object.keys(c).length,i=Object.keys(u).length,n=e.getCompletedReading?e.getCompletedReading():{},o=e.getCompletedListening?e.getCompletedListening():{},f=Object.keys(n).length,w=Object.keys(o).length,b=y.length?y[y.length-1]:null;return{grammar:{done:m,total:s.grammar},vocab:{done:d,total:s.vocab},kanji:{done:i,total:s.kanji},reading:{done:f,total:s.reading},listening:{done:w,total:s.listening},mockTest:b?{done:b.correct,total:b.total,percent:b.percent}:{done:0,total:0,percent:null,notAttempted:!0}}}function q(s){return[{idx:"01",id:"grammar",title:"Grammar",count:`${p(s.grammar)} patterns`,desc:"Basic sentence structure, particles, verb forms, adjectives, comparison, requests, and common N5 expressions.",href:"#/learn/grammar",action:"Open Grammar Syllabus"},{idx:"02",id:"vocab",title:"Vocabulary",count:`${p(s.vocab)} words`,desc:"Daily life words, time expressions, family, food, school, travel, verbs, adjectives, and common expressions.",href:"#/learn/vocab",action:"Open Vocabulary List"},{idx:"03",id:"kanji",title:"Kanji",count:`${p(s.kanji)} characters`,desc:"Numbers, time, people, school, directions, nature, common verbs, and basic recognition kanji.",href:"#/kanji",action:"Open Kanji List"},{idx:"04",id:"reading",title:"Reading",count:`${p(s.reading)} passages`,desc:"Short notices, simple messages, daily-life paragraphs, and basic comprehension practice.",href:"#/reading",action:"Start Reading Practice"},{idx:"05",id:"listening",title:"Listening",count:`${p(s.listening)} drills`,desc:"Greetings, classroom phrases, daily conversations, time, shopping, directions, and simple Q&A.",href:"#/listening",action:"Start Listening Practice"},{idx:"06",id:"test",title:"Mock Test",count:"15 questions",desc:"Auto-scored mock test with correct answers, explanations, and weak-area review.",href:"#/test",action:"Take Mock Test"}]}const B=[{text:"Learn basic sentence structure and particles",href:"#/learn/grammar"},{text:"Study core vocabulary",href:"#/learn/vocab"},{text:"Learn basic kanji recognition",href:"#/kanji"},{text:"Practice grammar questions",href:"#/drill"},{text:"Practice short reading passages",href:"#/reading"},{text:"Practice listening drills",href:"#/listening"},{text:"Take the mock test",href:"#/test"},{text:"Review weak areas",href:"#/review"}];function E(s){return`
  ...
  `}async function U(s){const t=e.getHistory(),u=e.getResults(),c=Object.keys(t).length>0||u.length>0,m=e.getSettings().lastLearnId||null,d=await G(),i=V(d),n=q(d),o=e.getStreak?e.getStreak():null,f=(()=>{const a=new Date;return`${a.getFullYear()}-${String(a.getMonth()+1).padStart(2,"0")}-${String(a.getDate()).padStart(2,"0")}`})(),w=c&&o&&o.lastStudyDate===f,b=e.getReviewsToday?e.getReviewsToday():0,$=e.getDailyGoal?e.getDailyGoal():20,T=Math.min(100,Math.round(100*b/$)),h=e.getDueCountsBySkill?e.getDueCountsBySkill():{grammar:e.getDueCount?e.getDueCount():0,vocab:0,kanji:0},x=h.grammar+h.vocab+h.kanji,C=h.vocab>0||h.kanji>0?`<span class="muted small" style="margin-left:6px;">(${h.grammar} grammar \xB7 ${h.vocab} vocab \xB7 ${h.kanji} kanji)</span>`:"",S=e.getReviewForecast?e.getReviewForecast(7):[],L=Math.max(1,...S.map(a=>a.count)),M=j&&j[m]||m,P=c&&m?`<a class="resume-strip" href="#/learn/${encodeURIComponent(m)}">Last session: ${l(M)}.</a>`:"",A=[{id:"n5-001",label:"\u3067\u3059\uFF0F\u301C\u307E\u3059",why:"How sentences end politely"},{id:"n5-002",label:"\u306F",why:"The topic marker"},{id:"n5-058",label:"Verb-\u307E\u3059",why:"Polite verb form"},{id:"n5-077",label:"\u3044-Adjectives",why:"Describing things"},{id:"n5-024",label:"\u304B",why:"Asking questions"}];let R="";try{if(c){const a=O(K({corpusCounts:d}));if(a){const g=_(),N=g==="hi"?a.label_hi:a.label_en,I=g==="hi"?a.why_hi:a.why_en;R=`
          <aside class="home-recommend" aria-labelledby="home-recommend-h">
            ...
            <a class="home-recommend-action" href="${l(a.href)}">
              <span class="home-recommend-label"><strong>${l(N)}</strong></span>
              <span class="home-recommend-meta muted small">${l(a.duration)} \xB7 ${l(a.rule_id)}</span>
            </a>
            <p class="home-recommend-why muted small">${l(I)}</p>
          </aside>
        `}}}catch(a){typeof console<"u"&&console.warn("[recommender] suppressed:",a)}const D=c?"":`
        ...
        ${A.map((a,g)=>`
          <li>
            <a href="#/learn/${encodeURIComponent(a.id)}" class="starter-pack-card">
              <span class="starter-pack-num">${g+1}</span>
        ...
        ${c?`
          <div class="syllabus-daily-status">
            ...
            <a class="syllabus-daily-progress" href="#/review" title="Open today's review queue">
              <span class="syllabus-daily-progress-label">${r("home.today_label")}: <strong>${b}</strong> / ${$}</span>
          ...
          ${B.map((a,g)=>`
            <li class="study-order-item">
              <a class="study-order-link" href="${a.href}">
                <span class="study-order-num" aria-hidden="true">${String(g+1).padStart(2,"0")}</span>
      ...
      ${c&&S.length?`
        ...
        <section class="syllabus-forecast" aria-label="Review forecast">
          ...
          <ol class="forecast-bar-chart">
            ${S.map(a=>{const g=a.count===0?4:Math.max(8,Math.round(56*a.count/L));return`
                <li class="forecast-bar">
                  <span class="forecast-bar-count">${a.count}</span>
                  <span class="forecast-bar-track" aria-hidden="true">
                    <span class="forecast-bar-fill" style="height:${g}px"></span>
                  </span>
  ...
  `}function l(s){return String(s??"").replace(/[&<>"']/g,t=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[t])}export{U as renderHome};
