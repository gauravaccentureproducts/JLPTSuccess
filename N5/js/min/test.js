import{t as H}from"./i18n.js";import{renderJa as v}from"./furigana.js";import{matchesAnswer as N}from"./normalize.js";import*as $ from"./storage.js";let n=null,w="setup",A=null,M=null,x=null,b=null,f=null;const G=60,k=60;async function E(){if(M)return M;const t=await fetch("data/questions.json");if(!t.ok)throw new Error(`Failed to load questions.json: ${t.status}`);return M=(await t.json()).questions||[],M}async function Q(){if(x)return x;const e=await(await fetch("data/grammar.json")).json();return x=new Map((e.patterns||[]).map(s=>[s.id,s])),x}async function rt(t,e){if(!e&&w==="results"&&(w="setup",n=null,A=null),w==="attempting"&&n)return I(t);if(w==="results"&&A)return R(t);if(e){const s=parseInt(decodeURIComponent(e),10);if([20,30,50].includes(s)){await E(),$.setSettings({lastTestLength:s}),j(s,t);return}}return L(t)}async function L(t){w="setup";const e=await E(),s=$.getSettings(),r=s.lastTestLength||20,c=($.getResults()||[]).length===0,i=!!s.examMode;let p="Mock-test papers";try{const a=await fetch("data/papers/manifest.json").then(o=>o.ok?o.json():null);a&&a.totalPapers&&a.totalQuestions&&(p=`${a.totalPapers} papers (${a.totalQuestions} questions)`)}catch{}t.innerHTML=`
    <h2>${H("page.test")}</h2>
    ${c?`
  ...
  `,document.getElementById("start-test").addEventListener("click",()=>{const a=parseInt(document.getElementById("test-length").value,10),o=!!document.getElementById("exam-mode")?.checked;$.setSettings({lastTestLength:a,examMode:o}),j(a,t,{examMode:o})})}function J(t,e){const s=e>=8?Math.ceil(e/5):1/0,r=new Map;for(const a of t)r.has(a.grammarPatternId)||r.set(a.grammarPatternId,[]),r.get(a.grammarPatternId).push(a);for(const a of r.values())_(a);const c=[],i=[...r.values()];_(i);let p=0;for(;c.length<e&&i.some(a=>a.length>0);){const a=i[p%i.length];if(a.length>0&&a.filter(o=>c.includes(o)).length<s){const o=a.shift();o&&c.push(o)}if(p++,p>e*50)break}return c.slice(0,Math.min(e,t.length))}function _(t){for(let e=t.length-1;e>0;e--){const s=Math.floor(Math.random()*(e+1));[t[e],t[s]]=[t[s],t[e]]}return t}async function j(t,e,s={}){const r=await E(),c=J(r,t),i=!!s.examMode;n={questions:c,answers:{},tileOrders:{},currentIdx:0,startedAt:new Date().toISOString(),examMode:i,durationSec:i?t*G:null},i?(f=Date.now()+n.durationSec*1e3,b&&clearInterval(b),b=setInterval(()=>U(e),1e3)):f=null,w="attempting",window.__testInProgress=!0,I(e)}function U(t){if(!f||w!=="attempting")return;const e=f-Date.now(),s=document.getElementById("test-timer");if(s){const r=Math.max(0,Math.ceil(e/1e3)),c=String(Math.floor(r/60)).padStart(2,"0"),i=String(r%60).padStart(2,"0");s.textContent=`${c}:${i}`,s.classList.toggle("danger",r<=60),s.classList.toggle("warning",r>60&&r<=300)}e<=0&&(clearInterval(b),b=null,f=null,D(t))}function I(t){const e=n.questions.length,s=n.questions[n.currentIdx],r=n.questions.filter(o=>!C(o)).length,c=r===0;let i="";s.type==="mcq"||s.type==="dropdown"?i=F(s):s.type==="sentence_order"?i=W(s):s.type==="text_input"?i=K(s):i=`<p class="placeholder-inline">Unsupported question type: ${m(s.type)}</p>`;let p="";if(n.examMode&&f){const o=Math.max(0,Math.ceil((f-Date.now())/1e3)),g=String(Math.floor(o/60)).padStart(2,"0"),y=String(o%60).padStart(2,"0");p=`<span id="test-timer" class="test-timer-chip${o<=60?" danger":o<=300?" warning":""}" aria-live="polite" title="Time remaining">${g}:${y}</span>`}t.innerHTML=`
  ...
  `,document.getElementById("prev-q")?.addEventListener("click",()=>B(n.currentIdx-1,t)),document.getElementById("next-q")?.addEventListener("click",()=>B(n.currentIdx+1,t)),document.getElementById("submit-test")?.addEventListener("click",()=>D(t)),t.querySelectorAll("[data-choice]").forEach(o=>{o.addEventListener("click",()=>{n.answers[s.id]=o.dataset.choice,I(t)})}),t.querySelectorAll("[data-tile-add]").forEach(o=>{o.addEventListener("click",()=>Y(s,o.dataset.tileAdd,t))}),t.querySelectorAll("[data-tile-remove]").forEach(o=>{o.addEventListener("click",()=>X(s,parseInt(o.dataset.tileRemove,10),t))});const a=t.querySelector("[data-text-input]");a&&(a.addEventListener("input",()=>{n.answers[s.id]=a.value;const o=n.questions.filter(T=>!C(T)).length,g=o===0,y=document.getElementById("submit-test");y&&(y.disabled=!g,y.textContent=g?"Submit":`Submit (${o} remaining)`,y.title=g?"Submit your test":`Answer all questions to submit (${o} remaining)`)}),typeof n.answers[s.id]=="string"&&(a.value=n.answers[s.id]))}function C(t){const e=n.answers[t.id];return t.type==="sentence_order"?Array.isArray(e)&&e.length===(t.tiles?.length||0):t.type==="text_input"?typeof e=="string"&&e.trim()!=="":e!=null&&e!==""}function B(t,e){t<0||t>=n.questions.length||(n.currentIdx=t,I(e))}function F(t){const e=n.answers[t.id];return`<div class="choice-grid">${(t.choices||[]).map(r=>`
    <button type="button" data-choice="${m(r)}" class="choice-button ${e===r?"selected":""}">
      ${v(r)}
    </button>
  `).join("")}</div>`}function K(t){const e=typeof n.answers[t.id]=="string"?n.answers[t.id]:"";return`
  ...
  `}function W(t){const e=n.answers[t.id]||[],s=(t.tiles||[]).filter(i=>!e.includes(i)),r=e.length?e.map((i,p)=>`
  ...
  `}function Y(t,e,s){n.answers[t.id]||(n.answers[t.id]=[]),!n.answers[t.id].includes(e)&&(n.answers[t.id].push(e),I(s))}function X(t,e,s){Array.isArray(n.answers[t.id])&&(n.answers[t.id].splice(e,1),n.answers[t.id].length===0&&delete n.answers[t.id],I(s))}function V(t,e){if(t.type==="sentence_order"){if(!Array.isArray(e))return!1;const s=t.correctOrder||[];return e.length!==s.length?!1:e.every((r,c)=>r===s[c])}if(t.type==="text_input"){const s=t.acceptedAnswers||[t.correctAnswer];return N(e,s)}return e===t.correctAnswer}function D(t){b&&(clearInterval(b),b=null);const e=n.startedAt?Math.round((Date.now()-new Date(n.startedAt).getTime())/1e3):null,s=!!(n.examMode&&f&&Date.now()>=f);f=null;const r=n.questions.map(a=>{const o=n.answers[a.id];return{questionId:a.id,grammarPatternId:a.grammarPatternId,type:a.type,userAnswer:o,correctAnswer:a.correctAnswer??a.correctOrder,isCorrect:V(a,o)}}),c=r.filter(a=>a.isCorrect).length,i=r.length,p={timestamp:new Date().toISOString(),type:"test",total:i,correct:c,incorrect:i-c,percent:i>0?Math.round(c/i*100):0,examMode:n.examMode||!1,elapsedSec:e,timedOut:s,responses:r};$.recordTestResponses(r),$.recordTestResult(p),A={result:p,questions:n.questions},w="results",window.__testInProgress=!1,R(t)}async function R(t){const{result:e,questions:s}=A;await Q();const r=e.responses.map(d=>{const l=s.find(u=>u.id===d.questionId);return Z(l,d)}).join(""),i=tt(e.responses).map(d=>{const l=x.get(d),u=l?l.pattern:d;return`<li><a href="#/review">${m(u)}</a></li>`}).join(""),p=e.percent>=k,a=`
  ...
  `;let o="";if(typeof e.elapsedSec=="number"){const d=Math.floor(e.elapsedSec/60),l=String(e.elapsedSec%60).padStart(2,"0"),u=e.timedOut?" (auto-submitted at zero)":"";o=`<span class="score-time muted small">Time: ${d}m ${l}s${u}</span>`}const g=new Map;for(const d of e.responses){const l=x.get(d.grammarPatternId),u=l&&l.category||"Other";g.has(u)||g.set(u,{correct:0,total:0});const h=g.get(u);h.total+=1,d.isCorrect&&(h.correct+=1)}const y=[...g.entries()].sort((d,l)=>d[1].correct/d[1].total-l[1].correct/l[1].total).map(([d,{correct:l,total:u}])=>{const h=u>0?Math.round(l/u*100):0;return`
      ...
      `}).join(""),T={mcq:"Multiple choice",sentence_order:"Sentence ordering",text_input:"Text input",dropdown:"Dropdown"},S=new Map;for(const d of e.responses){const l=d.type||"mcq";S.has(l)||S.set(l,{correct:0,total:0});const u=S.get(l);u.total+=1,d.isCorrect&&(u.correct+=1)}const O=[...S.entries()].sort((d,l)=>d[1].correct/d[1].total-l[1].correct/l[1].total).map(([d,{correct:l,total:u}])=>{const h=u>0?Math.round(l/u*100):0,P=h>=k?"pass":"fail",z=T[d]||d;return`
        <tr class="${P}">
  ...
  `,document.getElementById("new-test")?.addEventListener("click",()=>{n=null,A=null,w="setup",L(t)}),document.getElementById("back-to-learn")?.addEventListener("click",()=>{location.hash="#/learn"})}function Z(t,e){const s=e.isCorrect?"\u2713":"\u2717",r=e.isCorrect?"correct":"incorrect",c=q(t,e.userAnswer),i=q(t,e.correctAnswer),p=!e.isCorrect&&t.distractor_explanations&&typeof e.userAnswer=="string"?t.distractor_explanations[e.userAnswer]:null,a=x?.get(t.grammarPatternId),o=a?a.pattern:t.grammarPatternId;return`
    <li class="review-item ${r}">
      ...
      <div class="review-body">
        ...
        ${p?`<p class="distractor-explanation"><em>Why your choice was wrong:</em> ${m(p)}</p>`:""}
        <p class="review-pattern">Pattern: <a href="#/learn/${encodeURIComponent(t.grammarPatternId)}">${m(o)}</a></p>
      </div>
    </li>
  `}function q(t,e){return t.type==="sentence_order"&&Array.isArray(e)?v(e.join(" ")):v(String(e??"-"))}function tt(t){return[...new Set($.getWeakPatternIds())]}function m(t){return String(t??"").replace(/[&<>"']/g,e=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[e])}export{rt as renderTest};
