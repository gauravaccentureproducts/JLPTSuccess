import"./storage.js";import{t as h}from"./i18n.js";import{loadPacing as j,renderPacingChip as T}from"./mondai-pacing.js";import{estimate as v}from"./score-estimator.js";import{openStrategyModal as w}from"./strategy-modal.js";import{navigateTo as S}from"./router.js";const f=[["mojigoi","Moji + Goi","\u6587\u5B57\u30FB\u8A9E\u5F59",["moji","goi"],25],["bunpoudok","Bunpou + Dokkai","\u6587\u6CD5\u30FB\u8AAD\u89E3",["bunpou","dokkai"],50],["choukai","Listening","\u8074\u89E3",["listening"],30]],L=60;let l=null,m=null,k=null;function o(a){return String(a??"").replace(/[&<>"']/g,i=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[i])}async function M(a,i){const n=await fetch(`data/papers/${a}/paper-${i}.json`);return n.ok?n.json():null}async function I(){const a=await fetch("data/listening.json");return a.ok?a.json():{items:[]}}function _(a){const i=Math.floor(a/60),n=String(a%60).padStart(2,"0");return`${String(i).padStart(2,"0")}:${n}`}function E(a,i,n){k=Date.now()+a*1e3,m&&clearInterval(m),m=setInterval(()=>{const t=Math.max(0,Math.ceil((k-Date.now())/1e3));i(t),t<=0&&(clearInterval(m),m=null,n())},1e3),i(a)}async function q(a,i){const n=(i||"").split("/").filter(Boolean);if(n.length===0)return P(a);const t=parseInt(n[0],10);if(!Number.isFinite(t)||t<1||t>7){a.innerHTML=`<p>${o(h("meta.bad_paper"))} <a href="#/sitting">${o(h("meta.pick_again"))}</a></p>`;return}if(n[1]==="result")return A(a,t);const c=n[1]?parseInt(n[1],10):0;if(!Number.isFinite(c)||c<0||c>=f.length){a.innerHTML=`<p>${o(h("meta.bad_section"))} <a href="#/sitting">${o(h("meta.pick_again"))}</a></p>`;return}return B(a,t,c)}function P(a){a.innerHTML=`
  ...
  `}async function B(a,i,n){const[,t,c,p,g]=f[n];(!l||l.paperNumber!==i)&&(l={paperNumber:i,currentSection:n,startedAt:new Date().toISOString(),sectionResults:[],answers:{}});let u=[];if(p[0]==="listening")u=((await I()).items||[]).slice(0,12).map(s=>({id:s.id,stem_html:s.title_ja||s.id,audio:s.audio,script_ja:s.script_ja,prompt_ja:s.prompt_ja,choices:s.choices||[],correctIndex:(s.choices||[]).findIndex(d=>d===s.correctAnswer),kind:"listening",mondai:s.mondai}));else for(const e of p){const s=await M(e,i);s&&u.push(...s.questions.map(d=>({...d,kind:e})))}if(u.length===0){a.innerHTML=`<p>No questions for section ${n}. <a href="#/sitting">Restart.</a></p>`;return}await j();const $=()=>{let e=0;for(const s of u){const d=l.answers[s.id];typeof d=="number"&&d===s.correctIndex&&(e+=1)}l.sectionResults[n]={label:t,jaLabel:c,total:u.length,correct:e,durationSec:g*60},n+1<f.length?N(a,i,n+1):S(`sitting/${i}/result`)},b=u.length;a.innerHTML=`
    <article class="sitting-section">
      <header class="sitting-section-header">
        ...
        <p class="sitting-timer-chip" id="sitting-timer" aria-live="polite">${_(g*60)}</p>
      ...
      <form id="sitting-form" class="sitting-form">
        ${u.map((e,s)=>`
          <fieldset class="sitting-question" id="sq-${o(e.id)}">
            <legend>Q${s+1} ${T(e.kind,e.mondai)}</legend>
  ...
  `;const r=document.getElementById("sitting-strategy-btn");r&&r.addEventListener("click",()=>{const e=r.dataset.section,s=r.dataset.label;w(e,s)}),document.getElementById("sitting-form").addEventListener("submit",e=>{e.preventDefault(),m&&clearInterval(m),m=null;for(const s of u){const d=document.querySelector(`input[name="${s.id}"]:checked`);d&&(l.answers[s.id]=parseInt(d.value,10))}$()}),document.getElementById("sitting-form").addEventListener("change",e=>{if(e.target.tagName==="INPUT"){const s=u.find(d=>d.id===e.target.name);s&&(l.answers[s.id]=parseInt(e.target.value,10))}}),E(g*60,e=>{const s=document.getElementById("sitting-timer");s&&(s.textContent=_(e)),e<=60&&s&&s.classList.add("danger")},()=>{for(const e of u){const s=document.querySelector(`input[name="${e.id}"]:checked`);s&&(l.answers[e.id]=parseInt(s.value,10))}$()})}function N(a,i,n){const[,t,c]=f[n];let p=L;a.innerHTML=`
  ...
  `,m&&clearInterval(m),m=setInterval(()=>{p-=1;const g=document.getElementById("break-countdown");g&&(g.textContent=String(p)),p<=0&&(clearInterval(m),m=null,S(`sitting/${i}/${n}`))},1e3)}function A(a,i){if(!l||!l.sectionResults||l.sectionResults.length<f.length){a.innerHTML='<p>No completed sitting in memory. <a href="#/sitting">Start again.</a></p>';return}let n=0,t=0;for(const r of l.sectionResults)n+=r.correct,t+=r.total;const c=t>0?Math.round(100*n/t):0,p=60,g=[63,61,79],u="~19 / section",$=l.sectionResults.map((r,e)=>(r.total?100*r.correct/r.total:0)>=g[e]),b=$.every(Boolean);a.innerHTML=`
  ...
  `,l=null}function H(a){if(!a||a.length<3)return"";const i={correct:(a[0]?.correct||0)+(a[1]?.correct||0),total:(a[0]?.total||0)+(a[1]?.total||0)},n={correct:a[2]?.correct||0,total:a[2]?.total||0},t=v(i,n);return`
