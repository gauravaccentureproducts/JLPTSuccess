import{renderJa as q}from"./furigana.js";let f=null;const y=new Map;async function x(){if(f)return f;const n=await fetch("data/papers/manifest.json");if(!n.ok)throw new Error(`Failed to load papers manifest: ${n.status}`);return f=await n.json(),f}async function j(n,t){const a=`${n}-${t}`;if(y.has(a))return y.get(a);const i=await fetch(`data/papers/${n}/paper-${t}.json`);if(!i.ok)throw new Error(`Failed to load paper ${a}: ${i.status}`);const e=await i.json();return y.set(a,e),e}function w(n){const t=String(n).match(/^([a-z]+)-(\d+)$/);return t?{category:t[1],number:parseInt(t[2],10)}:null}function M(n,t){return(n.combined_sections||[]).find(a=>a.id===t)}function T(n,t){return(n.full_mock_papers||[]).find(a=>a.id===t)}function v(n,t){return(n.virtual_papers||[]).find(a=>a.id===t)}async function K(n,t){const a=(t||"").trim();if(!a)return h(n);const[i,e]=a.split("?"),r=decodeURIComponent(i),l=e&&/(?:^|&)key=1\b/.test(e)||!1;let o;try{o=await x()}catch(c){n.innerHTML=`<article class="print-paper-error"><p>Could not load papers manifest: ${s(String(c))}</p></article>`;return}if(/^full-mock-\d+$/.test(r))return E(n,o,r,l);if(/^genngo-chishiki-(moji-goi|bunpou-dokkai)-\d+$/.test(r))return I(n,o,r,l);if(/^chokai-\d+(?:-virtual)?$/.test(r))return Q(n,o,r,l);const p=w(r);return p&&["moji","goi","bunpou","dokkai"].includes(p.category)?D(n,o,p,l):h(n,r)}async function h(n,t){const a=await x(),i=[];(a.categories||[]).forEach(e=>{e.papers.forEach(r=>{i.push(g(r.id,r.name,`${r.questionCount} Q \xB7 ${e.label_ja}`))})}),(a.combined_sections||[]).forEach(e=>{i.push(g(e.id,e.name_en,`${e.questionCount} Q \xB7 ${s(e.sectionLabel)}`))}),(a.full_mock_papers||[]).forEach(e=>{i.push(g(e.id,e.name_en,`${e.totalQuestions} Q \xB7 ${e.totalDurationMin} min \xB7 full mock`))}),(a.virtual_papers||[]).forEach(e=>{i.push(g(e.id,e.name,`${e.questionCount} Q \xB7 ${e.expectedDurationMin} min \xB7 listening`))}),n.innerHTML=`
  ...
  `}async function D(n,t,a,i){const{category:e,number:r}=a;let l;try{l=await j(e,r)}catch(u){n.innerHTML=`<article class="print-paper-error"><p>Could not load <code>${s(e)}-${r}</code>: ${s(String(u))}</p><p><a href="#/print">Back to print index.</a></p></article>`;return}const o=(t.categories||[]).find(u=>u.id===e)||{label:e,label_ja:""},p=(o.papers||[]).find(u=>u.id===l.id)||{},c=[{label_ja:o.label_ja||o.label||e,label_en:o.label||e,questions:l.questions,duration_min:p.expectedDurationMin||null}],_={name_en:l.name||`${o.label} Paper ${r}`,name_ja:`${o.label_ja||""} Paper ${r}`,total_q:l.questions.length,duration_min:p.expectedDurationMin||null,instructions:k(e)};return $(n,l.id,_,c,i)}async function I(n,t,a,i){const e=M(t,a);if(!e)return h(n,a);const l=(await Promise.all(e.componentPapers.map(async p=>{const c=w(p);if(!c)return null;const _=await j(c.category,c.number),u=(t.categories||[]).find(d=>d.id===c.category)||{label:c.category,label_ja:""},m=(u.papers||[]).find(d=>d.id===p)||{};return{label_ja:u.label_ja,label_en:u.label,questions:_.questions,duration_min:m.expectedDurationMin||null}}))).filter(Boolean),o={name_en:e.name_en,name_ja:e.name_ja,total_q:e.questionCount,duration_min:e.expectedDurationMin,instructions:k("combined")};return $(n,e.id,o,l,i)}async function E(n,t,a,i){const e=T(t,a);if(!e)return h(n,a);const r=[];for(const o of e.sections)if(/^genngo-chishiki/.test(o)){const p=M(t,o);if(!p)continue;for(const c of p.componentPapers){const _=w(c);if(!_)continue;const u=await j(_.category,_.number),m=(t.categories||[]).find(d=>d.id===_.category)||{label:_.category,label_ja:""};r.push({label_ja:m.label_ja,label_en:m.label,questions:u.questions,duration_min:null})}}else if(/^chokai-\d+/.test(o)){const p=v(t,o.replace(/-virtual$/,""));p&&r.push({label_ja:"\u8074\u89E3",label_en:"Listening",questions:C(p),duration_min:p.expectedDurationMin||null,is_chokai:!0,source_listening_ids:p.source_listening_ids||[]})}const l={name_en:e.name_en,name_ja:e.name_ja,total_q:e.totalQuestions,duration_min:e.totalDurationMin,instructions:k("full-mock")};return $(n,e.id,l,r,i)}async function Q(n,t,a,i){const e=a.replace(/-virtual$/,""),r=v(t,e);if(!r)return h(n,a);const l={name_en:`${r.name} (script)`,name_ja:r.label_ja,total_q:r.questionCount,duration_min:r.expectedDurationMin||null,instructions:k("chokai")},o=[{label_ja:"\u8074\u89E3",label_en:"Listening",questions:C(r),duration_min:r.expectedDurationMin||null,is_chokai:!0,source_listening_ids:r.source_listening_ids||[]}];return $(n,r.id,l,o,i)}let b=null;async function F(){if(b)return b;const n=await fetch("data/listening.json");return b=n.ok?await n.json():{items:[]},b}function C(n){return n.source_listening_ids.map((t,a)=>({_chokai_listen_id:t,id:`${n.id}-${a+1}`,mondai:null,type:"mcq",stem_html:"",choices:[],correctIndex:0,rationale:""}))}async function $(n,t,a,i,e){if(i.some(c=>c.is_chokai)){const c=await F(),_=new Map((c.items||[]).map(u=>[u.id,u]));i.forEach(u=>{u.is_chokai&&(u.questions=u.questions.map(m=>{const d=_.get(m._chokai_listen_id);if(!d)return{...m,stem_html:"(listening item not found: "+s(m._chokai_listen_id)+")"};const P=(d.choices||[]).findIndex(S=>S===d.correctAnswer);return{...m,mondai:d.mondai||null,stem_html:U(d),choices:d.choices||[],correctIndex:P>=0?P:0,rationale:d.explanation_ja||d.explanation_en||""}}))})}const l=i.map(c=>{const u=J(c.questions).map(m=>`
  ...
  `;try{typeof q=="function"&&q(n)}catch{}const p=n.querySelector("#print-paper-now-btn");p&&p.addEventListener("click",()=>window.print())}function H(n){return`
  ...
  `}function U(n){const t=n.prompt_ja||"",a=n.script_ja||"";return`
  ...
  `}function L(n){return["","\u2460","\u2461","\u2462","\u2463","\u2464","\u2465","\u2466","\u2467","\u2468"][n]||String(n)}function s(n){return String(n??"").replace(/[&<>"']/g,t=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[t])}export{K as renderPrint};
