import{renderJa as v}from"./furigana.js";import{getBranding as P}from"./branding.js";let f=null;const y=new Map;async function C(){if(f)return f;const n=await fetch("data/papers/manifest.json");if(!n.ok)throw new Error(`Failed to load papers manifest: ${n.status}`);return f=await n.json(),f}async function w(n,t){const a=`${n}-${t}`;if(y.has(a))return y.get(a);const i=await fetch(`data/papers/${n}/paper-${t}.json`);if(!i.ok)throw new Error(`Failed to load paper ${a}: ${i.status}`);const e=await i.json();return y.set(a,e),e}function j(n){const t=String(n).match(/^([a-z]+)-(\d+)$/);return t?{category:t[1],number:parseInt(t[2],10)}:null}function S(n,t){return(n.combined_sections||[]).find(a=>a.id===t)}function D(n,t){return(n.full_mock_papers||[]).find(a=>a.id===t)}function q(n,t){return(n.virtual_papers||[]).find(a=>a.id===t)}async function W(n,t){const a=(t||"").trim();if(!a)return h(n);const[i,e]=a.split("?"),r=decodeURIComponent(i),l=e&&/(?:^|&)key=1\b/.test(e)||!1;let o;try{o=await C()}catch(c){n.innerHTML=`<article class="print-paper-error"><p>Could not load papers manifest: ${s(String(c))}</p></article>`;return}if(/^full-mock-\d+$/.test(r))return H(n,o,r,l);if(/^genngo-chishiki-(moji-goi|bunpou-dokkai)-\d+$/.test(r))return I(n,o,r,l);if(/^chokai-\d+(?:-virtual)?$/.test(r))return J(n,o,r,l);const p=j(r);return p&&["moji","goi","bunpou","dokkai"].includes(p.category)?E(n,o,p,l):h(n,r)}async function h(n,t){const a=await C(),i=[];(a.categories||[]).forEach(e=>{e.papers.forEach(r=>{i.push(g(r.id,r.name,`${r.questionCount} Q \xB7 ${e.label_ja}`))})}),(a.combined_sections||[]).forEach(e=>{i.push(g(e.id,e.name_en,`${e.questionCount} Q \xB7 ${s(e.sectionLabel)}`))}),(a.full_mock_papers||[]).forEach(e=>{i.push(g(e.id,e.name_en,`${e.totalQuestions} Q \xB7 ${e.totalDurationMin} min \xB7 full mock`))}),(a.virtual_papers||[]).forEach(e=>{i.push(g(e.id,e.name,`${e.questionCount} Q \xB7 ${e.expectedDurationMin} min \xB7 listening`))}),n.innerHTML=`
  ...
  `}async function E(n,t,a,i){const{category:e,number:r}=a;let l;try{l=await w(e,r)}catch(u){n.innerHTML=`<article class="print-paper-error"><p>Could not load <code>${s(e)}-${r}</code>: ${s(String(u))}</p><p><a href="#/print">Back to print index.</a></p></article>`;return}const o=(t.categories||[]).find(u=>u.id===e)||{label:e,label_ja:""},p=(o.papers||[]).find(u=>u.id===l.id)||{},c=[{label_ja:o.label_ja||o.label||e,label_en:o.label||e,questions:l.questions,duration_min:p.expectedDurationMin||null}],_={name_en:l.name||`${o.label} Paper ${r}`,name_ja:`${o.label_ja||""} Paper ${r}`,total_q:l.questions.length,duration_min:p.expectedDurationMin||null,instructions:$(e)};return k(n,l.id,_,c,i)}async function I(n,t,a,i){const e=S(t,a);if(!e)return h(n,a);const l=(await Promise.all(e.componentPapers.map(async p=>{const c=j(p);if(!c)return null;const _=await w(c.category,c.number),u=(t.categories||[]).find(d=>d.id===c.category)||{label:c.category,label_ja:""},m=(u.papers||[]).find(d=>d.id===p)||{};return{label_ja:u.label_ja,label_en:u.label,questions:_.questions,duration_min:m.expectedDurationMin||null}}))).filter(Boolean),o={name_en:e.name_en,name_ja:e.name_ja,total_q:e.questionCount,duration_min:e.expectedDurationMin,instructions:$("combined")};return k(n,e.id,o,l,i)}async function H(n,t,a,i){const e=D(t,a);if(!e)return h(n,a);const r=[];for(const o of e.sections)if(/^genngo-chishiki/.test(o)){const p=S(t,o);if(!p)continue;for(const c of p.componentPapers){const _=j(c);if(!_)continue;const u=await w(_.category,_.number),m=(t.categories||[]).find(d=>d.id===_.category)||{label:_.category,label_ja:""};r.push({label_ja:m.label_ja,label_en:m.label,questions:u.questions,duration_min:null})}}else if(/^chokai-\d+/.test(o)){const p=q(t,o.replace(/-virtual$/,""));p&&r.push({label_ja:"\u8074\u89E3",label_en:"Listening",questions:M(p),duration_min:p.expectedDurationMin||null,is_chokai:!0,source_listening_ids:p.source_listening_ids||[]})}const l={name_en:e.name_en,name_ja:e.name_ja,total_q:e.totalQuestions,duration_min:e.totalDurationMin,instructions:$("full-mock")};return k(n,e.id,l,r,i)}async function J(n,t,a,i){const e=a.replace(/-virtual$/,""),r=q(t,e);if(!r)return h(n,a);const l={name_en:`${r.name} (script)`,name_ja:r.label_ja,total_q:r.questionCount,duration_min:r.expectedDurationMin||null,instructions:$("chokai")},o=[{label_ja:"\u8074\u89E3",label_en:"Listening",questions:M(r),duration_min:r.expectedDurationMin||null,is_chokai:!0,source_listening_ids:r.source_listening_ids||[]}];return k(n,r.id,l,o,i)}let b=null;async function Q(){if(b)return b;const n=await fetch("data/listening.json");return b=n.ok?await n.json():{items:[]},b}function M(n){return n.source_listening_ids.map((t,a)=>({_chokai_listen_id:t,id:`${n.id}-${a+1}`,mondai:null,type:"mcq",stem_html:"",choices:[],correctIndex:0,rationale:""}))}async function k(n,t,a,i,e){if(i.some(c=>c.is_chokai)){const c=await Q(),_=new Map((c.items||[]).map(u=>[u.id,u]));i.forEach(u=>{u.is_chokai&&(u.questions=u.questions.map(m=>{const d=_.get(m._chokai_listen_id);if(!d)return{...m,stem_html:"(listening item not found: "+s(m._chokai_listen_id)+")"};const x=(d.choices||[]).findIndex(T=>T===d.correctAnswer);return{...m,mondai:d.mondai||null,stem_html:R(d),choices:d.choices||[],correctIndex:x>=0?x:0,rationale:d.explanation_ja||d.explanation_en||""}}))})}const l=i.map(c=>{const u=A(c.questions).map(m=>`
      <section class="print-mondai">
        ...
        <ol class="print-q-list">
          ${m.questions.map(d=>O(d,c.is_chokai)).join("")}
        </ol>
      </section>
    `).join("");return`
        ...
        <defs>
          <pattern id="jlpts-watermark" x="0" y="0" width="280" height="160" patternUnits="userSpaceOnUse">
            <text x="140" y="86"
                  ...
                  letter-spacing="2"
                  text-anchor="middle"
                  transform="rotate(-28 140 86)">${s(t)}</text>
          </pattern>
        </defs>
  ...
  `}function R(n){const t=n.prompt_ja||"",a=n.script_ja||"";return`
  ...
  `}function V(n){return`
    <section class="print-key" aria-labelledby="print-key-h">
      ...
      ${n.map(a=>{const i=a.questions.map(e=>{const r=L((e.correctIndex||0)+1),l=(e.choices||[])[e.correctIndex||0]||"",o=(e.rationale||"").trim();return`
        <li>
  ...
  `}function L(n){return["","\u2460","\u2461","\u2462","\u2463","\u2464","\u2465","\u2466","\u2467","\u2468"][n]||String(n)}function s(n){return String(n??"").replace(/[&<>"']/g,t=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[t])}export{W as renderPrint};
