import{renderJa as b}from"./furigana.js";import*as $ from"./storage.js";import{t as _}from"./i18n.js";import{attachRomajiKanaAll as E}from"./romaji-kana.js";import{navigateTo as P}from"./router.js";let w=null,h=null,d=null,g="setup";async function S(){if(w)return w;const e=await fetch("data/questions.json");if(!e.ok)throw new Error(`Failed to load questions.json: ${e.status}`);return w=(await e.json()).questions||[],w}async function L(){if(h)return h;const t=await(await fetch("data/grammar.json")).json();return h=new Map((t.patterns||[]).map(n=>[n.id,n])),h}async function J(e){return g==="finished"&&(g="setup",d=null),g==="drilling"&&d?m(e):g==="finished"&&d?j(e):A(e)}async function A(e){g="setup",await Promise.all([S(),L()]);const t=$.getDuePatternIds(),n=Object.entries($.getHistory()),r=n.filter(([,a])=>a.srsBox&&a.srsBox!=="graduated").length,s=n.filter(([,a])=>a.srsBox==="graduated").length;if(t.length===0){e.innerHTML=`
      <h2>${_("page.drill")}</h2>
    ...
    `;return}const o=T(t,w,10);if(o.length===0){e.innerHTML=`
      <h2>${_("page.drill")}</h2>
      <div class="placeholder">
  ...
  `,document.getElementById("drill-again")?.addEventListener("click",()=>{d=null,g="setup",A(e)}),document.getElementById("drill-back")?.addEventListener("click",()=>{P("learn")})}function f(e){return String(e??"").replace(/[&<>"']/g,t=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[t])}export{J as renderDrill};
