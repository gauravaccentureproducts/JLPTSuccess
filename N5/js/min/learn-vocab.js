import{renderJa as V}from"./furigana.js";import*as _ from"./storage.js";import{esc as t,wireExpandCollapseControls as x}from"./learn.js";import{currentLocale as $}from"./i18n.js";import{renderItemBadge as M}from"./provenance-badge.js";function T(s,o){if(!s||!Number.isFinite(s.mora))return"";const i=s.mora,r=s.drop;let c="";for(let e=1;e<=i;e++)r===0?c+=e===1?"L":"H":r===1?c+=e===1?"H":"L":c+=e===1?"L":e<=r?"H":"L";return c}function E(s){return{satsu:"\u3055\u3064",dai:"\u3060\u3044",hiki:"\u3072\u304D",wa:"\u308F",mai:"\u307E\u3044",ken:"\u3051\u3093",hon:"\u307B\u3093",soku:"\u305D\u304F",ko:"\u3053",nin:"\u306B\u3093",tsu:"\u3064",kai:"\u304B\u3044",do:"\u3069",fun:"\u3075\u3093",ji:"\u3058"}[s]||s}function j(s){const o=$();if(o&&o!=="en"){const i=s[`gloss_${o}`];if(typeof i=="string"&&i.trim())return i}return s.gloss||""}const y=[["People and Body",["1. People - Pronouns and Self","2. People - Family","3. People - Roles","4. Body Parts"]],["Demonstratives, Questions, Numbers, Time",["5. Demonstratives","6. Question Words","7. Numbers","8. Native Counters (\u3064-series)","9. Counters (Common)","10. Time - General","11. Time - Days, Weeks, Months, Years","12. Time - Frequency / Sequence"]],["Places and Things",["13. Locations and Places (general)","14. Nature and Weather","15. Animals","16. Food and Drink - General","17. Food - Items","18. Drinks","19. Tableware and Cooking","20. Colors","21. Clothing and Accessories","22. Money and Shopping","23. Transport","24. School and Study","25. Languages and Countries","26. House and Furniture"]],["Verbs",["27. Verbs - Group 1 (\u3046-verbs)","28. Verbs - Group 2 (\u308B-verbs)","29. Verbs - Irregular and \u3059\u308B-verbs","30. Verbs - Existence and Possession"]],["Adjectives and Function Words",["31. \u3044-Adjectives","32. \u306A-Adjectives","33. Adverbs","34. Conjunctions","35. Particles (functional vocabulary)","36. Greetings and Set Phrases"]],["Misc",["37. Common Nouns - Miscellaneous","38. Sounds and Voice","39. Function / Filler Expressions","40. Misc Useful Items"]]];function S(s){for(const[o,i]of y)if(i.includes(s))return o;return"Misc"}function L(s){const o=new Map;for(const[r]of y)o.set(r,[]);for(const r of s){const c=S(r.section||"Other");o.get(c).push(r)}const i=[];for(const[,r]of o.entries())r.sort((c,e)=>{const p=parseInt(c.section||"",10),l=parseInt(e.section||"",10);return!isNaN(p)&&!isNaN(l)&&p!==l?p-l:(c.form||"").localeCompare(e.form||"")}),i.push(...r);return i}let k="";function F(s,o){return o?[s.form||"",s.reading||"",s.gloss||"",s.section||""].join(" ").toLowerCase().includes(o):!0}function N(s,o){const i=o.entries||[],r=k.trim().toLowerCase(),e=L(i).filter(n=>F(n,r)),p=new Map;for(const[n]of y)p.set(n,[]);for(const n of e){const a=S(n.section||"Other");p.get(a).push(n)}const l=n=>n.toLowerCase().replace(/[^a-z0-9]+/g,"-").replace(/^-|-$/g,""),h=r.length>0,d=$(),f=d&&d!=="en",m=n=>{if(!f)return"";const a=n.length;if(a===0)return"";const g=n.filter(C=>typeof C[`gloss_${d}`]=="string"&&C[`gloss_${d}`].trim()).length,u=Math.round(100*g/a);return`<span class="vocab-coverage-badge tone-${u>=50?"good":u>0?"partial":"none"}" title="${g}/${a} translated">${u}%</span>`},b=[...p.entries()].filter(([,n])=>n.length>0).map(([n,a])=>{const g=a.map(u=>`
        <a class="vocab-card" href="#/learn/vocab/${encodeURIComponent(u.form||"")}">
          <span class="vocab-form" lang="ja">${t(u.form||"")}</span>
        </a>
      `).join("");return`
        <details class="vocab-section" id="vocab-${l(n)}"${h?" open":""}>
          <summary><strong>${t(n)}</strong> <span class="muted small">(${a.length})</span> ${m(a)}</summary>
    ...
    <article class="vocab-toc">
      ...
      <h2>Vocabulary</h2>
      <p class="page-lede">${i.length} N5 words in ${y.length} sections.</p>
      <div class="kanji-filters" role="search" aria-label="Filter vocabulary">
        ...
        <p class="kanji-filter-count muted small" aria-live="polite">
          Showing <strong>${e.length}</strong> of ${i.length}.
        </p>
      </div>
  ...
  `,x(s,"details.vocab-section");const v=document.getElementById("vocab-filter-q");v&&v.addEventListener("input",()=>{k=v.value,N(s,o);const n=document.getElementById("vocab-filter-q");if(n){n.focus();const a=n.value;n.setSelectionRange(a.length,a.length)}})}function D(s,o,i,r){const c=o.entries||[],e=c.find(n=>n.form===r);if(!e){s.innerHTML=`
    ...
    `;return}const p=new Set,l=[];for(const n of i.patterns||[]){for(const a of n.examples||[]){if(!a.ja||a.ja.includes("(see ")||p.has(a.ja))continue;let g=!1;if(Array.isArray(a.vocab_ids))g=a.vocab_ids.includes(e.id);else{const u=[r];e.reading&&e.reading!==r&&u.push(e.reading),g=u.some(w=>a.ja.includes(w))}if(g&&(p.add(a.ja),l.push({ja:a.ja,en:a.translation_en,source:n.pattern}),l.length>=24))break}if(l.length>=24)break}l.sort((n,a)=>(n.ja?.length||0)-(a.ja?.length||0));const h=l.slice(0,5),d=L(c),f=d.findIndex(n=>n.id===e.id),m=f>0?d[f-1]:null,b=f>=0&&f<d.length-1?d[f+1]:null,v=_.isVocabKnown(e.form);s.innerHTML=`
    <article class="vocab-detail">
      <a class="back-link" href="#/learn/vocab">\u2190 Back to Vocabulary</a>
      ...
      <section>
        ...
        ${h.length?`
          <ol class="example-list">
            ${h.map(n=>`
              <li>
                <p lang="ja" class="example-ja">${V(n.ja)}</p>
  ...
  `,document.getElementById("mark-known-vocab")?.addEventListener("change",n=>{_.setVocabKnown(e.form,n.target.checked)})}export{D as renderVocabularyDetail,N as renderVocabularyList};
