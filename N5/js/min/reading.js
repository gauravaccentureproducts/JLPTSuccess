import{renderJa as i}from"./furigana.js";import*as p from"./storage.js";import{t as g}from"./i18n.js";import{navigateTo as _,assetUrl as v}from"./router.js";const b={easy:"\u3084\u3055\u3057\u3044",medium:"\u3075\u3064\u3046","info-search":"\u3058\u3087\u3046\u307B\u3046\u3051\u3093\u3055\u304F"},j={"self-introduction":"\u3058\u3053\u3057\u3087\u3046\u304B\u3044","daily routine":"\u307E\u3044\u306B\u3061\u306E \u305B\u3044\u304B\u3064","weekend plan":"\u3057\u3085\u3046\u307E\u3064\u306E \u3088\u3066\u3044",weekend:"\u3057\u3085\u3046\u307E\u3064",shopping:"\u304B\u3044\u3082\u306E",family:"\u304B\u305E\u304F",weather:"\u3066\u3093\u304D",schedule:"\u3088\u3066\u3044",transport:"\u3053\u3046\u3064\u3046",hobby:"\u3057\u3085\u307F",school:"\u5B66\u6821",food:"\u305F\u3079\u3082\u306E",travel:"\u308A\u3087\u3053\u3046",health:"\u3051\u3093\u3053\u3046",study:"\u3079\u3093\u304D\u3087\u3046",people:"\u3072\u3068",request:"\u304A\u306D\u304C\u3044",room:"\u3078\u3084",directions:"\u307F\u3061\u3042\u3093\u306A\u3044"};let $=null,d=null;async function A(){return $||($=await(await fetch("data/reading.json")).json(),$)}function x(n){d={passage:!!(typeof p<"u"&&p.getSettings?p.getSettings():{}).readingMockTestMode?{...n,questions:(n.questions||[]).filter(r=>r.format_role==="primary"||!r.format_role)}:n,phase:"read",answers:{},idx:0}}async function L(n,e){await A();const l=e?decodeURIComponent(e):"";if(l){const r=($.passages||[]).find(t=>t.id===l);if(r)return(!d||d.passage?.id!==r.id)&&x(r),k(n)}return d?k(n):f(n)}function f(n){const e=$.passages||[],o=!!(typeof p<"u"&&p.getSettings?p.getSettings():{}).readingMockTestMode,r=e.map(t=>`
      <li>
  ...
  `,document.getElementById("reading-mock-mode").addEventListener("change",t=>{p.setSettings({readingMockTestMode:t.target.checked}),f(n)}),n.querySelectorAll("[data-id]").forEach(t=>{t.addEventListener("click",()=>{_(`reading/${encodeURIComponent(t.dataset.id)}`)})})}function k(n){const e=d.passage;return d.phase==="read"?E(n,e):d.phase==="questions"?h(n,e):y(n,e)}function E(n,e){const l=$?.passages||[],o=l.findIndex(s=>s.id===e.id),r=o>0?l[o-1]:null,t=o>=0&&o<l.length-1?l[o+1]:null,u=`
  ...
  `;n.innerHTML=`
    <article class="reading-passage">
      ...
      <h2>${i(e.title_ja)}</h2>
      <p class="muted small">\u30EC\u30D9\u30EB: ${i(b[e.level]||e.level)} \u30FB \u30C8\u30D4\u30C3\u30AF: ${i(j[e.topic]||e.topic)}</p>
      <div class="passage-text">${i(e.ja)}</div>
      ${e.cultural_context?`
        <aside class="reading-cultural-context">
          <p class="muted small"><strong>${a(g("dokkai_detail.cultural_context"))}:</strong> ${a(e.cultural_context)}</p>
        </aside>
      `:""}
      ${e.authentic_categories?.length?`
        ...
        <aside class="reading-authentic-link muted small">
          ...
          ${e.authentic_categories.map(s=>`<a href="#/authentic" class="authentic-cat-chip">${a(s)}</a>`).join(" ")}
        </aside>
      `:""}
      ${Array.isArray(e.paragraphs)&&e.paragraphs.some(s=>s.summary_en)?`
        ...
        <details class="reading-paragraph-summaries">
          ...
          <ol>
            ${e.paragraphs.map(s=>s.summary_en?`<li class="muted small">${a(s.summary_en)}</li>`:"").join("")}
          </ol>
        </details>
      `:""}
      ${Array.isArray(e.reflection_prompts)&&e.reflection_prompts.length?`
        ...
        <aside class="reading-reflection-prompts">
          ...
          <ul>
            ${e.reflection_prompts.map(s=>`<li><span class="reflection-type-chip muted small">${a(s.type||"reflect")}</span> ${a(s.prompt_en||s.prompt_ja||"")}</li>`).join("")}
          </ul>
        </aside>
      `:""}
      ${w(e)}
      ${(()=>{const s=e.time_target_seconds;return!s||typeof s!="object"?"":`
          <aside class="reading-time-target muted small">
            ...
            ${s.reading_seconds?` (${a(g("dokkai_detail.read"))}: ${a(s.reading_seconds)}s`:""}${s.comprehension_seconds?` + ${a(g("dokkai_detail.comprehend"))}: ${a(s.comprehension_seconds)}s)`:s.reading_seconds?")":""}
            ${s.note?`<br><span class="muted small">${a(s.note)}</span>`:""}
          </aside>
        `})()}
      ${(()=>{const s=Array.isArray(e.comprehension_strategy_hints)?e.comprehension_strategy_hints:[];return s.length?`
          <aside class="reading-strategy-hints">
            <details>
              <summary><strong>${a(g("dokkai_detail.strategy_hints"))} (${s.length})</strong></summary>
              <ul>
                ${s.map(c=>`<li class="muted small">${a(c)}</li>`).join("")}
              </ul>
            </details>
          </aside>
        `:""})()}
      ${(()=>{const s=e.register_signal;return!s||typeof s!="object"||!s.register?"":`
          <p class="muted small reading-register-signal">
            <strong>${a(g("dokkai_detail.register"))}:</strong> ${a(s.register)}${s.confidence?` <span class="muted">(${a(s.confidence)} ${a(g("dokkai_detail.confidence"))})</span>`:""}
            ${Array.isArray(s.signals)&&s.signals.length?`<br><span class="muted small">${a(g("dokkai_detail.signals"))}: ${s.signals.map(a).join("; ")}</span>`:""}
          </p>
        `})()}
      ${(()=>{const s=e.target_reading_age;return!s||typeof s!="object"?"":`
          <p class="muted small reading-target-age">
            <strong>${a(g("dokkai_detail.native_reading_level"))}:</strong> ${a(g("dokkai_detail.age"))} ${a(s.native_equivalent_age_years||"?")}
            ${s.kanji_ratio!=null?` <span class="muted">(${(s.kanji_ratio*100).toFixed(1)}% ${a(g("dokkai_detail.kanji"))}, ${a(s.char_count||"?")} ${a(g("dokkai_detail.chars"))})</span>`:""}
          </p>
        `})()}
      ${(()=>{const s=Array.isArray(e.discourse_markers_used)?e.discourse_markers_used:[];return s.length?`
          <p class="muted small reading-discourse-markers">
            <strong>${a(g("dokkai_detail.discourse_markers"))}:</strong>
            ${s.map(c=>`<span class="discourse-marker-chip" lang="ja">${a(c)}</span>`).join(" ")}
          </p>
        `:""})()}
          ...
          <div class="reading-translation-pair">
            ${e.translation_literal?`
              <section class="reading-translation-block">
                ...
                <p class="reading-translation-text">${a(e.translation_literal)}</p>
              </section>
            `:""}
            ${e.translation_natural?`
              <section class="reading-translation-block">
                <h4 class="reading-translation-label">Natural</h4>
                <p class="reading-translation-text">${a(e.translation_natural)}</p>
              </section>
            `:""}
      ...
      ${e.audio?`
        <div class="reading-audio">
          ...
          <audio controls preload="none" src="${a(v(e.audio))}">Your browser does not support audio.</audio>
        </div>
      `:""}
    ...
    <article class="reading-passage">
      ...
      <div class="question-card">
        ...
        <div class="choice-grid">
          ${r.choices.map(c=>{let m="choice-button";return u?c===r.correctAnswer?m+=" correct-choice":c===t&&(m+=" wrong-choice"):t===c&&(m+=" selected"),`<button data-pick="${a(c)}" class="${m}" ${u?"disabled":""}>${i(c)}</button>`}).join("")}
        </div>
        ${u?`
          <div class="drill-feedback ${s?"correct":"incorrect"}">
            <div class="feedback-headline">${s?i("\u305B\u3044\u304B\u3044"):i("\u3056\u3093\u306D\u3093")}</div>
            ${r.explanation_en?`<p class="muted small">${a(r.explanation_en)}</p>`:""}
  ...
  `,document.getElementById("reading-back-list").addEventListener("click",()=>{d=null,f(n)})}function a(n){return String(n??"").replace(/[&<>"']/g,e=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[e])}function w(n){const e=Array.isArray(n.grammar_footnotes)?n.grammar_footnotes:[];if(e.length===0)return"";const l=(n.ja||"").split("\u3002").filter(t=>t.trim()).map(t=>t.trim()),o=new Map;for(const t of e){const u=t.sentence_index??0;o.has(u)||o.set(u,[]),o.get(u).push(t)}const r=[...o.entries()].sort((t,u)=>t[0]-u[0]).map(([t,u])=>{const s=(l[t]||"").slice(0,50)+(l[t]&&l[t].length>50?"\u2026":"\u3002"),c=u.map(m=>`
    ...
    `).join("");return`
      <li class="reading-footnote-group">
        <p class="reading-footnote-sentence" lang="ja">
          <span class="muted small">Sentence ${t+1}:</span> ${a(s)}
        </p>
