import{renderJa as t}from"./furigana.js";import*as l from"./storage.js";const k={easy:"\u3084\u3055\u3057\u3044",medium:"\u3075\u3064\u3046","info-search":"\u3058\u3087\u3046\u307B\u3046\u3051\u3093\u3055\u304F"},b={"self-introduction":"\u3058\u3053\u3057\u3087\u3046\u304B\u3044","daily routine":"\u307E\u3044\u306B\u3061\u306E \u305B\u3044\u304B\u3064","weekend plan":"\u3057\u3085\u3046\u307E\u3064\u306E \u3088\u3066\u3044",weekend:"\u3057\u3085\u3046\u307E\u3064",shopping:"\u304B\u3044\u3082\u306E",family:"\u304B\u305E\u304F",weather:"\u3066\u3093\u304D",schedule:"\u3088\u3066\u3044",transport:"\u3053\u3046\u3064\u3046",hobby:"\u3057\u3085\u307F",school:"\u5B66\u6821",food:"\u305F\u3079\u3082\u306E",travel:"\u308A\u3087\u3053\u3046",health:"\u3051\u3093\u3053\u3046",study:"\u3079\u3093\u304D\u3087\u3046",people:"\u3072\u3068",request:"\u304A\u306D\u304C\u3044",room:"\u3078\u3084",directions:"\u307F\u3061\u3042\u3093\u306A\u3044"};let c=null,o=null;async function y(){return c||(c=await(await fetch("data/reading.json")).json(),c)}function j(s){o={passage:!!(typeof l<"u"&&l.getSettings?l.getSettings():{}).readingMockTestMode?{...s,questions:(s.questions||[]).filter(a=>a.format_role==="primary"||!a.format_role)}:s,phase:"read",answers:{},idx:0}}async function x(s,e){await y();const d=e?decodeURIComponent(e):"";if(d){const a=(c.passages||[]).find(n=>n.id===d);if(a)return(!o||o.passage?.id!==a.id)&&j(a),v(s)}return o?v(s):f(s)}function f(s){const e=c.passages||[],i=!!(typeof l<"u"&&l.getSettings?l.getSettings():{}).readingMockTestMode,a=e.map(n=>`
      <li>
        <button class="reading-pick" data-id="${g(n.id)}">
  ...
  `,document.getElementById("reading-mock-mode").addEventListener("change",n=>{l.setSettings({readingMockTestMode:n.target.checked}),f(s)}),s.querySelectorAll("[data-id]").forEach(n=>{n.addEventListener("click",()=>{location.hash=`#/reading/${encodeURIComponent(n.dataset.id)}`})})}function v(s){const e=o.passage;return o.phase==="read"?q(s,e):o.phase==="questions"?$(s,e):h(s,e)}function q(s,e){const d=c?.passages||[],i=d.findIndex(u=>u.id===e.id),a=i>0?d[i-1]:null,n=i>=0&&i<d.length-1?d[i+1]:null,p=`
    <nav class="reading-nav" aria-label="Passage navigation">
      ${a?`<a href="#/reading/${encodeURIComponent(a.id)}" title="${g(a.title_ja||a.id)}">\u2190 <span lang="ja">${t(a.title_ja||a.id)}</span></a>`:"<span></span>"}
      ${n?`<a href="#/reading/${encodeURIComponent(n.id)}" title="${g(n.title_ja||n.id)}"><span lang="ja">${t(n.title_ja||n.id)}</span> \u2192</a>`:"<span></span>"}
    </nav>
  `;s.innerHTML=`
    <article class="reading-passage">
      ...
      ${e.audio?`
        <div class="reading-audio">
          ...
          <audio controls preload="none" src="${g(e.audio)}">Your browser does not support audio.</audio>
        </div>
      `:""}
      <button id="reading-start-q" class="btn-primary">${t("\u3057\u3064\u3082\u3093\u3092 \u306F\u3058\u3081\u308B")} (${e.questions.length})</button>
      ${p}
    </article>
  `,document.getElementById("reading-back").addEventListener("click",u=>{u.preventDefault(),o=null,location.hash="#/reading"}),document.getElementById("reading-start-q").addEventListener("click",()=>{o.phase="questions",$(s,e)})}function $(s,e){const d=e.questions.length,i=o.idx,a=e.questions[i],n=o.answers[a.id],p=n!=null,u=n===a.correctAnswer;s.innerHTML=`
    <article class="reading-passage">
      <div class="srs-progress">
      ...
      <div class="question-card">
        ...
        <div class="choice-grid">
          ${a.choices.map(r=>{let m="choice-button";return p?r===a.correctAnswer?m+=" correct-choice":r===n&&(m+=" wrong-choice"):n===r&&(m+=" selected"),`<button data-pick="${g(r)}" class="${m}" ${p?"disabled":""}>${t(r)}</button>`}).join("")}
        </div>
        ${p?`
          <div class="drill-feedback ${u?"correct":"incorrect"}">
            <div class="feedback-headline">${u?t("\u305B\u3044\u304B\u3044"):t("\u3056\u3093\u306D\u3093")}</div>
            ${a.explanation_en?`<p class="muted small">${g(a.explanation_en)}</p>`:""}
  ...
  `,document.getElementById("reading-back-list").addEventListener("click",()=>{o=null,f(s)})}function g(s){return String(s??"").replace(/[&<>"']/g,e=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[e])}export{x as renderReading};
