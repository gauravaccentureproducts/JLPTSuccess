import{renderJa as U}from"./furigana.js";import*as c from"./storage.js";import{t as T}from"./i18n.js";import{navigateTo as _}from"./router.js";const A=10,C=50;let o=null,v="setup",k=null,w=null,y=null,$=null,x=null;async function B(){if(k&&w&&y&&x)return;const[t,a,i,r]=await Promise.all([fetch("data/grammar.json").then(e=>e.json()),fetch("data/questions.json").then(e=>e.json()),fetch("data/vocab.json").then(e=>e.json()),fetch("data/kanji.json").then(e=>e.json())]);k=new Map((t.patterns||[]).map(e=>[e.id,e])),w=new Map;for(const e of a.questions||[])w.has(e.grammarPatternId)||w.set(e.grammarPatternId,[]),w.get(e.grammarPatternId).push(e);y=new Map,$=new Map;for(const e of i.entries||[])e.form&&y.set(e.form,e),e.id&&$.set(e.id,e);x=new Map((r.entries||[]).map(e=>[e.glyph,e]))}function q(t){const a=c.getHistory(),i=new Date,r=[];for(const[e,n]of Object.entries(a))n.isMastered||n.nextDue&&new Date(n.nextDue)<=i&&r.push({pid:e,entry:n,isNew:!1});return r.sort((e,n)=>new Date(e.entry.nextDue)-new Date(n.entry.nextDue)),r.slice(0,t)}function P(t,a){const i=c.getHistory(),r=new Set([...a.map(n=>n.pid),...Object.keys(i)]),e=[];for(const[n]of k)if(!r.has(n)){if(e.length>=t)break;e.push({pid:n,entry:null,isNew:!0})}return e}async function F(t){return await B(),v==="finished"&&(v="setup",o=null),v==="session"&&o?I(t):v==="finished"&&o?S(t):M(t)}function M(t){v="setup";const a=c.getSettings(),i=a.dailyNewLimit??A,r=a.dailyReviewCap??C,e=q(r),n=P(i,e),u=c.getDueCountsBySkill(),m=u.grammar+u.vocab+u.kanji;t.innerHTML=`
  ...
  `,document.getElementById("srs-restart")?.addEventListener("click",()=>{o=null,v="setup",M(t)}),document.getElementById("srs-back")?.addEventListener("click",()=>{_("learn")})}function d(t){return String(t??"").replace(/[&<>"']/g,a=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[a])}export{F as renderReview};
