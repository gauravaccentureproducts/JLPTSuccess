import{renderJa as f}from"./furigana.js";import*as v from"./storage.js";let p=null,d=null,n=null,g="setup";const y=10;async function k(){return p||(p=(await(await fetch("data/questions.json")).json()).questions||[],p)}async function I(){if(d)return d;const e=await(await fetch("data/grammar.json")).json();return d=new Map((e.patterns||[]).map(s=>[s.id,s])),d}async function B(t){return await Promise.all([k(),I()]),g==="attempting"&&n?l(t):g==="results"&&n?$(t):D(t)}function D(t){g="setup";const e=v.getSettings(),s=e.diagnosticCompleted;t.innerHTML=`
    <h2>Diagnostic Test</h2>
    <div class="diagnostic-setup">
      <p><strong>${y} questions</strong> sampled across the highest-frequency N5 categories. The diagnostic gives you a quick map of which patterns to practice - without affecting your test score history.</p>
      <ul>
      ...
      ${s?`<p class="muted">You already took the diagnostic on ${M(e.lastDiagnosticDate)}. Re-taking it will replace any current SRS state for sampled patterns.</p>`:""}
  ...
  `,document.getElementById("start-diagnostic").addEventListener("click",()=>S(t)),document.getElementById("skip-diagnostic").addEventListener("click",()=>{v.setSettings({diagnosticCompleted:!0}),location.hash="#/learn"})}function S(t){const e=x(p,y);if(e.length===0){t.innerHTML=`
      <h2>Diagnostic Test</h2>
      <div class="placeholder"><p>No questions available. Add to <code>data/questions.json</code> first.</p></div>
    `;return}n={questions:e,answers:{},currentIdx:0,startedAt:new Date().toISOString()},g="attempting",l(t)}function x(t,e){const s=new Map;for(const r of t){const o=d.get(r.grammarPatternId)?.category||"?";s.has(o)||s.set(o,[]),s.get(o).push(r)}for(const r of s.values())w(r);const i=[...s.entries()];w(i);const c=[];let a=0;for(;c.length<e&&i.some(([,r])=>r.length>0);){const[,r]=i[a%i.length];if(r.length>0&&c.push(r.shift()),a++,a>e*50)break}return c}function w(t){for(let e=t.length-1;e>0;e--){const s=Math.floor(Math.random()*(e+1));[t[e],t[s]]=[t[s],t[e]]}return t}function l(t){const e=n.questions.length,s=n.questions[n.currentIdx],i=n.questions.filter(r=>!T(r)).length,c=i===0;let a="";s.type==="mcq"||s.type==="dropdown"?a=A(s):s.type==="sentence_order"&&(a=E(s)),t.innerHTML=`
  ...
  `,document.getElementById("prev-q")?.addEventListener("click",()=>b(n.currentIdx-1,t)),document.getElementById("next-q")?.addEventListener("click",()=>b(n.currentIdx+1,t)),document.getElementById("finish-diagnostic")?.addEventListener("click",()=>C(t)),t.querySelectorAll("[data-choice]").forEach(r=>{r.addEventListener("click",()=>{n.answers[s.id]=r.dataset.choice,l(t)})}),t.querySelectorAll("[data-tile-add]").forEach(r=>{r.addEventListener("click",()=>q(s,r.dataset.tileAdd,t))}),t.querySelectorAll("[data-tile-remove]").forEach(r=>{r.addEventListener("click",()=>L(s,parseInt(r.dataset.tileRemove,10),t))})}function A(t){const e=n.answers[t.id];return`<div class="choice-grid">${(t.choices||[]).map(s=>`<button type="button" data-choice="${m(s)}" class="choice-button ${e===s?"selected":""}">${f(s)}</button>`).join("")}</div>`}function E(t){const e=n.answers[t.id]||[],s=(t.tiles||[]).filter(a=>!e.includes(a)),i=e.length?e.map((a,r)=>`<button type="button" data-tile-remove="${r}" class="tile ordered">${f(a)}</button>`).join(""):'<span class="tile-placeholder">Click tiles below to build the sentence</span>',c=s.map(a=>`<button type="button" data-tile-add="${m(a)}" class="tile">${f(a)}</button>`).join("");return`<div class="sentence-order"><div class="ordered-tray">${i}</div><div class="tile-pool">${c}</div></div>`}function T(t){const e=n.answers[t.id];return t.type==="sentence_order"?Array.isArray(e)&&e.length===(t.tiles?.length||0):e!=null&&e!==""}function b(t,e){t<0||t>=n.questions.length||(n.currentIdx=t,l(e))}function q(t,e,s){n.answers[t.id]||(n.answers[t.id]=[]),!n.answers[t.id].includes(e)&&(n.answers[t.id].push(e),l(s))}function L(t,e,s){Array.isArray(n.answers[t.id])&&(n.answers[t.id].splice(e,1),n.answers[t.id].length===0&&delete n.answers[t.id],l(s))}function j(t,e){if(t.type==="sentence_order"){if(!Array.isArray(e))return!1;const s=t.correctOrder||[];return e.length===s.length&&e.every((i,c)=>i===s[c])}return e===t.correctAnswer}function C(t){const e=n.questions.map(s=>{const i=n.answers[s.id];return{questionId:s.id,grammarPatternId:s.grammarPatternId,type:s.type,userAnswer:i,correctAnswer:s.correctAnswer??s.correctOrder,isCorrect:j(s,i)}});v.recordTestResponses(e),v.setSettings({diagnosticCompleted:!0,lastDiagnosticDate:new Date().toISOString()}),n.responses=e,g="results",$(t)}function $(t){const e=n.responses,s=e.length,i=e.filter(o=>o.isCorrect).length,c=s-i,a=[...new Set(e.filter(o=>!o.isCorrect).map(o=>o.grammarPatternId))],r=a.map(o=>{const u=d.get(o);return`<li><a href="#/learn/${encodeURIComponent(o)}">${m(u?.pattern||o)}</a> - ${m(u?.meaning_en||"")}</li>`}).join(""),h=new Set;for(const o of n.questions){const u=d.get(o.grammarPatternId);u?.category&&h.add(u.category)}t.innerHTML=`
  ...
  `,document.getElementById("diag-back")?.addEventListener("click",()=>{location.hash="#/learn"})}function M(t){if(!t)return"";try{return new Date(t).toLocaleDateString()}catch{return t}}function m(t){return String(t??"").replace(/[&<>"']/g,e=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[e])}export{B as renderDiagnostic};
